...
- `JSZip`도 ZIP 프리뷰/실행을 선택할 때만 동적 로딩합니다. 일반 URL 게임과 런처의 메인 번들은 Node 22 프로덕션 기준 `415.62 kB`(`124.79 kB` gzip)이며 ZIP 전용 `97.54 kB` 청크와 분리됩니다.
...
- 평상시 캐러셀 탐색은 주소를 바꾸지 않고 탭 세션에만 마지막 선택을 기억합니다. `선택 링크 복사`를 눌렀을 때만 `?demo=<gameId>` 공유 URL을 만들며, 예전 `#demo=<gameId>` 링크는 같은 쿼리 형식으로 자동 이전합니다. 사용자가 공유 링크에서 다른 데모를 탐색하면 선택 쿼리를 제거해 주소와 화면이 어긋나지 않게 합니다.
