...
- 화자 강조는 데스크톱/모바일 공통 규칙으로 동작합니다. 현재 화자는 가장 앞의 z축과 원래 불투명도를 유지하고 비화자는 합성 가능한 불투명도로 낮추되, 개별 캐릭터 크기는 바꾸지 않습니다. 다인 `close`에서는 주변 인물 페이드가 시작된 72ms 뒤 카메라 이동을 겹쳐 한 호흡으로 전환하고, `reaction`에서는 카메라 대상인 듣는 인물을 전면에 둡니다.
