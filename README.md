...
- 스티커·증거물은 HUD·대사창·기기 safe-area를 제외한 공통 안전 프레임을 좌표 기준으로 사용합니다. 런타임이 아이템과 현재 보이는 캐릭터의 실제 렌더 경계를 첫 페인트 전에 측정해, 겹치지 않으면 작성 좌표를 유지하고 겹칠 때만 가장 가까운 좌·우·상단 빈 공간으로 옮깁니다. 카메라 전환·캐릭터 교체·프레임 리사이즈 중에도 다시 계산하며, 빈 공간이 부족할 때만 종횡비를 유지한 채 단계적으로 축소합니다.
