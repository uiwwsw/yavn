...
- 카메라·등장 인물 변경으로 스티커 충돌 가능성이 생기면 기존 좌표를 같은 페인트에서 미확정 상태로 숨기고, 인물 경계가 안정된 뒤 충돌 없는 위치에서 160ms 페이드로 다시 노출합니다. 캐릭터 등장은 `char.enter`로 컷·페이드·팝·슬라이드·와이프·블러·회전을 고르고, `layout: push`일 때만 기존 인물이 자연스럽게 자리를 내줍니다. 카메라 `push`는 확대만 보간하며 명시한 `pan`만 수평 이동합니다.
...
- 다이얼로그 수동 숨김/복원이나 높이 변화는 `prompt-top` 캐릭터 기준선과 다음 스티커의 최초 안전 배치에 반영됩니다. 이 변화만으로는 이미 노출되어 위치가 잠긴 스티커와 `stage-bottom` 캐릭터 좌표를 다시 계산하지 않습니다.
...
- 스티커·증거물은 HUD·대사창·기기 safe-area를 제외한 공통 안전 프레임을 좌표 기준으로 사용합니다. 런타임이 아이템과 현재 보이는 캐릭터의 실제 렌더 경계를 숨김 상태에서 연속 측정해 캐릭터 입장·슬롯 이동·카메라 전환이 실제로 멈춘 것을 확인한 뒤, 겹치지 않으면 작성 좌표를 유지하고 겹칠 때만 가장 가까운 좌·우·상단 빈 공간을 최종 위치로 선택합니다. 최초 좌표·크기는 확정 뒤 제자리에서 입장 이펙트를 시작합니다. 노출 중 카메라나 등장 인물이 바뀌면 이전 좌표를 같은 페인트에서 미확정 상태로 숨겨 이동 중인 캐릭터와 겹치는 프레임을 차단하고, 경계가 안정된 뒤 기존 좌표가 안전하면 그대로, 충돌하면 가장 가까운 안전 위치에서 160ms 페이드로 다시 노출합니다. 대사 타이핑과 단순 화자 변경은 배치를 다시 계산하지 않습니다. `clearSticker.leave`는 같은 렌더 인스턴스와 마지막 확정 좌표를 유지한 채 퇴장 클래스로만 전환하므로, 재측정 때문에 잠시 숨거나 다시 나타나지 않습니다. 기기 회전이나 큰 PC 프레임 변경 때만 숨긴 상태에서 한 번 다시 계산하며, 모바일 주소창 높이 변화는 무시합니다. 빈 공간이 부족할 때만 종횡비를 유지한 채 단계적으로 축소합니다.
...
- 다이얼로그를 수동으로 숨기면 하단 안전 inset을 `0`으로 갱신해 `prompt-top` 캐릭터 기준선을 화면 바닥으로 부드럽게 내리고, 다시 열면 실측 프롬프트 상단으로 복원합니다. 이 숨김/복원만으로는 이미 노출된 스티커의 잠긴 좌표와 `stage-bottom` 캐릭터 좌표를 바꾸지 않습니다.
