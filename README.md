...
- `with`는 같은 무대에 배치된 배우를 퇴장시키지 않고 현재 카메라에서만 숨깁니다. 잠시 화면 밖에 있던 인물이 다음 대사에서 다시 잡혀도 DOM/Live2D 인스턴스와 물리적 슬롯을 유지하므로 `characterEnter`나 위치 이동을 반복하지 않습니다. 실제 `char` 배치로 새 배우가 무대에 들어올 때만 등장 모션을 실행합니다.
