...
- `/game-list/:gameId` 직접 진입 시 설정 프리뷰와 본편을 준비하는 동안에는 비대화형 부트 화면을 유지합니다. 런처나 빈 게임 HUD가 먼저 노출되지 않으며, 지연된 타이틀·버튼 애니메이션은 첫 키프레임을 첫 페인트부터 유지해 버튼이 잠깐 나타났다 사라지는 현상을 방지합니다.
