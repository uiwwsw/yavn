...
- `/game-list/deokman/`은 **선덕여왕: 죽은 공주의 왕관**의 정식 12챕터 완결판입니다. V9.0에서는 모든 대사·내레이션·선택 문구를 설명형 역사 문답에서 인물의 욕망, 행동, 위기와 여운이 이어지는 시대극 문장으로 전면 개작했습니다. V9.1은 덕만을 `어린 공주 → 10년 뒤 국경 유랑자 → 신분을 되찾은 공주 → 선덕왕`으로 시각적으로 성장시켰습니다. V9.5는 생존 위기와 일반 판단 게이트에 정답을 1개만 두되, 오답을 누르자마자 판정 카드를 띄우지 않습니다. 선택의 행동, 덕만의 독백이나 상대 반응, 죽음을 확정하는 기록까지 짧은 실패 후일담으로 보여 준 뒤 게임오버를 표시합니다. 지연 사망의 원인 선택 복귀와 11번째 히든 엔딩도 유지합니다.
- 전 작품에 고유한 선택 36개, 기존 장면형 12개와 짧은 실패 후일담형 16개를 합친 `gameOver` 28개, 누적 관계·정치 상태로 판정되는 결말 11개를 제공합니다. 모든 실패는 최소 네 번의 대사·내레이션·기록 흐름을 거치며, 초반의 관찰과 증거가 후반의 왕위 표결과 최종 기록 공개 여부까지 다시 작동합니다.
