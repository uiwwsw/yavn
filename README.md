...
- 다이얼로그 수동 숨김/복원이나 높이 변화는 `prompt-top` 캐릭터 기준선과 다음 스티커의 최초 안전 배치에 반영됩니다. 이미 노출되어 위치가 잠긴 스티커와 `stage-bottom` 캐릭터 좌표는 다시 계산하지 않습니다.
...
- 화자 강조는 데스크톱/모바일, 정적 이미지/Live2D에 같은 규칙으로 동작합니다. 화면에 남아 있는 화자와 비화자는 모두 원래 불투명도를 유지하고, 현재 대사 화자에게만 바닥 기준의 `3.2s` 미세 상하·스케일 호흡 모션을 반복합니다. `reaction`은 듣는 카메라 대상을 전면에 두되 주변 인물을 흐리지 않으며, 다인 `close`의 주변 인물 페이드는 화자 강조가 아니라 쇼트 밖 퇴장 연출로만 유지됩니다.
...
- 화자 시각 강조는 플랫폼별 분기 없이 실제 `dialog.speakerId`에만 `3.2s` 미세 호흡 모션을 적용합니다. `speakerOrder`는 합성 순서에만 사용하며, 같은 쇼트의 비화자는 불투명도를 낮추지 않습니다. 모션 감소 환경에서는 호흡 모션을 자동으로 끕니다.
...
- 일반 다이얼로그 박스 최대 높이는 게임 화면 가림을 줄이기 위해 중앙 플레이 프레임 높이의 `데스크톱 38%`(38cqh), `모바일 48%`(48cqh)로 제한하고, 초과 내용은 내부 스크롤로 처리합니다. 선택지가 열리면 데스크톱 `46cqh`, 모바일 `64cqh`까지 확장하고 일반 대사용 최소 높이를 해제해 짧은 프롬프트가 빈 공간을 차지하지 않으며 마지막 옵션을 우선 노출합니다.
...
- 다이얼로그를 수동으로 숨기면 하단 안전 inset을 `0`으로 갱신해 `prompt-top` 캐릭터 기준선을 화면 바닥으로 부드럽게 내리고, 다시 열면 실측 프롬프트 상단으로 복원합니다. 이미 노출된 스티커의 잠긴 좌표와 `stage-bottom` 캐릭터 좌표는 유지합니다.
