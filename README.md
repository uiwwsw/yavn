...
- 여러 인물이 보이는 `close`의 비즉시 전환은 비대상 인물을 최대 180ms 동안 페이드하고, 카메라는 72ms 뒤부터 같은 전환 안에서 겹쳐 움직입니다. 퇴장과 줌이 따로 끊겨 보이지 않으면서도 가장자리 실루엣이 오래 남지 않으며, 1인 `close`와 `cut`은 지연하지 않습니다.
...
- 스티커·증거물은 HUD·대사창·기기 safe-area를 제외한 공통 안전 프레임을 좌표 기준으로 사용합니다. 런타임이 아이템과 현재 보이는 캐릭터의 실제 렌더 경계를 숨김 상태에서 연속 측정해 캐릭터 입장·슬롯 이동·카메라 전환이 실제로 멈춘 것을 확인한 뒤, 겹치지 않으면 작성 좌표를 유지하고 겹칠 때만 가장 가까운 좌·우·상단 빈 공간을 최종 위치로 선택합니다. 좌표·크기를 확정한 뒤에만 입장 이펙트를 시작하며, 노출 중에는 안전한 좌표를 유지하되 카메라나 등장 인물 변경 뒤 새 충돌이 생긴 경우에만 한 번 다시 보정합니다. `clearSticker.leave`는 같은 렌더 인스턴스와 마지막 확정 좌표를 유지한 채 퇴장 클래스로만 전환하므로, 재측정 때문에 잠시 숨거나 다시 나타나지 않습니다. 기기 회전이나 큰 PC 프레임 변경 때만 숨긴 상태에서 한 번 다시 계산하며, 모바일 주소창 높이 변화는 무시합니다. 빈 공간이 부족할 때만 종횡비를 유지한 채 단계적으로 축소합니다.
...
- `close/reaction`은 대상 앵커를 화면 중앙으로 옮기는 명시적 수평 팬과 단일 확대를 함께 전환합니다. 이전처럼 `transform-origin`을 순간 변경하거나 구성 배율과 쇼트 배율을 중첩하지 않으므로, 같은 배율의 좌우 대상 전환도 실제 팬으로 이어지고 `medium ↔ close`에서 크기가 단계적으로 튀지 않습니다. 단독 배우는 모든 쇼트에서 중앙 앵커를 사용합니다.
...
- `with`는 같은 무대에 배치된 배우를 퇴장시키지 않고 현재 카메라에서만 이미지·Live2D 공통 180ms 페이드로 숨깁니다. 페이드가 끝난 뒤 `visibility: hidden`을 적용하고, 퇴장 중에는 마지막으로 보였던 PC·모바일 앵커와 시선 방향을 유지하므로 새 1인/2인 구도 위치로 미끄러지거나 뒤집힌 뒤 사라지지 않습니다. 빠른 연속 노출 변경은 다음 페인트 프레임에 마지막 구성만 커밋해 중간 배우가 번쩍이지 않으며, 일반 `wide/medium`의 2→1·3→2 퇴장은 페이드가 끝난 뒤 남은 배우의 레이아웃 이동을 시작합니다. 잠시 화면 밖에 있던 인물이 다음 대사에서 다시 잡혀도 DOM/Live2D 인스턴스와 물리적 슬롯을 유지해 `characterEnter`를 반복하지 않습니다. 로딩 중 최초 구성은 지연하지 않고, `prefers-reduced-motion`에서는 레이아웃 대기와 모션을 생략합니다.
...
- 정말 새로 등장한 인물만 최종 위치에서 240ms 페이드로 나타나며 위로 튀는 바운스는 사용하지 않습니다. 이미 보이던 동일 인물의 위치와 크기는 합성 전용 `translate/scale`로 이동하고, 좌우 반전은 중간에 납작해지는 모양을 막기 위해 즉시 바뀝니다. 카메라는 단일 절대 배율과 별도 수평 팬을 같은 시간축으로 전환해 `left/width/filter/transform-origin` 재계산을 피합니다. 캐릭터 표정은 사전 디코딩 후 비동기 표시하며, 모든 모션은 `prefers-reduced-motion`에서 비활성화됩니다.
