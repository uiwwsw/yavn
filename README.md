...
- `src/engine.ts`: 챕터 전환, 저장 슬롯/백업, 새로고침에도 유지되는 선택 복구점, 게임오버·다중 엔딩 재탐색, 장면 단위 무대 카메라, 캐릭터 원본 정렬, 연출 실행, 입력 게이트, 디코드 동기화 배경 전환, 오디오 BGM 크로스페이드
...
- `/game-list/deokman/`은 **선덕여왕: 죽은 공주의 왕관** V9.6의 정식 12챕터 완결판입니다. 덕만은 `어린 공주 → 10년 뒤 국경 유랑자 → 신분을 되찾은 공주 → 선덕왕`으로 성장하며, 앞서 관찰한 탈출로·선택한 통치 방식·쌓은 관계가 뒤 장의 선택 공개와 인물 반응을 바꿉니다. V9.6은 하나뿐인 정답을 찾는 과반 사망 구조를 버리고, 11개 위기마다 최소 두 가지의 서로 다른 생존 전략과 하나의 극화된 실패를 둡니다. 일반 정치 선택도 여러 실행 가능한 대가를 남겨 플레이어가 정답 암기보다 덕만의 통치 방식을 만들게 합니다.
- 전 작품에 고유한 선택 36개, 선택 직후 끝내지 않고 최소 네 번의 대사·내레이션·기록을 거치는 장면형 `gameOver` 13개, 누적 관계·정치 상태로 판정되는 결말 11개를 제공합니다. 게임오버의 원인 선택 복귀를 지원하며, 초반의 관찰과 증거가 후반의 왕위 표결과 최종 기록 공개 여부까지 다시 작동합니다.
...
- 시스템 탭의 자동저장 토글도 같은 게임별 설정 키에 저장됩니다. 자동저장은 게임오버의 원인 선택 복구와 시작 화면 이어하기에 사용하고, 일반 화면에서는 직접 불러오기 버튼을 표시하지 않습니다. PC와 모바일 모두 저장 카드 영역만 스크롤하며 배경음악·`게임 시작 화면으로 가기`·상태 문구는 하단 고정 행에 유지합니다.
...
- 화자 시각 강조는 플랫폼별 분기 없이 실제 `dialog.speakerId`에만 `2.2s` 미세 호흡 모션을 적용합니다. 모션은 좌우 반전용 `transform`과 분리된 위치·스케일을 하단 중앙 기준으로 움직이고, 정점에서 최대 `3%` 밝기와 옅은 웜 글로우를 더합니다. 화자가 바뀌면 직전 화자는 진행 중인 호흡을 중립 프레임까지 마친 뒤 정지해 중간 변형값에서 튀지 않습니다. `speakerOrder`는 합성 순서에만 사용하며, 같은 쇼트의 비화자는 불투명도를 낮추지 않습니다. 모션 감소 환경에서는 호흡 모션을 자동으로 끕니다.
- 챕터 로딩은 현재 챕터가 실제로 참조하는 시각 에셋만 첫 장면 순서대로 제한 병렬 프리로드하고, 전송뿐 아니라 이미지 디코딩 완료까지 확인합니다. 첫 장면을 DOM에 반영한 뒤에도 실제 배경·정적 캐릭터와 Live2D가 준비됐는지 다시 확인한 다음 로딩을 닫으므로, 로딩 직후 캐릭터가 뒤늦게 나타나지 않습니다. 플레이 중 `bg`가 바뀌면 새 이미지를 숨김 레이어에서 먼저 로드·디코드하고 기존 배경을 유지한 채 360ms 동안 겹쳐 전환합니다. 빠르게 연속된 이전 요청의 완료는 무시하고, 새 에셋이 실패하면 직전 배경을 보존합니다. 이미 준비한 URL은 세션 캐시에서 재사용하며 오디오/비디오는 브라우저 스트리밍을 유지하고, 데이터 절약/2G에서는 동시성을 낮추고 다음 챕터 백그라운드 예열을 생략합니다.
