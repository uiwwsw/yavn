...
- 인물 10명의 같은 투명 원본에 캐릭터별 `full/bust/closeup` 보정을 등록하고, 현재 637개 화자 대사와 24개 선택 장면 전체에 구도를 지시했습니다. 206개의 명시적 등장 지시와 분기별 화면 배치 검증으로 말하는 인물이 화면에서 사라지는 경로를 막습니다. 실제로 보이는 인물과 공간은 전신, 정보 교환은 상체, 위기·감정·결단은 클로즈업으로 전환합니다.
...
- 인벤토리 모달은 `가방(획득)`/`도감(전체)` 2탭과 수집 진행 막대를 제공하며, 검색·카테고리·정렬을 두 탭에서 동일하게 사용합니다.
- `도감` 탭의 미획득 아이템은 잠금 카드(`미확인 아이템`)로 표시하고, 상세보기 팝업에서도 설명/이미지를 숨겨 스포일러를 최소화합니다.
- 인벤토리 그리드는 반응형 고정 열 수를 사용합니다: 데스크톱 5열, 태블릿 4열, 모바일 3열(400px 이하 2열).
- 인벤토리 스크롤은 슬롯 그리드 영역에만 적용합니다. 슬롯을 한 번 누르면 별도 중간 선택 단계 없이 상세 팝업이 바로 열립니다.
...
- 게임 플레이 HUD는 좌측 게임 제목 + 우측 안내 문구 구조를 유지하며, 기본 안내 문구는 `YAVN ENGINE`입니다. 모바일에서는 안내 문구를 숨기고 44px 로그/가방/저장 터치 영역을 우선합니다. (`uploading=true` 동안에는 `ZIP Loading...`)
