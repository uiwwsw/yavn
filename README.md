...
- `medium`은 모바일·PC의 기본 드라마 구도입니다. 정적 이미지는 인원수와 관계없이 원본 높이 `58cqh`를 사용하고, PC는 `wide/medium/close/reaction = 1.18/1.62/2.08/1.90`, 모바일은 `1.12/1.58/1.84/1.72`의 단일 최종 배율을 적용합니다. 세로 원점은 PC `86%`, 모바일 `82%`로 나눠 하단 대화창 위에서 얼굴과 상반신의 비중을 확보합니다. 1·2·3인 변화는 가로 앵커만 바꾸며 등록된 `framing.scale`도 인원수에 따라 약화하지 않습니다.
...
- 시작 화면은 금지된 기록과 모란패를 든 어린 덕만의 별궁 화재 키아트를 사용하며, 캐릭터 3종은 흰 배경 잔여 픽셀을 제거한 투명 WebP입니다. 모란도·사망 기록·청동 모란패는 배경 없는 SVG 오브젝트로 다시 설계했습니다.
