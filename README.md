...
- Vercel: `package.json`의 Node 22·pnpm 10.20 고정을 따르도록 설치와 빌드를 `corepack pnpm`으로 실행
- Live2D 렌더러는 실제 Live2D 캐릭터가 등장할 때 동적 로딩되어 기본 런처/2D 게임의 초기 번들에서 분리됩니다.
- 런처 캐러셀은 Embla Carousel React 안정판을 사용합니다. 이동 프레임마다 게임 선택·SEO·고지 상태를 갱신하지 않고 선택 스냅이 바뀔 때 한 번만 반영합니다.
- 게임별 `legalNotices`를 선언하면 작품이나 캐릭터 ID 분기 없이 런처·시작 화면·시스템 설정·엔딩 크레딧에 권리/출처 고지를 공통 노출합니다.
- `JSZip`도 ZIP 프리뷰/실행을 선택할 때만 동적 로딩합니다. 일반 URL 게임과 런처의 메인 번들은 Node 22 프로덕션 기준 `434.29 kB`(`132.31 kB` gzip)이며 ZIP 전용 `97.54 kB` 청크와 분리됩니다.
...
- `calibration.scale`은 `0.5..2`, `x/y`는 `-30..30`이며, 쇼트 확대와 별도로 원본의 발·눈높이 차이만 보정합니다. `spacing`(`0.75..1.25`)은 PC 앙상블에서 해당 에셋의 실루엣 폭에 맞춰 중앙으로부터의 최대 간격을 조절합니다. 기본값은 모두 `1`입니다.
...
- 배경과 전체 화면 시스템 오버레이는 항상 뷰포트를 채웁니다. 캐릭터·스티커·HUD·대사창·선택/입력·컷신·CASE FILE은 중앙의 `stage-content-frame`을 공유합니다. PC 프레임은 화면 안에서 최대한 크게 확장하되 가로세로 비율을 `4:3 ~ 16:9` 사이로 제한하므로, 세로 모니터에서는 4:3 중앙 무대가 되고 울트라와이드에서는 16:9를 넘지 않습니다. 프레임 밖은 검은 띠 대신 같은 풀블리드 배경이 이어집니다. 터치 기반 세로 모바일은 기존 최대 `9:16` 구도를 유지합니다.
...
- 화자 강조는 데스크톱/모바일 공통 규칙으로 동작합니다. 현재 화자는 가장 앞의 z축과 원래 불투명도를 유지하고 비화자는 합성 가능한 불투명도로 낮추되, 개별 캐릭터 크기는 바꾸지 않습니다. `close`에서는 주변 인물을 페이드해 잘린 손·하체가 남지 않으며 `reaction`에서는 카메라 대상인 듣는 인물을 전면에 둡니다.
- `with`를 생략한 대사·선택·입력·내레이션은 현재 무대에 배치된 인물을 기본 앙상블로 유지합니다. 모바일은 2인 25%/75%, 3인 15%/50%/85%의 예측 가능한 분할을 사용합니다. PC는 화면을 기계적으로 분할하지 않고 중앙 기준 유동 간격에 260px 상한을 둔 뒤 각 인물의 `calibration.spacing`을 반영하므로, 넓은 화면에서도 배우들이 한 장면 안에 모여 보입니다.
- 모바일 1인 `medium/close/reaction`은 캐릭터 원본이 세로를 가득 채워도 머리가 잘리지 않도록 상단 `0%`를 확대 원점으로 사용하고, `4cqh` 안전 헤드룸을 아래 방향으로 확보합니다. 2·3인 모바일 숏은 폭 제약으로 생기는 원본 상단 여백을 활용해 기존 `23%` 눈높이 원점을 유지하며 PC에는 모바일 헤드룸 이동을 적용하지 않습니다.
...
- 정말 새로 등장한 인물만 최종 위치에서 짧은 상승·페이드로 나타납니다. 이미 보이던 동일 인물의 위치는 합성 전용 `translate`로 이동하고, 좌우 반전은 중간에 납작해지는 모양을 막기 위해 즉시 바뀝니다. 카메라 확대와 대상 이동은 고정 원점의 중첩 transform 레이어로 분리되어 `left/width/filter/transform-origin` 재계산을 동반하지 않습니다. 캐릭터 표정은 사전 디코딩 후 비동기 표시하며, 모든 모션은 `prefers-reduced-motion`에서 비활성화됩니다.
...
- v3.1.0은 전 8장 679개 화자 대사를 쉬운 한국어와 투명한 감정 연기로 다시 감수했습니다. `왕경/인영/시진/지아비/정사`처럼 흐름을 막던 말을 `수도/도장 자국/두 시간/남편/나라일`로 풀고, 말보다 시선·손·소품이 먼저 속마음을 들키게 했습니다. 동시에 누룽지 다툼, 긴 소매, 말 이름, 먹물 묻은 코, 흙먼지 끼니, 신발 끈, 계피와 생강, 삐뚤어진 이름 같은 비사건 생활 대사를 더해 정치 설명 사이에 인물이 숨 쉴 자리를 만들었습니다. 평상시는 27 CPS, 고백·협박·상실·결단은 17~22 CPS로 대비합니다.
...
- v2.5.0은 관계와 동기를 설명하던 도입부를 실제 에피소드로 재구성했습니다. 진평왕이 덕만 몫의 누룽지를 남기고 정사 문서에 답을 쓰게 하는 밤 수라, 천명이 혼례 비녀를 직접 빼 주는 장면, 유신이 두 필의 말을 준비하는 선택, 진운에게 인질의 비녀가 도착하는 순간, 국경의 빈 그릇과 마지막 화백회의의 피 묻은 칼처럼 반복 소품과 행동으로 감정·관계·정치적 대가를 보여 줍니다. 장별 계획과 집필 기준은 [`docs/DEOKMAN_DRAMATURGY_PLAN.ko.md`](docs/DEOKMAN_DRAMATURGY_PLAN.ko.md)에 고정했습니다.
...
- 캐러셀은 Embla의 transform 기반 스냅 이동을 사용합니다. 이동 중 `scroll` 이벤트로 React 상태를 반복 갱신하지 않으며, `select` 이벤트에서 선택 스냅을 한 번만 확정합니다. 대표 이미지는 캐러셀 진입 전에 eager 로딩하고 슬라이드별 권리 고지 DOM을 유지해 이동 도중 이미지·고지 데이터가 교체되거나 높이가 흔들리지 않습니다.
- manifest 로딩 화면과 런처 컨테이너에는 진입·시머 애니메이션을 사용하지 않습니다. 데이터 준비 전에는 실제 쇼케이스와 같은 최소 높이의 정적인 상태를 유지합니다.
...
- 캐릭터 레이어는 중앙 플레이 프레임 안에서 다이얼로그 박스 상단 경계까지만 사용하며, 캐릭터 이미지는 레이어 하단 기준으로 정렬됩니다.
- 노출 캐릭터가 정확히 두 명이면 이미지와 Live2D 모두 자동으로 좌우 역할을 받습니다. 모바일은 화면 25%/75%, PC는 260px 최대 간격을 사용하며 한 명이 퇴장하면 같은 슬롯에 남은 동일 인물은 이전 좌우 역할을 유지합니다. 에셋별 미세 조정이 필요할 때만 `calibration.spacing`을 사용합니다.
