...
공격의 실제 결과는 `set`/`branch`/`gameOver`로 작성합니다. 세부 옵션과 저장·접근성 동작은 [개발 가이드](docs/DEVELOPMENT_GUIDE.ko.md#9-0-장면-전환과-공격-연출)를 참고하세요. 덕만 V10.4는 이 연출을 12개 장의 장소 전환, 칠숙의 공격 장면 네 곳, 11개 결말에 적용합니다. 장면 액션 위치 변경으로 이전 작성 버전의 저장은 이어하기 대상에서 제외하며 엔딩 수집 기록은 유지합니다.
