...
- `medium`은 모바일·PC의 기본 드라마 구도입니다. 정적 이미지는 인원수와 관계없이 원본 높이 `58cqh`, 구성 배율 `1.58`, 세로 원점 `80%`를 사용해 같은 머리 위치·하체 크롭·발 기준선을 유지합니다. `wide`는 최종 배율 `1.00`의 전신·공간 컷, `close`와 `reaction`은 각각 `2.02`, `1.83`의 대상 확대입니다. 1·2·3인 변화는 가로 앵커만 바꾸며 등록된 `framing.scale`도 인원수에 따라 약화하지 않습니다.
