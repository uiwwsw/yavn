...
- 스티커·증거물은 HUD·대사창·기기 safe-area를 제외한 공통 안전 프레임을 좌표 기준으로 사용합니다. 런타임이 아이템과 현재 보이는 캐릭터의 실제 렌더 경계를 숨김 상태에서 측정해, 겹치지 않으면 작성 좌표를 유지하고 겹칠 때만 가장 가까운 좌·우·상단 빈 공간을 최종 위치로 선택합니다. 좌표·크기를 확정한 뒤에만 입장 이펙트를 시작하고 노출 중에는 재배치하지 않습니다. 기기 회전이나 큰 PC 프레임 변경 때만 숨긴 상태에서 한 번 다시 계산하며, 모바일 주소창 높이 변화는 무시합니다. 빈 공간이 부족할 때만 종횡비를 유지한 채 단계적으로 축소합니다.
