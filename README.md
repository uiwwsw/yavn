...
- 스티커가 캐릭터와 새로 겹쳐 안전 위치를 다시 찾으면 280ms 동안 좌표·크기·배율을 보간합니다. 캐릭터는 등장·재등장·퇴장 때 위치를 이동하지 않고, `push`는 확대 전환만 보간하며 명시한 `pan`만 수평 이동합니다.
...
- 스티커·증거물은 HUD·대사창·기기 safe-area를 제외한 공통 안전 프레임을 좌표 기준으로 사용합니다. 런타임이 아이템과 현재 보이는 캐릭터의 실제 렌더 경계를 숨김 상태에서 연속 측정해 캐릭터 입장·슬롯 이동·카메라 전환이 실제로 멈춘 것을 확인한 뒤, 겹치지 않으면 작성 좌표를 유지하고 겹칠 때만 가장 가까운 좌·우·상단 빈 공간을 최종 위치로 선택합니다. 최초 좌표·크기는 확정 뒤 제자리에서 입장 이펙트를 시작하며, 노출 중 카메라나 등장 인물 변경으로 새 충돌이 생기면 다음 안전 위치까지 좌표·크기·배율을 280ms 동안 부드럽게 보간합니다. `clearSticker.leave`는 같은 렌더 인스턴스와 마지막 확정 좌표를 유지한 채 퇴장 클래스로만 전환하므로, 재측정 때문에 잠시 숨거나 다시 나타나지 않습니다. 기기 회전이나 큰 PC 프레임 변경 때만 숨긴 상태에서 한 번 다시 계산하며, 모바일 주소창 높이 변화는 무시합니다. 빈 공간이 부족할 때만 종횡비를 유지한 채 단계적으로 축소합니다.
...
- `with`는 같은 무대에 배치된 배우를 제거하지 않고 현재 카메라에서만 이미지·Live2D 공통 180ms 페이드로 숨깁니다. 퇴장이 끝날 때까지 이전 앙상블 레이아웃을 유지하므로 사라지는 인물은 제자리에서 불투명도만 낮아지고, 이후 숨김 상태에서 새 좌표로 즉시 정렬됩니다. 다시 보이는 인물과 새 인물은 다음 앙상블의 최종 좌표를 먼저 적용한 뒤 나타나므로 과거 슬롯에서 화면을 가로질러 오지 않습니다. 이미 계속 보이던 배우의 명시적 슬롯·구도 변경만 `translate/scale`로 이동합니다. 빠른 연속 노출 변경은 다음 페인트 프레임에 마지막 구성만 커밋하며, 로딩 중 최초 구성은 지연하지 않고 `prefers-reduced-motion`에서는 대기와 모션을 생략합니다.
...
- 정말 새로 등장하거나 다시 노출되는 인물은 최종 위치에서만 나타나며 위로 튀는 바운스나 과거 슬롯에서의 장거리 이동은 사용하지 않습니다. 이미 계속 보이던 동일 인물의 위치와 크기만 합성 전용 `translate/scale`로 이동하고, 좌우 반전은 중간에 납작해지는 모양을 막기 위해 즉시 바뀝니다. 카메라는 단일 절대 배율을 사용하며 `push`의 확대와 `pan`의 수평 이동을 분리해 `left/width/filter/transform-origin` 재계산을 피합니다. 캐릭터 표정은 사전 디코딩 후 비동기 표시하며, 모든 모션은 `prefers-reduced-motion`에서 비활성화됩니다.
...
- `/game-list/deokman/`은 **선덕여왕: 죽은 공주의 왕관** V10.1의 정식 12챕터 완결판입니다. 덕만은 `어린 공주 → 10년 뒤 국경 유랑자 → 신분을 되찾은 공주 → 선덕왕`으로 성장합니다. 플레이어는 사람답게 주고받는 대화 속 물리적·정치적 단서를 읽어 독살·모함·처형을 피하고, 매 장의 생존 정답을 이어 덕만을 여왕으로 만들어야 합니다.
