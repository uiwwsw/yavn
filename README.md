...
- `/game-list/deokman/`은 **선덕여왕: 죽은 공주의 왕관**의 정식 12챕터 완결판입니다. V9.0에서는 모든 대사·내레이션·선택 문구를 설명형 역사 문답에서 인물의 욕망, 행동, 위기와 여운이 이어지는 시대극 문장으로 전면 개작했습니다. V9.1은 덕만을 `어린 공주 → 10년 뒤 국경 유랑자 → 신분을 되찾은 공주 → 선덕왕`으로 시각적으로 성장시켰습니다. V9.4는 생존 위기와 일반 판단 게이트에 정답을 1개만 두고 나머지를 게임오버로 연결합니다. 예외는 1장의 북문처럼 다음 장의 죽음 원인이 되는 분기, 정책·관계 클루, 최종 엔딩 선택으로 명시했습니다. 지연 사망은 실제 원인 선택으로 복귀하고, 어린 시절부터 마지막 판결까지 특정 선택을 잇는 11번째 히든 엔딩은 유지합니다.
- 전 작품에 고유한 선택 36개, 장면형 12개와 즉시 판정형 16개를 합친 `gameOver` 28개, 누적 관계·정치 상태로 판정되는 결말 11개를 제공합니다. 초반의 관찰과 증거가 후반의 왕위 표결과 최종 기록 공개 여부까지 다시 작동합니다.
