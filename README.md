...
- 모바일은 PC와 별도의 인원별 확대값을 사용합니다. `medium`은 1/2/3인 `1.40/1.58/1.38`, `close`는 `1.67/2.02/1.92`, `reaction`은 `1.52/1.83/1.69`이며, 1인 원점은 `medium/reaction 55%`, `close 60%`, 다인 원점은 `80%`입니다. 짧은 세로 화면에서도 머리·얼굴을 남기고 인물군이 대사창 아래로 몰리지 않게 하며 PC 배율·원점은 별도로 유지합니다. 정적 캐릭터 폭은 고정 픽셀 상한 대신 플레이 프레임의 가로·세로(`cqw/cqh`)를 함께 사용해 휴대폰, 태블릿, 고해상도 PC에서 같은 신체 비율을 유지합니다.
- 단독 배우가 `left/right` 슬롯에 남아 있어도 `medium/close/reaction` 카메라는 해당 배우를 프레임 중앙으로 옮깁니다. `wide`는 장면의 물리적 좌우 위치를 보존합니다. 기본 좌우 앵커와 카메라 이동량은 모바일·PC 모두 25%/75% 중심을 같은 좌표계로 사용하므로 옆자리 클로즈업에서 얼굴이 프레임 밖으로 밀리지 않습니다.
