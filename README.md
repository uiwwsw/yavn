...
- `calibration.scale`은 `0.5..2`, `x/y`는 `-30..30`이며, 쇼트 확대와 별도로 원본의 발·눈높이 차이만 보정합니다. 기본값은 `scale: 1, x: 0, y: 0`입니다.
...
- 화자 강조는 데스크톱/모바일 공통 규칙으로 동작합니다. 현재 화자는 가장 앞의 z축과 밝기를 유지하고 비화자는 밝기를 낮추되, 개별 캐릭터 크기는 바꾸지 않습니다. `close`에서는 주변 인물을 페이드해 잘린 손·하체가 남지 않으며 `reaction`에서는 카메라 대상인 듣는 인물을 전면에 둡니다.
- `with`를 생략한 대사·선택·입력·내레이션은 현재 무대에 배치된 인물을 기본 앙상블로 유지합니다. 정확히 두 명이면 25%/75%의 안정된 2인 구도, 세 명이면 전용 3인 구도를 사용합니다. `medium`에서는 모든 인물이 하나의 `.char-camera-world` 안에서 함께 확대되고, 3인 가장자리는 PC 22%/78%·모바일 20%/80%로 들어와 얼굴이 잘리지 않습니다.
...
- 정말 새로 등장한 인물만 최종 위치에서 짧은 상승·페이드로 나타납니다. 이미 보이던 동일 인물의 위치는 부드럽게 이동하고, 좌우 반전은 중간에 납작해지는 모양을 막기 위해 즉시 바뀝니다. 카메라 전환과 인물 이동은 `prefers-reduced-motion`에서 비활성화됩니다.
