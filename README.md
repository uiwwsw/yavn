...
- 화자 강조는 데스크톱/모바일, 정적 이미지/Live2D에 같은 규칙으로 동작합니다. 화면에 남아 있는 화자와 비화자는 모두 원래 불투명도를 유지하고, 현재 대사 화자에게만 바닥 기준의 `2.2s` 미세 상하·스케일 호흡 모션을 반복합니다. `reaction`은 듣는 카메라 대상을 전면에 두되 주변 인물을 흐리지 않으며, 다인 `close`의 주변 인물 페이드는 화자 강조가 아니라 쇼트 밖 퇴장 연출로만 유지됩니다.
...
- 화자 시각 강조는 플랫폼별 분기 없이 실제 `dialog.speakerId`에만 `2.2s` 미세 호흡 모션을 적용합니다. `speakerOrder`는 합성 순서에만 사용하며, 같은 쇼트의 비화자는 불투명도를 낮추지 않습니다. 모션 감소 환경에서는 호흡 모션을 자동으로 끕니다.
