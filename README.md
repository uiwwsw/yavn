...
- 진행 데이터는 URL 게임 ID 또는 ZIP 파일명/크기로 분리한 `localStorage`에 저장합니다. `autoSave`는 첫 실행 기본값이며 플레이어가 저장 탭에서 언제든 켜고 끌 수 있습니다.
- 자동 저장, 수동 저장, 현재 챕터 시작점의 3개 보호 지점을 제공합니다. 챕터 시작점은 자동 저장을 꺼도 항상 갱신됩니다.
- HUD 저장 버튼에서 현재 진행 저장, 슬롯별 불러오기, JSON 백업 내보내기/가져오기를 사용할 수 있습니다.
...
게임오버 화면은 `불러오기`, `최근 저장으로`, `챕터 처음으로`를 제공합니다. 같은 옵션에 `goto`와 `gameOver`를 동시에 선언할 수 없습니다.
...
- 저장 탭의 자동저장 토글도 같은 게임별 설정 키에 저장됩니다. 수동 슬롯과 챕터 시작점은 자동저장 토글과 독립적으로 유지됩니다.
