...
- 대사·입력·선택 프롬프트는 기본 `170px`의 고정 바깥 높이를 유지하고, 게임/액션의 `promptHeight`로 덮어쓸 수 있습니다. 초과 내용만 내부에서 스크롤하며, 하단의 `다음`·`완료`·`입력 대기`·`선택 대기` 상태 전용 줄은 따로 예약하지 않아 프롬프트 안쪽 높이를 본문과 선택지가 모두 사용합니다.
