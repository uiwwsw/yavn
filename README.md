...
- 여러 인물이 보이는 `close`의 비즉시 전환은 전체 `duration` 안에서 비대상 인물을 먼저 최대 180ms 동안 페이드한 뒤 숨기고, 남은 시간에 카메라 확대를 실행합니다. 따라서 퇴장 중인 인물이 확대와 함께 화면 가장자리에서 반만 남는 현상을 막으며, 1인 `close`와 `cut`은 지연하지 않습니다.
...
- `narration`은 장면의 밝기와 무관하게 본문 대비가 유지되도록 템플릿별 고대비 전용 배경을 사용합니다. `paper-stage` 서술창은 비치는 비단색 대신 불투명한 종이색 바탕으로 렌더링합니다.
...
- `with`는 같은 무대에 배치된 배우를 퇴장시키지 않고 현재 카메라에서만 이미지·Live2D 공통 180ms 페이드로 숨깁니다. 페이드가 끝난 뒤 `visibility: hidden`을 적용하고, 퇴장 중에는 마지막으로 보였던 PC·모바일 앵커와 시선 방향을 유지하므로 새 1인/2인 구도 위치로 미끄러지거나 뒤집힌 뒤 사라지지 않습니다. 빠른 진행에서 80ms 안에 연속된 노출 변경은 마지막 구성만 반영해 중간 배우가 번쩍이지 않으며, 일반 `wide/medium`의 2→1·3→2 퇴장은 페이드가 끝난 뒤 남은 배우의 레이아웃 이동을 시작합니다. 잠시 화면 밖에 있던 인물이 다음 대사에서 다시 잡혀도 DOM/Live2D 인스턴스와 물리적 슬롯을 유지해 `characterEnter`를 반복하지 않습니다. 로딩 중 최초 구성은 지연하지 않고, `prefers-reduced-motion`에서는 레이아웃 대기와 모션을 생략합니다.
