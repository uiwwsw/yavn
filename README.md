...
- **인게임 플레이어 UI**: 테마별 대화·선택·입력, 기록·소지품·저장/설정과 결과 화면 통일. 메뉴 키보드 탐색·초점 복귀·이야기 시간 일시 정지 지원.
- **비주얼노벨 인물 연출**: 디코드 후 등장, 표정 크로스디졸브, 제자리 퇴장·교체, 안정적인 대화 구도와 카메라 유지.
...
- 화자 강조는 정적 이미지·Live2D 공통의 180ms 밝기 전환만 사용합니다. 화자가 바뀌어도 몸의 크기·위치·시선·슬롯 안의 합성 순서는 유지하며, 반복 상하 이동이나 비균등 확대는 하지 않습니다. Live2D 자체의 표정·시선·모션은 계속 재생합니다. Live2D는 기본 `52cqh` 정사각 캔버스(`prompt-top`은 남은 높이에 맞춰 축소)에 모델을 맞춘 뒤 공통 카메라를 적용하여 세로 투영과 확대가 겹쳐 머리가 잘리지 않도록 합니다.
...
- 화자 시각 강조는 실제 `dialog.speakerId`를 따라 밝기만 부드럽게 전환합니다. 인물의 몸·시선·합성 순서는 대사마다 바뀌지 않으며, 인물 크기는 작성한 카메라와 프레이밍에서만 변경합니다.
