...
- 직전 `close/reaction` 뒤에 화자와 명시 대상이 없는 내레이션이 이어지면, 엔진은 직전 인물로 다시 당기지 않고 현재 보이는 인물을 `medium` 그룹 구도로 보여 줍니다. 내레이션에서도 특정 인물을 계속 잡아야 한다면 같은 `say.camera.target`에 캐릭터 ID를 명시합니다.
...
- 모바일은 PC와 별도의 인원별 확대값을 사용합니다. `medium`은 1/2/3인 `1.40/1.58/1.38`, `close`는 `1.67/2.02/1.92`, `reaction`은 `1.52/1.83/1.69`이며, 1인 원점은 `medium/reaction 55%`, `close 60%`, 다인 원점은 `80%`입니다. 짧은 세로 화면에서도 머리·얼굴을 남기고 인물군이 대사창 아래로 몰리지 않게 하며 PC 배율·원점은 별도로 유지합니다.
