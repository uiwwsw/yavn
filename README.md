...
- 인물 10명의 같은 투명 원본에 캐릭터별 `full/bust/closeup` 보정을 등록하고, 현재 565개 화자 대사와 24개 선택 장면 전체에 구도를 지시했습니다. 173개의 명시적 등장 지시와 분기별 화면 배치 검증으로 말하는 인물이 화면에서 사라지는 경로를 막습니다. 실제로 보이는 인물과 공간은 전신, 정보 교환은 상체, 위기·감정·결단은 클로즈업으로 전환합니다.
