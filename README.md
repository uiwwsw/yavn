...
- `src/engine.ts`: 챕터 전환, 저장 슬롯/백업, 게임오버 복구, 장면 단위 무대 카메라, 캐릭터 원본 정렬, 좌우 대화 시선 자동 보정, 연출 실행, 입력 게이트, 오디오 BGM 크로스페이드
...
- 기존 `vn-engine-autosave` 및 게임별 autosave 데이터는 그대로 읽습니다. 새 메타가 없는 저장도 하위 호환으로 복원합니다.
...
- 배경과 전체 화면 시스템 오버레이는 항상 뷰포트를 채웁니다. 캐릭터·스티커·HUD·대사창·선택/입력·컷신·CASE FILE은 최대 `9:16`의 `stage-content-frame`을 공유하며, 화면이 이보다 길 때 프레임만 원본 비율을 유지한 채 세로 중앙에 놓입니다. `9:16`보다 덜 긴 화면은 기존처럼 전체 높이를 사용합니다.
...
- `/game-list/:gameId` 직접 진입 시 설정 프리뷰와 본편을 준비하는 동안에는 비대화형 부트 화면을 유지합니다. 런처나 빈 게임 HUD가 먼저 노출되지 않습니다. 시작·이어하기 버튼은 첫 페인트 전에 개별 페이드·상승 연출을 시작하지만 CSS 기본값은 항상 보이는 상태이며, 애니메이션 미지원·오류·정지 시 강제 취소되어 버튼 가시성을 복구합니다.
...
- `gameOver`는 엔딩/크레딧/엔딩 수집률을 발생시키지 않으며 자동, 수동, 챕터 저장 또는 가져온 백업에서 복구합니다.
