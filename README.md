...
게임오버 화면은 별도 선택 복구점을 우선 쓰는 `직전 선택으로`, `수동 저장으로`, `챕터 처음으로`, 백업 파일 불러오기를 제공합니다. `직전 선택으로` 또는 `챕터 처음으로` 돌아간 뒤 실패를 만든 선택 게이트에 도달하면, 엔진은 당시 선택을 저채도 회색 카드와 `직전 선택 · GAME OVER` 배지로 표시합니다. 선택지는 비활성화하지 않아 다시 고를 수 있으며, 가능한 경우 키보드 포커스는 아직 고르지 않은 다른 항목에서 시작합니다. 같은 옵션에 `goto`와 `gameOver`를 동시에 선언할 수 없습니다.
...
- `/game-list/deokman/`은 **선덕여왕: 죽은 공주의 왕관**의 정식 12챕터 완결판입니다. V9.0에서는 모든 대사·내레이션·선택 문구를 설명형 역사 문답에서 인물의 욕망, 행동, 위기와 여운이 이어지는 시대극 문장으로 전면 개작했습니다. V9.1은 덕만을 `어린 공주 → 10년 뒤 국경 유랑자 → 신분을 되찾은 공주 → 선덕왕`으로 시각적으로 성장시켰습니다. V9.2는 1~11장의 핵심 물리적 위기를 3지선다 기준 2개, 4지선다 기준 3개가 실제 사망으로 이어지도록 재설계하고, 서로 다른 위험 행동이 같은 약점 때문에 하나의 실패 장면으로 합류하게 했습니다. 조사·정치 선택과 최종 엔딩 선택은 여러 장기 결과를 그대로 유지합니다.
...
- 시스템 탭의 자동저장 토글도 같은 게임별 설정 키에 저장됩니다. 자동저장은 게임오버의 직전 선택 복구와 시작 화면 이어하기에 사용하고, 일반 화면에서는 직접 불러오기 버튼을 표시하지 않습니다. PC와 모바일 모두 저장 카드 영역만 스크롤하며 배경음악·`게임 시작 화면으로 가기`·상태 문구는 하단 고정 행에 유지합니다.
