...
- `effect`는 `none | fadeIn | wipeLeft | scaleIn | popIn | slideUp | slideDown | slideLeft | slideRight | wipeCenterX | wipeCenterY | blurIn | rotateIn`입니다. 생략 시 기존과 같은 `fadeIn(240ms)`입니다.
...
- 화자 강조는 데스크톱/모바일, 정적 이미지/Live2D에 같은 규칙으로 동작합니다. 화면에 남아 있는 화자와 비화자는 모두 원래 불투명도를 유지하고, 현재 대사 화자에게만 하단 중앙 기준의 `2.2s` 미세 상하·스케일 호흡과 최대 `3%` 밝기·옅은 웜 글로우를 반복합니다. 호흡은 좌우 반전 `transform`과 분리되며, 화자가 바뀌면 직전 화자는 진행 중인 호흡 1회를 중립 프레임까지 마친 뒤 정지합니다. `reaction`은 듣는 카메라 대상을 전면에 두되 주변 인물을 흐리지 않고, 다인 `close`의 주변 인물도 DOM·불투명도·Live2D 렌더러를 그대로 유지합니다.
...
- `medium`은 모바일·PC의 기본 드라마 구도입니다. 정적 이미지는 인원수와 관계없이 원본 높이 `52cqh`를 사용하고, PC는 `wide/medium/close/reaction = 1.18/1.62/2.08/1.90`, 모바일은 `1.12/1.58/1.84/1.72`의 단일 최종 배율을 적용합니다. 따라서 PC 렌더 높이는 약 `61%/84%/108%/99%`로 유지되어 `close`에서도 머리 위 여백이 남고, 모바일은 더 완만한 확대를 사용합니다. 세로 원점은 PC `86%`, 모바일 `82%`이며 1·2·3인 변화는 가로 앵커만 바꿉니다.
...
- 같은 위치의 같은 이미지 캐릭터가 감정만 바꾸면 DOM을 다시 만들지 않습니다. 새 소스가 전송·디코딩되는 동안 직전 표정을 그대로 그린 뒤 준비된 소스만 다음 페인트에 교체하므로 중간 빈 프레임이나 오래된 비동기 완료가 노출되지 않습니다. 처음 마운트된 이미지도 실제 DOM의 디코드가 끝날 때까지 숨긴 뒤 기존 짧은 opacity 등장으로 연결하며, 최초 등장 애니메이션을 반복하지 않습니다.
- `with`는 같은 무대에 배치된 배우를 제거하지 않고 현재 카메라에서만 이미지·Live2D 공통 180ms 페이드로 숨깁니다. 인원이 3→2·2→1로 줄거나 같은 인원에서 배우가 교체될 때 퇴장 배우의 PC·모바일 앵커와 시선은 직전 좌표로 스냅샷해 페이드 동안 고정하고, 생존 배우의 새 앙상블 레이아웃은 같은 페인트에 커밋해 재배치를 동시에 시작합니다. 따라서 `퇴장 대기 → 뒤늦은 이동`의 두 단계 멈춤 없이 하나의 연속 전환이 됩니다. 다시 보이는 인물과 새 인물은 `char.enter`를 실행합니다. `layout: cut`은 다음 앙상블 좌표를 즉시 확정해 과거 슬롯에서 날아오지 않고, `layout: push`는 계속 보이는 배우만 새 자리까지 보간하며 새 배우는 지정한 입장 효과로 합류합니다. 빠른 연속 노출 변경은 다음 페인트 프레임에 마지막 구성만 커밋하며, 로딩 중 최초 구성은 지연하지 않고 `prefers-reduced-motion`에서는 대기와 모션을 생략합니다.
...
- 화자 시각 강조는 플랫폼별 분기 없이 실제 `dialog.speakerId`에만 `2.2s` 미세 호흡 모션을 적용합니다. 모션은 좌우 반전용 `transform`과 분리된 위치·스케일을 하단 중앙 기준으로 움직이고, 정점에서 최대 `4%` 밝기와 옅은 웜 글로우를 더합니다. 화자가 바뀌면 직전 화자는 진행 중인 호흡을 중립 프레임까지 마친 뒤 정지해 중간 변형값에서 튀지 않습니다. `speakerOrder`는 합성 순서에만 사용하며, 같은 쇼트의 비화자는 불투명도를 낮추지 않습니다. 화면 효과를 `부드럽게`로 두면 호흡 주기를 늦추고 `최소화`나 OS 모션 감소 환경에서는 호흡 모션을 끕니다.
