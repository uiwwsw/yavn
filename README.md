...
- 여러 인물이 보이는 `close`의 비즉시 전환은 비대상 인물을 최대 180ms 동안 제자리에서 먼저 페이드한 뒤 카메라 확대 또는 팬을 시작합니다. 기본 `push`는 대상 수평 좌표를 즉시 맞추고 확대만 보간하므로 좌우 화자 교대 때 인물이 화면 밖에서 날아오지 않습니다. 실제 횡이동이 필요한 연출은 `transition: pan`을 명시합니다. 1인 `close`와 `cut`은 지연하지 않습니다.
...
- `close/reaction`은 대상 앵커를 화면 중앙에 맞춘 뒤 단일 확대를 적용합니다. 기본 `close`의 `push`는 수평 좌표를 즉시 맞추고 확대만 전환해 좌우 화자 교대가 장거리 캐릭터 이동처럼 보이지 않으며, `reaction` 기본값과 명시적 `transition: pan`만 수평 이동을 보간합니다. `transform-origin` 변경이나 구성 배율 중첩은 사용하지 않고, 단독 배우는 모든 쇼트에서 중앙 앵커를 사용합니다.
...
- 새로 등장하거나 다시 노출되는 인물은 과거 슬롯에서 날아오지 않고 다음 앙상블의 최종 자리를 기준으로 나타납니다. 기본 `fadeIn`과 `popIn`은 그 자리에서 재생되고, 명시한 `slide*`만 최종 자리 가까운 오프셋에서 들어옵니다. 이미 계속 보이던 동일 인물은 `layout: push`일 때만 합성 전용 `translate/scale`로 자리를 내주며, `cut`은 즉시 재배치합니다. 좌우 반전은 중간에 납작해지는 모양을 막기 위해 즉시 바뀝니다. 카메라는 단일 절대 배율을 사용하며 `push`의 확대와 `pan`의 수평 이동을 분리해 `left/width/filter/transform-origin` 재계산을 피합니다. 캐릭터 표정은 사전 디코딩 후 비동기 표시하며, 모든 모션은 `prefers-reduced-motion`에서 비활성화됩니다.
...
- `/game-list/deokman/`은 **선덕여왕: 죽은 공주의 왕관** V10.2의 정식 12챕터 완결판입니다. 덕만은 `어린 공주 → 10년 뒤 국경 유랑자 → 신분을 되찾은 공주 → 선덕왕`으로 성장합니다. V10.2는 인물 발화 320개와 선택 프롬프트 36개를 전수 교정해 설명 낭독을 짧은 질문·반응·명령으로 바꾸고, 인물별 말투 회귀 검사를 추가했습니다. 플레이어는 대화 속 물리적·정치적 단서를 읽어 독살·모함·처형을 피하고 덕만을 여왕으로 만들어야 합니다.
- 전 작품에 고유한 선택 36개와 보기 112개가 있으며, 각 선택에는 생존 정답이 정확히 하나 있습니다. 나머지 76개(67.9%) 오답은 장면형 `gameOver`로 이어집니다. 오답은 즉시 결과창으로 끊지 않고 선택 반응, 잠깐의 안도, 상대의 반격, 죽음을 확정하는 기록을 최소 네 번의 대사·내레이션으로 보여 줍니다. 독이 든 차·먹·인장·약·승전주, 정치적 모함, 매복, 화재, 쿠데타처럼 선택과 인물 관계에 맞는 실패를 사용하며 원인 선택으로 돌아갈 수 있습니다.
