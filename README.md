...
- v2.2.0은 35개 실패 선택을 즉시 결과창으로 끝내지 않고, 선택이 가능해 보인 이유·경고·상황 반전·마지막 반응을 5개 이상의 대사와 이펙트로 직접 재생한 뒤 `gameOver`가 나오도록 확장했습니다. 모든 실패 장면은 전신 앙상블로 공간과 인물을 먼저 보여 주고, 정보 교환은 상반신, 발각·배신·마지막 말만 단독 클로즈업으로 좁힙니다.
...
- 인물 10명의 같은 투명 원본에 캐릭터별 `full/bust/closeup` 보정을 등록하고, 현재 522개 화자 대사와 24개 선택 장면 전체에 구도를 지시했습니다. 169개의 명시적 등장 지시와 분기별 화면 배치 검증으로 말하는 인물이 화면에서 사라지는 경로를 막습니다. 첫 등장·공간 제시는 전신, 정보 교환은 상체, 위기·감정·결단은 클로즈업으로 전환합니다.
