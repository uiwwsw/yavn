...
- 다이얼로그 수동 숨김/복원이나 높이 변화는 다음 스티커의 최초 안전 배치에 반영되며, 이미 노출된 스티커와 캐릭터 위치는 다시 계산하지 않습니다.
...
- 다이얼로그를 수동으로 숨기거나 다시 열 때는 다음 스티커의 최초 배치에 쓰는 하단 안전 inset만 갱신합니다. 이미 노출된 스티커와 캐릭터 좌표는 유지합니다.
