...
- 복구 화면은 선택 복구점을 우선 쓰는 `직전 선택으로`, `수동 저장으로`, 자동으로 남긴 `챕터 처음으로`, 백업 파일 불러오기를 제공합니다. 선택 복구점은 선택 키·문구·인덱스도 별도 기록하고 실제 `gameOver`가 발생했을 때만 실패 이력으로 확정합니다. `직전 선택으로` 또는 `챕터 처음으로` 복귀해 같은 선택 게이트에 도달하면 해당 항목을 저채도 회색 카드와 `직전 선택 · GAME OVER` 배지로 표시하되 비활성화하지 않습니다. 현재 키보드 포커스는 밝은 테두리로 구분하고 가능한 경우 다른 선택지에서 시작하므로, 과거 이력과 현재 활성 상태를 혼동하지 않습니다. 직접 `gameOver` 옵션과 `goto` 뒤 독립 게임오버 모두 같은 규칙을 사용합니다.
