...
- 루트 런처는 manifest의 모든 게임을 전체 폭 데모 캐러셀로 렌더링합니다. 최초 선택 우선순위는 유효한 `?demo=<gameId>` 공유 쿼리, 현재 탭의 `sessionStorage` 선택, manifest 첫 게임 순서입니다. 예전 `#demo=<gameId>` 링크는 유효성을 확인한 뒤 쿼리 형식으로 자동 이전합니다.
...
- 2026-08-12: 엔진 홈의 평상시 캐러셀 선택을 URL 해시 대신 탭 세션에 저장하고, 명시적인 `선택 링크 복사`에서만 `?demo=<gameId>` 공유 주소를 생성하도록 변경했습니다. 기존 `#demo=` 링크는 자동 이전하며 다른 데모 탐색 시 오래된 선택 쿼리를 제거합니다. 데모별 `GitHub 폴더`를 `public/game-list/<gameId>`로 직접 연결하고, 엔진 제작 개요·가이드·샘플 YAML 진입점, 태그 축약/검색 복구, 접근 가능한 ZIP 입력, 모바일 가이드 노출과 런처 기본 브라우저 상호작용을 보강했습니다. DSL 문법 변경은 없습니다.
