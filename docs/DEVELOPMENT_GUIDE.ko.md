...
- 실제 플레이 UI는 `.stage-content-frame` 안에서 렌더링합니다. PC에서는 `width: min(100cqw, 100cqh × 16 / 9)`와 `height: min(100cqh, 100cqw × 3 / 4)`를 함께 사용해 화면 안에서 가장 큰 `4:3 ~ 16:9` 프레임을 만들고 가로·세로 중앙에 놓습니다. 세로 모니터는 4:3, 중간 비율은 전체 화면, 울트라와이드는 16:9에서 멈춥니다. 768px 이하 세로 화면은 포인터 종류와 무관하게 최대 9:16 프레임을 유지해 모바일 브라우저와 반응형 검사 환경이 같은 레이아웃을 사용합니다.
...
- 이미지 캐릭터(`.char-image`)는 `object-position: center bottom`으로 하단 정렬되고, 원본 보정 `scale`도 무대 바닥을 변형 원점으로 사용합니다. 정적 원본 높이는 인원수와 관계없이 `52cqh`이며 기기별 단일 카메라 배율과 원점을 적용합니다. 따라서 1인 중앙 컷에서 2·3인 컷으로 전환되어도 원본 박스의 발 기준선이 달라지지 않고, 기본 `medium`에서 배우의 렌더 높이는 PC 약 84%, 모바일 약 82%를 확보합니다. `close`도 PC 약 97%, 모바일 약 90%로 제한해 머리 위 여백을 남기고 하단 크롭은 대화창 뒤에서만 일어나게 합니다.
- 화자 강조는 정적 이미지·Live2D 공통의 180ms 밝기 전환만 사용합니다. 화자가 바뀌어도 몸의 크기·위치·시선·슬롯 안의 합성 순서는 유지하며, 반복 상하 이동이나 비균등 확대는 하지 않습니다. Live2D 자체의 표정·시선·모션은 계속 재생합니다. Live2D는 `52cqh` 정사각 캔버스에 모델을 맞춘 뒤 공통 카메라를 적용하여 세로 투영과 확대가 겹쳐 머리가 잘리지 않도록 합니다.
...
- 모바일 세로 화면에서 케이스 파일을 열면 `.stage-content-frame`의 `9:16` 높이 제한을 일시 해제해 전체 게임 뷰포트로 확장하고 헤더/탭을 고정합니다. 시스템 카드에는 `grid-auto-rows: max-content`를 적용해 각 카드가 컨트롤까지 포함한 내용 높이를 유지하고 카드 목록만 스크롤합니다. `게임 시작 화면으로 가기`와 상태 문구는 하단 고정 행에 유지합니다. 인벤토리 그리드는 수집 헤더와 필터 다음의 `minmax(0, 1fr)` 행을 사용해 남은 공간을 모두 채우며, 상세 이미지는 설명 위에 단일 열로 배치합니다. 짧은 가로 화면은 왼쪽 탐색과 스크롤 가능한 본문을 사용합니다.
