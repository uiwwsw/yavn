...
- `duration`: `0..3000ms`, 기본 `240ms`. `delay`: `0..5000ms`, 기본 `0`. `easing`: 최대 64자의 CSS easing 문자열이며 기본 `ease-out`입니다. `layout: push`에서는 이 세 값을 새 인물 효과와 계속 보이는 배우의 자리 이동에 함께 적용합니다.
- `enter`를 생략하면 기존처럼 `fadeIn + push + 240ms`를 사용합니다. 동일 인물의 표정·프레이밍만 바뀌면 등장 효과를 다시 실행하지 않으며 `say.char` 감정 교체 뒤에도 마지막 등장 설정을 보존합니다.
...
- 이미지 캐릭터(`.char-image`)는 `object-position: center bottom`으로 하단 정렬되고, 원본 보정 `scale`도 무대 바닥을 변형 원점으로 사용합니다. 정적 원본 높이는 인원수와 관계없이 `52cqh`이며 기기별 단일 카메라 배율과 원점을 적용합니다. 따라서 1인 중앙 컷에서 2·3인 컷으로 전환되어도 원본 박스의 발 기준선이 달라지지 않고, 기본 `medium`에서 배우의 렌더 높이는 PC 약 84%, 모바일 약 82%를 확보합니다. `close`도 PC 약 108%, 모바일 약 96%로 제한해 머리 위 여백을 남기고 하단 크롭은 대화창 뒤에서만 일어나게 합니다.
- 화자 강조는 데스크톱/모바일과 정적 이미지/Live2D에 공통으로 적용합니다. 같은 쇼트의 화자와 비화자는 모두 원래 불투명도를 유지하고, 실제 `dialog.speakerId`와 일치하는 현재 대사 화자만 `2.2s` 주기의 미세 상하 이동과 `1.003 × 1.009` 이하 비균등 확대를 하단 중앙(`50% 100%`) 기준으로 반복합니다. 호흡은 좌우 반전용 `transform`을 덮지 않고 독립 `translate/scale`만 움직이며, 정점에는 최대 `4%` 밝기와 옅은 웜 글로우를 더합니다. 화자 변경 시 직전 화자는 진행 중인 호흡 1회를 중립 프레임까지 마친 뒤 정지하므로 중간 변형값에서 이미지가 꺾이거나 튀지 않습니다. 카메라 포커스 역할(`is-camera-focus/is-camera-listener`)과 실제 발화 상태(`is-speaking`)를 분리하므로 `reaction`에서 듣는 인물을 전면에 두어도 호흡은 대사 화자에게 남고 주변 인물은 흐려지지 않습니다. 다인 `close`에서도 주변 인물의 DOM·불투명도·Live2D 렌더러를 유지하고, 실제 `with` 제외만 퇴장으로 처리합니다. 플레이어 화면 효과가 `reduced`면 호흡 주기를 3초로 늦추고 `minimal` 또는 OS 모션 감소 환경이면 호흡을 끕니다.
- 노출 캐릭터가 한 명이면 원래 `left/center/right` 슬롯과 관계없이 50% 중앙 앵커를 사용합니다. 정확히 두 명이면 `.char-layout-duo`가 적용되어 25%/75%, 세 명이면 `.char-layout-trio`가 적용되어 25%/50%/75%의 구성 앵커를 사용합니다. 좌우 역할은 `left -> center -> right` 순서로 결정되어 화자 변경으로 뒤집히지 않습니다.
- PC는 모바일과 같은 구성 비율을 출발점으로 사용하되, 좌우 간격을 `min(25cqw, 260px) × 보이는 배우들의 평균 calibration.spacing`으로 제한합니다. 같은 평균값을 좌우에 적용하므로 에셋별 보정이 달라도 앙상블 중심이 기울지 않고, 넓은 화면에서는 배우들이 대화권 밖으로 지나치게 벌어지지 않습니다. 2인 중 한 명만 남으면 옆자리를 기억하지 않고 즉시 1인 중앙 구성으로 돌아옵니다.
...
- 1·2·3인은 기기별로 하나의 최종 배율을 공유합니다. PC `wide/medium/close/reaction`은 `1.18/1.62/2.08/1.90`, 모바일은 `1.12/1.58/1.84/1.72`입니다. 세로 원점은 PC `86%`, 모바일 `82%`로 분리해 대화창 위에서 얼굴과 상반신 비중을 확보합니다. `framing.scale`도 보이는 인원수에 따라 감쇠하지 않아 같은 프리셋이 같은 화면 비율을 뜻하고, 타이핑 레이아웃 변화로 카메라를 다시 계산하지 않습니다.
- `close/reaction` 상태가 남아 있어도 현재 `say/choice/input`에 화자나 명시 `camera.target`이 없으면 직전 표적을 재사용하지 않습니다. 이 경우 현재 인원수의 실제 `medium` 그룹 구도로 완화해 확대값, 포커스, 비대상 인물 노출 규칙을 함께 맞추고, 저장용 요청 카메라 상태와 전환 정보는 유지합니다. 화자 없는 내레이션에서 특정 인물을 계속 잡으려면 해당 액션의 객체형 `camera.target`에 캐릭터 ID를 직접 지정해야 합니다.
...
- 같은 `position`의 이미지 캐릭터가 감정 소스만 바꾸면 렌더 키를 유지합니다. `StageImageCharacter`는 요청 소스가 디코딩될 때까지 직전 소스를 같은 DOM 위치에 유지하고, 최신 요청만 커밋합니다. 따라서 표정 교체 때 최초 등장 애니메이션이 재실행되거나 이미지가 잠시 사라지지 않습니다.
- `say/choice/input.with`가 현재 카메라의 인물 구성을 좁혀도 배치된 캐릭터 DOM과 Live2D 인스턴스는 제거하지 않습니다. 화면 밖 인물에는 이미지·Live2D 공통 `.is-camera-hidden` 퇴장 규칙을 적용해 180ms 동안 `opacity`를 낮추고 완료 시점에 `visibility: hidden`으로 합성에서 제외합니다. 노출 집합이 바뀌는 페인트 직전에 퇴장 인물의 PC·모바일 앵커, 오프셋, 2인 역할, 시선 배율을 직전 레이아웃에서 스냅샷하므로 사라지는 인물은 새 1인/2인 자리로 미끄러지거나 뒤집히지 않습니다. 동시에 생존 인물의 다음 앙상블 레이아웃과 노출 집합을 한 배치로 커밋합니다. 따라서 3→2·2→1과 같은 축소는 퇴장 페이드가 끝날 때까지 멈춘 뒤 재배치하는 두 단계가 아니라, 퇴장 페이드와 생존자 `translate/scale` 이동이 겹치는 한 번의 연속 전환입니다. 새로 등장하거나 다시 노출되는 인물도 다음 노출 집합과 레이아웃을 같은 페인트에 커밋합니다. `layout: cut`은 기존 배우와 새 배우의 위치·크기를 즉시 확정하고 지정한 입장 효과만 재생합니다. `layout: push`는 새 배우의 입장 효과와 함께 계속 보이는 배우의 `translate/scale`을 지정 시간으로 보간합니다. 빠른 진행의 여러 노출 요청은 다음 페인트 프레임에 마지막 집합만 커밋하고, 챕터 로딩 중 최초 노출은 지연 없이 반영하며, `prefers-reduced-motion`에서는 대기와 모션을 생략합니다.
- DOM 보존 대상과 구도 계산 대상은 분리합니다. `resolveCharacterStageLayout`과 인원별 프레이밍은 현재 `with`로 보이는 캐릭터만 사용하고, 숨겨진 캐릭터는 렌더 인스턴스만 유지합니다. 모바일에서 보이는 두 명은 숨은 세 번째 배우의 존재와 무관하게 25%/75% 2인 슬롯을 사용합니다.
...
- 계속 보이는 동일 인물의 슬롯 이동과 원본 보정/레거시 프레이밍 오프셋은 레이아웃을 일으키는 `left/width` 대신 합성 가능한 `translate`로 380ms 전환하고 크기는 독립 `scale`로 360ms 전환합니다. 퇴장과 `layout: cut` 등장에서는 두 전환을 끄고, `layout: push` 등장에서만 계속 보이는 배우의 자리와 크기를 `char.enter.duration/easing/delay`에 맞춰 보간합니다. 새 배우의 슬라이드는 레이아웃 전환이 아니라 해당 입장 키프레임이 담당합니다. 화자 호흡은 시선 반전용 `scaleX`와 분리된 `translate/scale/filter`를 사용하고, 기존 화자가 자리를 내주는 동안만 잠시 멈췄다가 전환 뒤 다시 시작하므로 앙상블 이동을 덮어쓰지 않습니다. 카메라 DSL의 `push` 기본 520ms, `pan` 기본 380ms, `cut` 즉시 전환은 별도 규칙이며, 축약형 `close`의 `push`는 그룹 수평 구도를 고정하고 명시적 대상 `close`의 기본 `pan`만 카메라 수평 이동을 보간합니다.
...
- 캐릭터가 새로 노출되거나 숨김 상태에서 다시 잡힐 때 해당 슬롯의 `char.enter` 효과를 실행합니다. 기본값은 최종 배치 위치의 240ms opacity 페이드이고 `none`은 애니메이션 없이 즉시 노출합니다. 팝·스케일·슬라이드·와이프·블러·회전은 불투명도와 독립 CSS 속성으로 구성해 원본 시선 반전 `transform`을 덮지 않습니다. 계속 보이는 같은 ID의 위치·표정·구도 변경은 같은 렌더 키를 사용하므로 등장 애니메이션을 다시 실행하지 않습니다.
...
- 화자만 확대하던 모바일/다인 depth 배율과 비화자 불투명도 감쇠는 제거되었습니다. 화자 구분은 미세 호흡 모션과 기존 z축 합성 순서가 담당하고, 쇼트 전체 크기 변화는 무대 카메라만 담당합니다.
...
- 다이얼로그 박스는 기본 `170px`의 고정 바깥 높이를 유지하고, `ui.promptHeight` 또는 액션별 `promptHeight`를 적용합니다. 화자·채널·대사·입력·선택 내용이 넘칠 때만 `.dialog-content-scroll` 내부를 스크롤합니다.
...
- 모바일 다이얼로그는 화면 좌우·하단에 최소 10px 외곽 여백을 두며, 노치·홈 인디케이터의 safe-area inset이 더 크면 해당 값을 우선합니다. 입력 컨트롤은 최소 48px, 선택 카드는 최소 58px 높이를 사용합니다.
...
- 시스템 탭은 자동 저장의 작동 상태와 최근 복구 시각을 상단 보호 카드에 표시하고, 수동 저장·챕터 시작점·JSON 백업·플레이 감각·법적 고지를 독립 역할 카드로 분리합니다. 플레이 감각은 텍스트 출력 배속(`0.75/1/1.5/2×`), 플레이어 AUTO와 완료 후 간격(`800/1400/2200ms`), 화면 효과 강도(`full/reduced/minimal`), BGM 활성/음량, SFX 음량을 제공합니다. 데스크톱은 저장 카드 3열과 플레이 감각 3열 컨트롤, 모바일은 모두 1열이며 현재 진행 저장만 주요 작업 색상을 사용합니다.
...
- 모바일 세로 화면에서 케이스 파일을 열면 `.stage-content-frame`의 `9:16` 높이 제한을 일시 해제해 전체 게임 뷰포트로 확장하고 헤더/탭을 고정합니다. 시스템 카드에는 `grid-auto-rows: max-content`를 적용해 각 카드가 컨트롤까지 포함한 내용 높이를 유지하고 카드 목록만 스크롤합니다. `게임 시작 화면으로 가기`와 상태 문구는 하단 고정 행에 유지합니다. 인벤토리 그리드는 수집 헤더와 필터 다음의 `minmax(0, 1fr)` 행을 사용해 남은 공간을 모두 채우며, 상세 이미지는 설명 위에 단일 열로 배치합니다. 짧은 가로 화면도 전체 높이 바텀시트를 사용합니다.
...
- 다만 현재 액션에 화자와 명시 `camera.target`이 모두 없는 내레이션은 직전 `close/reaction`의 인물 표적을 상속하지 않고 실제 `medium` 그룹 구도로 렌더링합니다. 확대값뿐 아니라 포커스와 비대상 배우 노출도 `medium` 규칙을 사용합니다. 의도적인 내레이션 클로즈업은 해당 `say.camera.target`에 캐릭터 ID를 함께 적습니다.
