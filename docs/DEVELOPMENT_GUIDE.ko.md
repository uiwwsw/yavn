...
- 프리로드 이미지의 `load` 이벤트와 실제 `decode()` 완료를 구분해 기다리고, 성공한 소스를 세션 디코드 레지스트리에 기록합니다. 첫 장면을 DOM에 반영한 뒤에도 현재 배경·정적 캐릭터 이미지와 Live2D의 준비 상태를 확인한 후 로딩 오버레이를 닫습니다. 정적 캐릭터는 처음 마운트된 실제 DOM 이미지의 디코드가 끝나기 전에는 `.is-image-pending`으로 페인트하지 않습니다. 같은 캐릭터의 표정·소스가 바뀌면 직전 디코딩 이미지를 계속 렌더링하면서 요청 소스를 별도 `Image`에서 준비하고, 최신 요청과 일치하는 성공만 다음 페인트에 커밋합니다. 따라서 느린 요청의 완료 순서가 뒤집혀도 중간 표정이나 빈 프레임이 나타나지 않습니다. 플레이 중 `bg` 교체도 새 URL을 숨김 pending 레이어에서 로드·디코드한 뒤 같은 DOM을 current 레이어로 승격하고, 직전 배경을 아래에 유지한 채 기본 620ms opacity 디졸브로 전환합니다. 객체형 `bg`는 전환 방식·길이·완료 대기를 지정할 수 있습니다. 더 최신 `bg` 요청이 도착하면 이전 비동기 완료는 커밋하지 않으며 로드/디코드 실패 시에는 직전 프레임을 보존합니다. 이미 준비한 에셋 URL은 같은 실행 세션에서 다시 처리하지 않습니다. 선형 다음 챕터의 데이터 파싱과 에셋 예열은 첫 장면 직후 네트워크/디코드 경쟁을 만들지 않도록 `requestIdleCallback(timeout: 3000)`에서 시작하고, 미지원 브라우저는 1.2초 fallback을 사용합니다. 챕터가 바뀌면 남은 idle 작업을 취소하며 데이터 절약/2G·백그라운드 탭에서는 예열하지 않습니다.
