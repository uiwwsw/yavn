...
- 데스크톱 확대 원점과 모바일 확대 원점은 별도 CSS 변수로 전달합니다. 세로 모바일에서 폭 기준으로 축소된 전신 원화를 상단 원점으로 확대하면 얼굴이 대사창 쪽으로 더 내려가므로, `medium/close/reaction`은 1인 55%, 2인 80%, 3인 92%의 하단 중심 원점을 사용합니다. 확대할수록 얼굴·상체가 대사창 위 안전영역으로 올라오며 `wide`는 중앙 50%를 유지합니다. 이 값은 쇼트 변경 때 카메라 세계 전체에 적용되므로 개별 캐릭터 위치를 다시 계산하거나 타이핑 중 흔들지 않습니다.
