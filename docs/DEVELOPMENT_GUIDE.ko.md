...
장편 역사극의 채널·조건부 대사/선택·누적 상태·다중 실패/엔딩 샘플은 `public/game-list/deokman/`을 참고합니다. 12개 챕터에서 `dialogue/narration/record`, `say.when`, `choice.options[].when`, 외부 챕터 `goto`, 누적 상태 `branch`, 장면형 `gameOver`, 다중 `ending`을 함께 사용합니다.
