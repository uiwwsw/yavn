...
- 이미지 캐릭터(`.char-image`)는 `object-position: center bottom`으로 하단 정렬되고, 원본 보정 `scale`도 무대 바닥을 변형 원점으로 사용합니다. 정적 원본 높이는 인원수와 관계없이 `58cqh`이며 같은 구성 배율과 원점을 적용합니다. 따라서 1인 중앙 컷에서 2·3인 컷으로 전환되어도 이미지 박스의 머리 위치·하체 크롭·발 기준선이 달라지지 않고, 모바일과 고해상도 PC에서 기본 배우 높이가 플레이 프레임의 약 92%를 유지합니다.
- 화자 강조는 데스크톱/모바일 공통 규칙으로 동작합니다. 현재 화자는 가장 앞의 z축·원래 불투명도를 사용하고 비화자는 `speakerOrder` 순위에 따라 불투명도를 낮추지만, 같은 쇼트 안의 개별 캐릭터 크기는 바꾸지 않습니다. 다인 `close`의 `push/pan`은 전체 카메라 `duration`의 절반 이하이면서 최대 180ms를 비대상 인물 페이드에 먼저 배정하고, `visibility: hidden`이 된 뒤 남은 시간에 확대합니다. 퇴장 인물이 확대와 동시에 가장자리로 밀려 반만 남는 프레임과 투명 인물의 불필요한 합성을 막으며, 1인 `close`와 `cut`은 지연하지 않습니다. `reaction`은 듣는 카메라 대상을 전면에 둡니다.
...
- 모든 캐릭터는 인원별 기본 구도를 만드는 `.char-composition-world`와 그 자리에서 쇼트만 확대하는 `.char-camera-world` 안에 유지됩니다. 별도의 수평 팬 레이어는 없습니다. `medium`은 구성 배율 그대로, `wide`는 구성 배율을 상쇄한 의도적 전신/공간 컷, `close/reaction`은 대상의 고정 앵커를 원점으로 한 추가 확대입니다. 따라서 `medium -> close -> medium`에서도 대상의 좌우 위치가 바뀌지 않습니다.
- 1·2·3인은 기기별로 하나의 최종 배율을 공유합니다. `medium` 구성 배율은 모바일·데스크톱 모두 `1.58`, `wide`는 `1.00`입니다. 대상 확대는 데스크톱 `close 2.02`/`reaction 1.83`, 모바일 `close 1.84`/`reaction 1.72`로 나눠 세로 화면에서 `medium`과 근접 컷이 반복될 때 이미지가 과도하게 커졌다 작아지는 느낌을 줄입니다. 구성 레이어의 세로 원점은 항상 80%이고, `framing.scale`도 보이는 인원수에 따라 감쇠하지 않아 같은 프리셋이 같은 화면 비율을 뜻합니다. 쇼트 레이어는 대상 앵커에서 배율만 바꾸며 타이핑 레이아웃 변화로 인물 위치를 다시 계산하지 않습니다.
...
- 동일 인물의 슬롯 이동과 원본 보정/레거시 프레이밍 오프셋은 레이아웃을 일으키는 `left/width` 대신 합성 가능한 `translate`로 320ms 전환합니다. 화자 강조는 대형 이미지 `filter` 재도색 대신 `opacity`를 사용합니다. DSL의 `push` 기본 520ms, `pan` 기본 380ms, `cut` 즉시 전환은 유지하지만 카메라 레이어에는 배율만 전환되며 수평 이동은 발생하지 않습니다.
...
- 모바일 세로 화면에서 케이스 파일을 열면 `.stage-content-frame`의 `9:16` 높이 제한을 일시 해제해 전체 게임 뷰포트로 확장하고 헤더/탭을 고정합니다. 시스템 탭은 저장 카드 목록에만 내부 스크롤을 적용하되 `grid-auto-rows: max-content`로 각 카드가 버튼까지 포함한 내용 높이를 유지합니다. 배경음악·초기화면 설정 및 상태 문구는 하단 고정 행에 항상 온전히 표시합니다. 인벤토리 그리드는 수집 헤더와 필터 다음의 `minmax(0, 1fr)` 행을 사용해 남은 공간을 모두 채우며, 상세 이미지는 설명 위에 단일 열로 배치합니다. 짧은 가로 화면도 전체 높이 바텀시트를 사용합니다.
...
- `medium`은 1·2·3인 고정 구성에 동일한 월드 변환을 적용합니다. `close/reaction`은 대상 이동 없이 그 인물의 구성 앵커를 확대 원점으로 사용하고, `wide`는 구성 배율을 상쇄합니다. 따라서 원본 폭과 무관하게 쇼트 전환 전후의 대상 x 좌표가 같습니다.
...
- `narration`은 밝은 배경 이미지 위에서도 본문 대비를 유지하도록 템플릿별 고대비 전용 배경을 사용합니다. `paper-stage`는 완전 불투명한 종이색 바탕을 적용합니다.
