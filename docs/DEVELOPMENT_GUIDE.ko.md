...
- 화자 강조는 데스크톱/모바일과 정적 이미지/Live2D에 공통으로 적용합니다. 같은 쇼트의 화자와 비화자는 모두 원래 불투명도를 유지하고, 실제 `dialog.speakerId`와 일치하는 현재 대사 화자만 `2.2s` 주기의 미세 상하 이동과 `1.003 × 1.009` 이하 비균등 확대를 하단 중앙(`50% 100%`) 기준으로 반복합니다. 호흡은 좌우 반전용 `transform`을 덮지 않고 독립 `translate/scale`만 움직이며, 정점에는 최대 `3%` 밝기와 옅은 웜 글로우를 더합니다. 화자 변경 시 직전 화자는 진행 중인 호흡 1회를 중립 프레임까지 마친 뒤 정지하므로 중간 변형값에서 이미지가 꺾이거나 튀지 않습니다. 카메라 포커스 역할(`is-camera-focus/is-camera-listener`)과 실제 발화 상태(`is-speaking`)를 분리하므로 `reaction`에서 듣는 인물을 전면에 두어도 호흡은 대사 화자에게 남고 주변 인물은 흐려지지 않습니다. 다인 `close`의 비대상 인물 페이드는 화자 강조가 아니라 쇼트 밖 퇴장 연출로만 유지합니다.
...
- 시스템 탭은 자동 저장의 작동 상태와 최근 복구 시각을 상단 보호 카드에 표시하고, 수동 저장·챕터 시작점·JSON 백업을 독립 역할 카드로 분리합니다. 데스크톱은 3열, 모바일은 1열이며 현재 진행 저장만 주요 작업 색상을 사용합니다. 두 환경 모두 저장 카드 목록만 내부 스크롤하고 배경음악·`게임 시작 화면으로 가기`·상태 문구는 하단 고정 행에 유지합니다.
- 모바일 세로 화면에서 케이스 파일을 열면 `.stage-content-frame`의 `9:16` 높이 제한을 일시 해제해 전체 게임 뷰포트로 확장하고 헤더/탭을 고정합니다. 시스템 카드에는 `grid-auto-rows: max-content`를 적용해 각 카드가 버튼까지 포함한 내용 높이를 유지합니다. 인벤토리 그리드는 수집 헤더와 필터 다음의 `minmax(0, 1fr)` 행을 사용해 남은 공간을 모두 채우며, 상세 이미지는 설명 위에 단일 열로 배치합니다. 짧은 가로 화면도 전체 높이 바텀시트를 사용합니다.
