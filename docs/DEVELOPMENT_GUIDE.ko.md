...
- 스티커 레이어만 중앙 프레임 안의 실제 다이얼로그 상단을 `ResizeObserver`로 측정해 `stickerSafeInset`을 적용합니다. 따라서 증거물은 대화창을 피하지만 인물은 대사만 바뀌었다는 이유로 위아래로 움직이지 않습니다.
- 이미지 캐릭터(`.char-image`)는 `object-position: center bottom`으로 하단 정렬되고, 원본 보정 `scale`도 무대 바닥을 변형 원점으로 사용합니다. 정적 원본 높이는 인원수와 관계없이 `58cqh`이며 기기별 단일 카메라 배율과 원점을 적용합니다. 따라서 1인 중앙 컷에서 2·3인 컷으로 전환되어도 원본 박스의 발 기준선이 달라지지 않고, 기본 `medium`에서 배우의 렌더 높이는 PC 약 94%, 모바일 약 92%를 확보합니다.
