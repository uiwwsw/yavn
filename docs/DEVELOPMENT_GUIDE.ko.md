...
- `choice` 게이트는 일반 대사의 높이 예약을 해제하고 최대 높이를 플레이 프레임의 데스크톱 `46cqh`, 모바일 `48cqh`까지 확장합니다. 선택지 수는 렌더러의 `data-choice-count`로 전달되며 1개는 전체 폭, 2개는 2열, 3개는 2열과 전체 폭 마지막 행, 4개는 2×2로 배치됩니다. 따라서 720px 데스크톱과 일반 세로 모바일의 기본 4개 보기는 불필요한 내부 스크롤 없이 표시되고, 5개 이상의 긴 선택 묶음만 기존 한 열 본문 영역에서 스크롤합니다.
