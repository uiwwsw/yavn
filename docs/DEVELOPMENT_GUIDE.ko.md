...
- 이미지 캐릭터(`.char-image`)는 `object-position: center bottom`으로 하단 정렬되고, 원본 보정 `scale`도 무대 바닥을 변형 원점으로 사용합니다. 정적 원본 높이는 1/2/3인 각각 `66/58/67cqh`로 정규화한 뒤 구성 배율을 적용하므로 종횡비가 다른 원본도 발 기준선을 공유하고, 모바일과 고해상도 PC에서 최종 배우 높이가 플레이 프레임의 약 92%를 유지합니다.
...
- 모바일과 데스크톱은 동일한 인원별 최종 배율을 사용합니다. `medium`은 1/2/3인 `1.40/1.58/1.38`, `close`는 `1.67/2.02/1.92`, `reaction`은 `1.52/1.83/1.69`입니다. 구성 레이어의 세로 원점은 1인 55%, 다인 80%이며, 쇼트 레이어는 대상 앵커에서 배율만 바꿉니다. 짧은 세로 화면에서도 머리·얼굴을 안전영역에 남기고 타이핑 레이아웃 변화로 인물 위치를 다시 계산하지 않습니다.
