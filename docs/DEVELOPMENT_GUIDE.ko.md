...
- 화자 강조는 데스크톱/모바일과 정적 이미지/Live2D에 공통으로 적용합니다. 같은 쇼트의 화자와 비화자는 모두 원래 불투명도를 유지하고, 실제 `dialog.speakerId`와 일치하는 현재 대사 화자만 `2.2s` 주기의 미세 상하 이동과 `1.003 × 1.009` 이하 비균등 확대를 바닥 기준으로 반복합니다. 카메라 포커스 역할(`is-camera-focus/is-camera-listener`)과 실제 발화 상태(`is-speaking`)를 분리하므로 `reaction`에서 듣는 인물을 전면에 두어도 호흡은 대사 화자에게 남고 주변 인물은 흐려지지 않습니다. 다인 `close`의 비대상 인물 페이드는 화자 강조가 아니라 쇼트 밖 퇴장 연출로만 유지합니다.
