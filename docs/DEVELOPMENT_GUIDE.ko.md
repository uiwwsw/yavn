...
- `say/choice/input.with`가 현재 카메라의 인물 구성을 좁혀도 배치된 캐릭터 DOM과 Live2D 인스턴스는 제거하지 않습니다. 화면 밖 인물에는 이미지·Live2D 공통 `.is-camera-hidden` 퇴장 규칙을 적용해 180ms 동안 `opacity`를 낮추고 완료 시점에 `visibility: hidden`으로 합성에서 제외합니다. 퇴장 중에는 이전 앙상블 레이아웃을 모든 쇼트에서 유지하고 위치·크기 전환 시간을 `0ms`로 강제하므로, 사라지는 인물이 새 1인/2인 자리로 미끄러지거나 뒤집히지 않습니다. 페이드가 끝난 뒤 숨김 상태에서 새 좌표로 즉시 정렬합니다. 새로 등장하거나 다시 노출되는 인물은 다음 노출 집합과 레이아웃을 같은 페인트에 커밋합니다. `layout: cut`은 기존 배우와 새 배우의 위치·크기를 즉시 확정하고 지정한 입장 효과만 재생합니다. `layout: push`는 새 배우의 입장 효과와 함께 계속 보이는 배우의 `translate/scale`을 지정 시간으로 보간합니다. 빠른 진행의 여러 노출 요청은 다음 페인트 프레임에 마지막 집합만 커밋하고, 챕터 로딩 중 최초 노출은 지연 없이 반영하며, `prefers-reduced-motion`에서는 레이아웃 대기와 모션을 생략합니다.
