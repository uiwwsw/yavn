...
- 모든 캐릭터는 고정 원점에서 확대하는 `.char-camera-world`와 대상을 중앙으로 옮기는 `.char-camera-pan` 안에 유지됩니다. 두 transform을 분리해 `transform-origin`을 애니메이션하지 않으며, `medium`은 전원을 함께 확대하고 `close/reaction`은 같은 무대를 대상 인물 쪽으로 이동합니다.
...
- `close/reaction` 상태가 남아 있어도 현재 `say/choice/input`에 화자나 명시 `camera.target`이 없으면 직전 표적을 재사용하지 않습니다. 이 경우 렌더링만 현재 인원수의 `medium` 그룹 구도로 완화하고, 저장용 카메라 상태와 전환 정보는 유지합니다. 화자 없는 내레이션에서 특정 인물을 계속 잡으려면 해당 액션의 객체형 `camera.target`에 캐릭터 ID를 직접 지정해야 합니다.
...
- 다만 현재 액션에 화자와 명시 `camera.target`이 모두 없는 내레이션은 직전 `close/reaction`의 인물 표적을 상속하지 않고 `medium` 그룹 구도로 렌더링합니다. 의도적인 내레이션 클로즈업은 해당 `say.camera.target`에 캐릭터 ID를 함께 적습니다.
...
- `medium`은 인물 수에 따라 안전 배율을 사용하되 모든 인물에 동일한 월드 변환을 적용합니다. `close/reaction`은 개별 인물을 키우는 대신 고정 원점 확대 레이어와 대상 이동 레이어를 조합합니다.
