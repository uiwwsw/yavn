...
- 스티커·증거물은 화면 전체가 아니라 HUD·다이얼로그·기기 safe-area를 제외한 `.sticker-safe-frame`을 좌표 기준으로 사용합니다. 이미지와 초기 캐릭터·카메라 레이아웃이 안정될 때까지 숨긴 뒤 실제 렌더 경계를 한 번 측정하고, 보이는 `.char-layer .char`와 겹치면 작성 위치에서 가장 가까운 좌·우·상단 빈 후보를 최종 위치로 선택합니다. 빈 공간이 없을 때만 단계적으로 균일 축소하며, 확정한 좌표·크기는 픽셀로 잠가 대사창 높이와 후속 카메라 전환을 따라 이동하지 않습니다. 입장 이펙트도 위치 확정 뒤 시작합니다. 기기 회전이나 큰 PC 프레임 변경 때만 다시 숨기고 한 번 재배치하며, 모바일 주소창에 의한 높이 변화는 무시합니다.
