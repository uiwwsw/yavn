...
- 모바일 세로 화면에서 케이스 파일을 열면 `.stage-content-frame`의 `9:16` 높이 제한을 일시 해제해 전체 게임 뷰포트로 확장하고 헤더/탭을 고정합니다. 시스템 탭은 저장 카드 목록에만 내부 스크롤을 적용하고 배경음악·초기화면 설정 및 상태 문구는 하단 고정 행에 항상 온전히 표시합니다. 인벤토리 그리드는 수집 헤더와 필터 다음의 `minmax(0, 1fr)` 행을 사용해 남은 공간을 모두 채우며, 상세 이미지는 설명 위에 단일 열로 배치합니다. 짧은 가로 화면도 전체 높이 바텀시트를 사용합니다.
