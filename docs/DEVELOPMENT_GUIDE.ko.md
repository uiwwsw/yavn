...
- 스티커 안전 프레임은 중앙 프레임 안의 실제 다이얼로그 상단을 `ResizeObserver`로 측정해 `stickerSafeInset`을 반영합니다. 스티커는 초기 안정화가 끝난 시점의 대화창·보이는 인물 경계를 기준으로 충돌 없는 위치를 한 번 정하고, 노출 후에는 그 좌표를 유지합니다. 기기 회전이나 의미 있는 PC 무대 크기 변경에서만 숨김-재측정-재노출 순서로 갱신하므로 대사 타이핑, 화자 전환, 모바일 주소창 변화가 증거물을 흔들지 않습니다.
...
- 다이얼로그가 수동 숨김 상태면 하단 대화창 inset을 `0`으로 바꿉니다. 이미 노출된 스티커의 잠긴 좌표와 `stage-bottom` 캐릭터는 그대로 두고, `prompt-top` 캐릭터 레이어만 화면 바닥 기준선으로 부드럽게 이동합니다. 복원 시에는 실측 프롬프트 상단으로 돌아갑니다.
...
- 일반 다이얼로그 박스 최대 높이는 게임 화면 가림을 줄이기 위해 `데스크톱 38%`(38cqh), `모바일 48%`(48cqh)로 제한하며, 초과 콘텐츠는 내부 스크롤로 처리합니다. 선택 게이트는 별도로 데스크톱 `46cqh`, 모바일 `64cqh`까지 확장됩니다.
