...
- 이미지 캐릭터(`.char-image`)는 `object-position: center bottom`으로 하단 정렬되고, 원본 보정 `scale`도 무대 바닥을 변형 원점으로 사용합니다. 서로 다른 캔버스 비율을 키워도 발 기준선이 고정되며, 폭은 프레임의 `cqw/cqh` 중 안전한 값으로 정해져 고해상도 화면에서 고정 픽셀 상한 때문에 배우가 작아지지 않습니다.
...
- 실제 노출 캐릭터가 정확히 두 명이면 `.char-layout-duo`가 자동 적용됩니다. 모바일은 25%/75%를 유지하고, PC는 중앙으로부터 `min(22cqw, 260px) × spacing`만큼 떨어뜨려 넓은 화면에서도 두 사람이 같은 대화권 안에 있도록 합니다. 좌우는 `left -> center -> right` 슬롯 순서를 따르며 화자 변경으로 뒤집히지 않습니다.
- 2인 중 한 명이 퇴장하면 같은 ID와 슬롯에 남은 인물은 이전 좌우 역할과 2인용 너비를 유지합니다. 새 인물로 교체되거나 동일 인물이 다른 슬롯으로 이동하면 이 기억을 상속하지 않습니다.
- 3인은 `.char-layout-trio`를 사용합니다. 모바일은 15%/50%/85% 분할을 유지하고, PC는 중앙으로부터 `min(18cqw, 260px) × spacing`만큼 떨어뜨립니다. 카메라 쇼트나 화자가 바뀌어도 앵커 자체는 이동하지 않아 불필요한 흔들림이 없습니다.
- 모든 캐릭터는 고정 원점에서 확대하는 `.char-camera-world`와 대상을 중앙으로 옮기는 `.char-camera-pan` 안에 유지됩니다. 두 transform을 분리해 `transform-origin`을 애니메이션하지 않으며, `medium`은 전원을 함께 확대하고 `close/reaction`은 같은 무대를 대상 인물 쪽으로 이동합니다. 단독 배우가 물리적으로 `left/right` 슬롯에 있어도 `wide`가 아닌 카메라는 그 슬롯 중심을 자동으로 프레임 중앙에 맞추며, `wide`만 원래 좌우 배치를 유지합니다.
- 데스크톱과 모바일은 확대 배율·원점을 별도 CSS 변수로 전달합니다. 모바일 `medium`은 1/2/3인 `1.40/1.58/1.38`, `close`는 `1.67/2.02/1.92`, `reaction`은 `1.52/1.83/1.69`를 사용합니다. 1인 원점은 `medium/reaction 55%`, `close 60%`, 다인 원점은 `80%`, `wide`는 중앙 50%입니다. 짧은 세로 화면에서도 머리·얼굴을 안전영역에 남기고 인물군이 대사창 하단에 몰리지 않으며, 이 값은 카메라 세계 전체에만 적용되어 개별 캐릭터 위치나 타이핑 레이아웃을 다시 계산하지 않습니다.
...
- 동일 인물의 슬롯 이동과 원본 보정/레거시 프레이밍 오프셋은 레이아웃을 일으키는 `left/width` 대신 합성 가능한 `translate`로 320ms 전환합니다. 화자 강조는 대형 이미지 `filter` 재도색 대신 `opacity`를 사용합니다. 카메라는 `push` 기본 520ms, `pan` 기본 380ms이며 `cut`은 즉시 바뀝니다.
...
- 대사 문자열과 대화창 높이는 카메라 상태에 영향을 주지 않습니다. 같은 인물·같은 `camera`가 이어지면 고정된 쇼트를 유지하고, 위치 이동은 새 `char.position`, 쇼트 이동은 다른 `camera` 지시가 있을 때만 수행합니다.
- `medium`은 인물 수에 따라 안전 배율을 사용하되 모든 인물에 동일한 월드 변환을 적용합니다. `close/reaction`은 개별 인물을 키우는 대신 고정 원점 확대 레이어와 대상 이동 레이어를 조합합니다. 좌우 기본 슬롯은 25%/75% 중심과 `-50%` 자체 오프셋을 사용하고 카메라도 같은 중심의 반대 이동량을 사용하므로, 단독 또는 표적 클로즈업에서 원본 폭과 무관하게 대상 중심이 맞습니다.
- PC 앙상블은 화면 비율 분할 대신 유동 간격과 260px 상한을 사용하며 `calibration.spacing`으로 넓거나 좁은 실루엣을 미세 조정합니다. 모바일은 일관된 2·3분할을 유지합니다.
