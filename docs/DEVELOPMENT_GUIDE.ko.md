...
- 화자 강조는 데스크톱/모바일 공통 규칙으로 동작합니다. 현재 화자는 가장 앞의 z축·원래 불투명도를 사용하고 비화자는 `speakerOrder` 순위에 따라 불투명도를 낮추지만, 같은 쇼트 안의 개별 캐릭터 크기는 바꾸지 않습니다. `close`에서는 비대상 인물을 페이드해 잘린 손·하체가 화면 가장자리에 남지 않으며, `reaction`은 듣는 카메라 대상을 전면에 둡니다.
...
- `say/choice/input.with`가 현재 카메라의 인물 구성을 좁혀도 배치된 캐릭터 DOM과 Live2D 인스턴스는 제거하지 않습니다. 화면 밖 인물에는 `.is-camera-hidden`을 적용해 즉시 숨기고 위치·표정 상태를 유지합니다. 다음 컷에서 같은 인물을 다시 보여 줄 때 `characterEnter`와 슬롯 이동 전환을 재실행하지 않으며, 실제 `char` 액션으로 새 슬롯이 배치되거나 교체될 때만 최초 등장으로 처리합니다.
