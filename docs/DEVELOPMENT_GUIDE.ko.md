...
- 모바일은 케이스 파일 높이를 안정적인 바텀시트로 고정하고 헤더/탭을 제외한 본문만 스크롤합니다. 인벤토리 그리드는 수집 헤더와 필터 다음의 `minmax(0, 1fr)` 행을 사용해 남은 공간을 모두 채우며, 상세 이미지는 설명 위에 단일 열로 배치합니다. 짧은 가로 화면에서는 바텀시트를 플레이 프레임 전체 높이로 확장합니다.
