...
- 화자 강조는 데스크톱/모바일 공통 규칙으로 동작합니다. 현재 화자는 가장 앞의 z축·원래 불투명도를 사용하고 비화자는 `speakerOrder` 순위에 따라 불투명도를 낮추지만, 같은 쇼트 안의 개별 캐릭터 크기는 바꾸지 않습니다. 다인 `close`의 `push/pan`은 비대상 인물을 최대 180ms 동안 페이드하고 72ms 뒤 카메라 이동을 겹쳐, 퇴장과 줌이 두 단계로 끊기지 않게 합니다. 1인 `close`와 `cut`은 지연하지 않으며 `reaction`은 듣는 카메라 대상을 전면에 둡니다.
...
- 동일 인물의 슬롯 이동과 원본 보정/레거시 프레이밍 오프셋은 레이아웃을 일으키는 `left/width` 대신 합성 가능한 `translate`로 380ms 전환하고 크기는 독립 `scale`로 360ms 전환합니다. 화자 강조는 대형 이미지 `filter` 재도색 대신 `opacity`를 사용합니다. DSL의 `push` 기본 520ms, `pan` 기본 380ms, `cut` 즉시 전환은 유지하며 카메라 배율과 수평 팬은 같은 시간축을 사용합니다.
...
- 화자만 확대하던 모바일/다인 depth 배율은 제거되었습니다. 화자와 청자는 불투명도·z축으로 분리하고 크기 변화는 무대 카메라만 담당합니다.
