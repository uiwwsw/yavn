...
- 권장 런타임: Node.js `22.13+` (배포·CI는 `22.x`, pnpm `11.x`)
- Vercel 빌드도 `corepack pnpm`을 사용해 `package.json`에 고정된 pnpm `11.21.0`을 실행합니다.
- pnpm 11은 `package.json.pnpm` 설정을 읽지 않으므로 의존성 설치 스크립트 허용 목록을 `pnpm-workspace.yaml`의 `allowBuilds`에 둡니다. 현재는 `esbuild: true`만 허용합니다.
...
assets:
  ...
  characters:
    conan:
      ...
      placement: prompt-top # 반신형 원화를 프롬프트 상단에 정렬
      facing: left
...
`assets.characters.<id>.placement`는 레이어 병합 시 다른 캐릭터 속성과 함께 보존되는 **인물별 에셋 선언**입니다. 게임 전역이나 장면 액션에 한 번 지정하는 설정이 아닙니다. 허용값은 `stage-bottom | prompt-top`이며, 생략 시 `stage-bottom`으로 해석합니다. 전신형 원화는 `stage-bottom`, 대사/선택 프롬프트 위에 놓을 반신형 원화는 `prompt-top`을 사용합니다.
...
- 점프 대상이 현재 준비된 번호 챕터 시퀀스에 있으면 원래 시퀀스와 `chapterIndex/chapterTotal`을 유지하고 기존 인덱스로 이동
- 현재 시퀀스 밖의 경로로 점프할 때만 대상부터 같은 폴더의 번호 파일을 새 시퀀스로 구성해 순차 진행
- URL/ZIP 저장 재개도 먼저 최초 준비 시퀀스에서 `chapterPath`를 찾습니다. 루트 12개 중 `7.yaml` 저장을 열었을 때 `1/5`로 재구성하지 않고 원래 `8/12` 진행도와 다음 챕터 프리로드 범위를 유지합니다.
...
- 이미지 준비와 실제 렌더링은 `imageResources.ts`의 공통 리소스 풀을 사용합니다. `ResourceImage`는 `display: contents` 호스트에 준비된 실제 `<img>`를 빌려 붙이고, 화면에서 제거되면 풀에 돌려줍니다. 타이틀·캐릭터 표정·배경·스티커·공격 컷인·엔딩에 적용합니다. 동일 요소/소스의 `decode()`와 로딩 화면의 준비 확인은 하나의 결과를 공유하므로, 준비된 이미지가 다시 등장할 때 요소 생성·디코딩을 반복하지 않습니다. 동시에 여러 위치에 표시할 때는 요소를 분리하되 원본 바이트를 공유합니다.
- 상대·절대 URL을 절대 URL로 통합하고 같은 출처의 HTTP 이미지는 한 번 받은 바이트를 공유 Blob URL로 표시합니다. 외부 출처 이미지는 CORS가 없는 서버도 지원하도록 원래 `<img>` 로딩을 유지합니다. ZIP의 Blob URL과 data URL은 그대로 사용하며, ZIP URL 해제 시 해당 캐시를 무효화합니다. 실패·시간 초과한 준비 결과는 재시도 가능하도록 캐시에서 제거합니다. HTTP 캐시가 허용되지 않아도 보유한 같은 출처 이미지 바이트로 표시할 수 있습니다.
- 리소스 정리 기준은 디코딩 이미지 96MiB(너비×높이×4 추정치), 원본 바이트 64MiB, 최대 80개 리소스입니다. 사용 중인 요소는 보호하고 오래된 미사용 요소부터 해제합니다. 디코딩 요소가 해제되어도 원본 바이트가 남으면 네트워크 없이 다시 준비합니다. 원본 바이트까지 정리된 경우에는 다시 요청할 수 있습니다. 이 상한은 브라우저/GPU 전체 메모리의 하드 제한이 아니며, 표시 중인 이미지가 기준을 넘으면 반환 후 정리합니다. 같은 페이지의 이야기 재시작에서도 보유한 HTTP 이미지를 재사용합니다.
- 첫 장면을 DOM에 반영한 뒤 현재 배경·정적 캐릭터·Live2D의 준비 상태를 확인한 후 로딩을 닫습니다. 이미지 준비가 늦으면 직전 표정/배경을 유지하며 최신 요청과 일치하는 성공만 표시합니다. `bg`는 준비된 pending 요소를 current로 승격하고 직전 배경과 기본 620ms 디졸브합니다. 캐릭터 표정도 풀에서 빌린 요소를 크로스디졸브하므로 기존 구도·입퇴장·빠른 입력의 최신 요청 우선 동작은 유지합니다.
- 다음 챕터 예열은 `requestIdleCallback(timeout: 3000)` 또는 미지원 브라우저의 1.2초 fallback에서 시작합니다. 다음 챕터의 같은 출처 이미지는 바이트만 받아 두고 디코딩과 요소 생성은 챕터 진입 시 수행해, 현재 장면과 CPU/디코딩 메모리를 다투지 않습니다. 외부 출처 이미지는 실제 준비 시 원래 URL로 로딩합니다. 챕터가 바뀌면 대기 중인 idle 작업을 취소하며 데이터 절약/2G·백그라운드 탭에서는 예열하지 않습니다.
...
- 스티커·증거물은 HUD·다이얼로그·기기 safe-area를 제외한 `.sticker-safe-frame`을 좌표 기준으로 사용합니다. 최초 등장에만 이미지 디코딩과 캐릭터·카메라 경계 안정화를 기다린 뒤, 작성 위치와 가장 가까운 빈 공간에서 입장 이펙트를 시작합니다. 공간이 부족하면 종횡비를 유지해 축소합니다. 한 번 노출한 뒤에는 대사·화자·카메라·대화창 높이가 바뀌어도 같은 DOM과 마지막 안전 배치를 계속 표시합니다. 재측정 중 숨기거나 입장 페이드를 반복하지 않습니다. 기존 위치가 새 프레임 안에 있고 인물과 겹치지 않으면 유지하며, 잘림·충돌이 생기거나 작성 좌표가 바뀌면 보이는 상태에서 좌표·크기·배율을 기본 280ms로 보간합니다. 배치 측정은 이미지 없는 임시 요소에서 수행해 표시 중인 DOM의 좌표나 진행 중 전환을 건드리지 않습니다. 기기 회전이나 큰 PC 프레임 변경도 기존 그림을 유지한 채 다시 맞추며 모바일 주소창의 작은 높이 변화는 무시합니다. 모션 감소 환경에서는 보간을 생략합니다.

### 같은 아이템과 인물의 연속성

- `sticker.id`는 화면에 살아 있는 물건의 식별자입니다. 같은 ID를 다시 선언하면 기존 DOM과 입장 완료 상태를 유지하며, `image`가 바뀌면 새 이미지를 디코딩한 뒤 180ms로 겹쳐 전환합니다. 준비 지연·실패 중에는 직전 그림을 유지하고 빠른 연속 요청은 최신 이미지로 연결합니다.
- 같은 ID에서 생략한 `x/y/width/height/anchorX/anchorY/rotate/opacity/zIndex`는 현재 값을 이어받습니다. 전부 같은 선언은 상태를 갱신하지 않고 `inputLockMs`도 다시 걸지 않습니다. 새 아이템이나 실제 변경에는 작성된 잠금을 적용합니다. `width/height: auto`로 크기 지정을 해제할 수 있습니다.
- `clearSticker`가 실제로 제거한 뒤 다시 표시하면 새 물건 수명으로 입장합니다. 퇴장 중 같은 ID를 다시 지정하면 제거 예약을 취소하고 기존 인스턴스를 사용합니다. 퇴장 효과 중에도 마지막 안전 배치를 유지합니다.
- 같은 `char.id`의 표정과 좌우 위치 변경은 기존 배우 DOM을 유지합니다. 렌더 배열도 위치 순으로 다시 정렬하지 않아, 두 배우가 자리를 바꿀 때 DOM 이동으로 CSS 전환이 끊기지 않습니다. 표정은 디코딩 후 겹쳐 전환하고 월드 좌표는 별도로 보간합니다. 명시적 `enter.layout: cut`과 모션 감소 설정은 즉시 배치 동작을 유지합니다.
- 기존 YAML·저장 형식은 그대로 사용합니다. 덕만 첫 방의 족보처럼 여러 대사에 걸쳐 같은 아이템을 보여 주는 장면은 데이터 수정 없이 개선을 적용받습니다.

```yaml
# assets.backgrounds에 closed_register / open_register를 선언한 장면
- sticker: { id: register, image: closed_register, x: 30, y: 25, width: "220px" }
- say: { channel: thought, text: "이 이름이… 내 이름인가?" }
- sticker: { id: register, image: open_register, x: 70 } # y·폭 유지, 그림과 위치 전환
- say: { channel: action, text: "접힌 장을 펼쳐 빛에 비춘다." }
- clearSticker: { id: register, leave: fadeOut }
```
...
- V10.8은 `investigation_count`의 방문 완료와 핵심 증거 완성을 별도로 판정합니다. 네 명을 모두 만나도 필요한 증거가 빠졌다면 `route_select`로 돌아가며, 조건부 힌트와 라인별 후속 대면으로 놓친 증거를 회수한 뒤 결론으로 이동합니다.
- 라운지 챕터 권장 흐름:
...
- `goto: ./...` 또는 `goto: /...`는 챕터 점프입니다. 대상이 현재 준비된 시퀀스에 있으면 전체 시퀀스를 보존한 채 해당 인덱스로 이동하고, 시퀀스 밖이면 대상 파일부터 같은 폴더의 번호 챕터를 새로 구성합니다.
...
- 최종 챕터 병합 결과 캐시
- URL 원문과 존재 확인은 새 게임 실행을 시작할 때 `cache: no-store` 요청으로 다시 확인합니다. 동일 실행 안에서 챕터를 재진입할 때만 위 메모리 캐시를 재사용해 재fetch/재parse를 막습니다.
...
- 엔딩은 마지막 장면 위에서 제목·결과·후일담을 먼저 보여 줍니다. 플레이어가 `엔딩 기록과 크레딧`을 눌러야 수집 현황과 제작진으로 이동하며 처음부터 자유롭게 스크롤할 수 있습니다. `결말 다시 보기`로 되돌아갈 수 있습니다.
- 게임오버도 마지막 무대와 인물을 보존한 결과 장면을 먼저 보여 줍니다. `선택을 되돌아보기`를 누르면 기존 원인 선택·자동 저장·수동 저장·챕터·백업 복구 UI로 연결됩니다. 강제 자동 진행은 없으며 키보드 초점은 결과 모달 안에서 순환합니다.
- 로컬 BGM은 게임오버에서 900ms 동안 사라지고, 엔진 자체 제작 효과음이 결과를 알립니다. 엔딩은 마지막 BGM을 이어 쓰거나 `endings.<id>.music`으로 크로스페이드합니다. YouTube BGM은 게임오버에서 즉시 정지합니다.
...
- Start Gate 또는 게임 런타임이 활성화되면 `html/body/#root`에 `.game-shell-locked`를 적용하고 `body`를 `position: fixed`, `100dvh`, `overscroll-behavior: none`인 앱 셸로 전환합니다. 이 상태에서 배경 드래그와 스크롤 컨테이너의 상·하단 경계를 넘는 단일 터치를 `passive: false` 핸들러가 차단하므로 iOS Safari의 당겨 새로고침·고무줄 바운스와 Android 문서 스크롤이 본편을 움직이지 않습니다.
- `.dialog-content-scroll`, 로그, 인벤토리, 저장 카드, 법적 고지, 게임오버, 엔딩 크레딧처럼 실제로 넘치는 요소는 `touch-action: pan-y`, `overscroll-behavior: contain`, iOS 관성 스크롤을 유지합니다. 내부에서 이동할 여지가 있을 때는 터치 기본 동작을 막지 않습니다. 런처는 `.game-shell-locked` 대상이 아니므로 기존 페이지 스크롤을 유지합니다.

## 8-5) 캐릭터 레이어/정렬 동작

기본 배치는 `auto`입니다. 작성자는 인원을 세어 좌우 빈칸을 정리할 필요가 없습니다. `char.position`을 생략하거나 `auto`로 지정하면 같은 ID는 기존 슬롯을 유지하고 새 ID는 `left → center → right`의 첫 빈 슬롯에 등록합니다. 화면의 실제 구도는 현재 보이는 1·2·3명에 맞춰 계산합니다. 기존 명시 슬롯도 기본 자동 구도에서는 상대적인 순서를 정하는 용도로 호환됩니다.
...
scenes:
  conversation:
    # layout: auto # 기본값
    actions:
      - char: { id: deokman } # position: auto와 동일
      - char: { id: king, position: auto }
      - char: { id: yushin, position: auto }
      - say: { char: deokman, text: "모두 모였군요." }
      - say: { char: deokman, with: [king], text: "둘이 이야기하지요." } # 두 명을 좌우로
      - say: { char: deokman, with: [], text: "생각할 시간이 필요해." } # 한 명을 중앙으로
  doorway:
    layout: fixed # 이 장면에서만 카메라 이전의 좌·중·우 기준을 고정
    actions:
      - char: { id: deokman, position: left }
      - say: { char: deokman, with: [], text: "오른쪽 문을 비워 두어라." }
```

슬롯은 최대 세 개이며 숨은 배우도 등록을 유지합니다. 모두 차 있을 때 새 ID를 `auto`로 추가하면 기존 배우를 임의로 지우지 않고 작성 오류를 표시합니다. 그 경우 교체할 `position`을 명시합니다. `scene.layout`의 `auto/fixed`는 **구도 정책**, 아래 `char.enter.layout`의 `push/cut`은 **입장 시 자리 이동 방식**입니다. 저장 복원도 같은 자동 슬롯 배정과 현재 장면의 정책을 사용합니다.

정적 전신 원화는 `defaultFraming`과 하단 크롭으로 대화용 상반신 구도를 만들 수 있습니다. `cropBottom: 0.4`는 원화 아래 40%를 제외하고 위 60%를 안전 높이에 맞추므로 얼굴이 약 1.67배 커집니다. 머리 기준선과 화면 상단 여백은 유지하며 대화창 아래쪽에서만 자릅니다. 이 옵션은 정적 이미지용이며 Live2D는 기존 모델 프레이밍을 사용합니다.
  ...
  characters:
    deokman:
      ...
      framings:
        ...
        full: { scale: 1 }
      calibration: { spacing: 0.8 }
# 대화에는 기본 portrait, 전신을 볼 지점에는 say: { char: deokman, framing: full, ... }
```

크롭을 바꿔도 바깥 배치 박스와 이미지 요소는 유지하고 내부 그림의 `transform` 확대율만 360ms로 전환합니다. 그림 높이를 매 프레임 변경하지 않아 반복 레이아웃·래스터 작업을 줄이며, 위쪽 중앙 기준 확대와 하단 클립으로 기존 크기·머리 여백을 유지합니다. 표정 디졸브와 입장 효과를 다시 시작하지 않습니다. `cropBottom` 범위는 `0..0.65`, 생략 시 0이며 모션 감소 환경에서는 보간을 생략합니다.

대화창은 엔진이 정한 너비·높이를 사용하는 독립 레이아웃 영역(`contain: layout size`)입니다. 타이핑에 따른 줄바꿈·스크롤 계산이 무대 전체로 전파되지 않습니다. `paint` containment는 사용하지 않으므로 대화창 바깥 컨트롤과 포커스 테두리를 자르지 않습니다. `promptHeight`, 선택지/입력에 따른 높이 변경, 화면 회전은 계속 반영됩니다. 타이핑 마지막 글자의 연출은 `Intl.Segmenter`를 재사용하고 마지막 위치의 grapheme만 조회합니다. 매 글자마다 표시된 문장 전체의 배열을 만들지 않으며, 한글 자모·결합문자·이모지의 경계를 보존합니다. 구형 Segmenter 및 미지원 환경의 기존 대체 경로도 유지합니다.

2026-09-11 성능 확인: production 빌드의 Chromium, 390×844 및 1440×900 뷰포트, CPU 4배 제한에서 덕만·진평왕·유신을 배치하고 덕만의 `full`↔`portrait`를 6회 전환했습니다. 두 크기 모두 해당 구간의 레이아웃 횟수가 138→36으로 줄었고, 후속 대사 타이핑의 레이아웃 루트 85회는 모두 문서 전체에서 대화창 내부로 바뀌었습니다. 이미지 DOM과 기존 로드·디코딩 캐시를 유지합니다. 이는 제어된 장면의 브라우저 작업량 비교이며 실제 기기의 FPS 개선율을 뜻하지 않습니다. DSL·개별 게임·저장 데이터 마이그레이션은 필요 없습니다.

캐릭터를 무대에 배치할 때 `char.enter`로 다음 등장 연출을 지정할 수 있습니다.
...
- 문자열 단축형과 `{ effect, layout, duration, easing, delay }` 객체형을 지원합니다.
- `effect`: `none | fadeIn | wipeLeft | scaleIn | popIn | slideUp | slideDown | slideLeft | slideRight | wipeCenterX | wipeCenterY | blurIn | rotateIn`.
- `layout`: `push`면 계속 보이는 배우도 새 앙상블 자리까지 같은 시간에 이동하고, `cut`이면 기존 배우의 새 자리를 즉시 확정합니다. `none`의 기본은 `cut`, 다른 효과의 기본은 기존 동작을 보존하는 `push`입니다.
- `duration`: `0..3000ms`, 기본 `360ms`. `delay`: `0..5000ms`, 기본 `0`. `easing`: 최대 64자의 CSS easing 문자열이며 기본 `cubic-bezier(0.22, 0.61, 0.36, 1)`입니다. `layout: push`에서는 이 세 값을 새 인물 효과와 계속 보이는 배우의 자리 이동에 함께 적용합니다.
- `enter`를 생략하면 `fadeIn + push + 360ms`를 사용합니다. 동일 인물의 표정·프레이밍만 바뀌면 등장 효과를 다시 실행하지 않으며 `say.char` 감정 교체 뒤에도 마지막 등장 설정을 보존합니다.
- OS `prefers-reduced-motion` 환경에서는 등장 애니메이션과 앙상블 자리 이동을 모두 생략합니다.

- 배경(`.bg`)과 시스템 로딩/엔딩 오버레이는 전체 뷰포트를 유지합니다.
- 실제 플레이 UI는 `.stage-content-frame` 안에서 렌더링합니다. PC에서는 `width: min(100cqw, 100cqh × 16 / 9)`와 `height: min(100cqh, 100cqw × 3 / 4)`를 함께 사용해 화면 안에서 가장 큰 `4:3 ~ 16:9` 프레임을 만들고 가로·세로 중앙에 놓습니다. 세로 모니터는 4:3, 중간 비율은 전체 화면, 울트라와이드는 16:9에서 멈춥니다. 768px 이하 세로 화면은 포인터 종류와 무관하게 `width: 100cqw; height: 100cqh`로 전체 뷰포트를 채웁니다. HUD는 상단 안전영역, 대화창은 하단 안전영역에 맞추므로 긴 화면에서 하단 여백이 남거나 UI가 위로 몰리지 않습니다.
- `.effect-viewport`와 `.stage-content-frame`은 각각 크기 컨테이너입니다. 배경은 바깥 뷰포트를 기준으로 풀블리드 렌더링하고, 정적 이미지·Live2D 캐릭터의 폭과 카메라/배치 좌표는 안쪽 프레임의 `cqw/cqh`를 사용하므로 프레임 밖 배경 크기에 끌려가지 않습니다.
- 프레임에는 캐릭터·스티커·HUD·다이얼로그·선택/입력·컷신·CASE FILE이 함께 들어가므로 서로 다른 좌표계를 사용해 벌어지지 않습니다.
- 캐릭터 레이어(`.char-layer`)는 중앙 플레이 프레임 전체에 `inset: 0`으로 고정됩니다. 대화 타이핑으로 박스 높이가 달라져도 캐릭터 무대의 경계나 위치는 다시 계산하지 않습니다.
- 캐릭터 좌표와 배율 계산은 중앙 프레임의 `cqw/cqh`를 그대로 사용하되, `.stage-content-frame`과 `.char-layer`·`.char-composition-world`·`.char-camera-world`는 좌우 `overflow: visible`과 비페인트 레이아웃 containment를 사용합니다. 따라서 `close` 확대나 팬으로 실루엣이 중앙 프레임을 넘더라도 풀블리드 배경 위에 계속 그려지며, 바깥 `.effect-viewport`의 paint/overflow 경계가 실제 브라우저 뷰포트 밖 페인트와 문서 스크롤만 차단합니다. 스티커 안전 프레임과 컷신 등 자체 클리핑 레이어는 기존 경계를 유지합니다.
- 스티커 안전 프레임은 실제 다이얼로그 상단을 `ResizeObserver`로 측정해 `stickerSafeInset`을 반영합니다. 최초 배치가 끝난 스티커는 대사 전환·카메라 이동·회전 중에도 계속 표시합니다. 새 경계가 안정되면 기존 위치의 잘림과 충돌만 검사하고, 안전하면 유지하며 필요할 때만 부드럽게 보정합니다. 재측정 대기와 표시 여부를 분리했으므로 매 대사마다 숨김/재등장하지 않습니다.
- 이미지 캐릭터(`.char-image`)는 `object-position: center bottom`으로 하단 정렬되고, 원본 보정 `scale`도 무대 바닥을 변형 원점으로 사용합니다. 기본 `stage-bottom` 정적 원본 높이는 인원수와 관계없이 `52cqh`이며 기기별 단일 카메라 배율과 원점을 적용합니다. 따라서 1인 중앙 컷에서 2·3인 컷으로 전환되어도 원본 박스의 발 기준선이 달라지지 않고, 기본 `medium`에서 배우의 렌더 높이는 PC 약 84%, 모바일 약 82%를 확보합니다. `close`도 PC 약 97%, 모바일 약 90%로 제한해 머리 위 여백을 남기고 하단 크롭은 대화창 뒤에서만 일어나게 합니다.
- 화자 강조는 정적 이미지·Live2D 공통으로 하단 중심에서 `98.2% → 100%`를 340ms 동안 전환하고, 말하는 인물을 앞에 합성합니다. 밝기·채도 감쇠와 캐릭터 외곽 그림자는 사용하지 않습니다. 최대 크기는 이미 화면에 맞춘 100%이므로 머리 여백을 침범하지 않습니다. 같은 화자의 대사 사이 빈 로딩 구간에서는 강조를 유지하고 실제 내레이션에서 중립으로 돌아갑니다. `prefers-reduced-motion`에서는 크기 전환을 생략하며 이름표와 합성 순서로 구분합니다. 등장·표정·좌우 반전·카메라는 각자의 레이어를 유지합니다.
- 기본 `scene.layout: auto`에서는 현재 보이는 캐릭터가 한 명이면 중앙, 두 명이면 좌우, 세 명이면 좌·중·우로 배치합니다. 기존 `left/center/right`는 자동 구도에서 상대적인 순서를 정하며, 생략하거나 `auto`로 지정하면 같은 ID의 슬롯을 유지하고 새 인물은 첫 빈 슬롯부터 배정합니다. `fixed` 장면에서는 한두 명이어도 명시한 슬롯을 옮기지 않습니다.
- `auto`의 좌우 간격은 PC에서 `min(25cqw, 260px) × 평균 calibration.spacing`, 모바일에서 `25cqw × 평균 calibration.spacing`입니다. 카메라 팬도 같은 간격으로 계산해 클로즈업 대상이 중앙에 맞습니다. 기본 spacing 1은 기존 25%/75% 구도이며, 덕만은 큰 상반신 원화가 가장자리로 밀리지 않도록 간격을 줄였습니다. `fixed`는 간격 보정보다 작성한 슬롯의 기준 좌표를 우선합니다.
- 모든 캐릭터는 단일 절대 배율을 담당하는 `.char-composition-world`와 대상의 수평 좌표를 담당하는 `.char-camera-world` 안에 유지됩니다. `transform-origin`을 대상마다 순간 변경하거나 구성 배율과 쇼트 배율을 중첩하지 않습니다. 축약형 `camera: close`의 기본 `push`는 현재 그룹의 수평 구도를 유지하고 `.char-composition-world`의 배율만 보간합니다. `close`에 `target`을 명시하면 기본 `pan`으로 대상을 따라가며, 명시적 `push`는 대상이 있어도 수평 이동 없이 확대만 수행합니다.
- 1·2·3인은 기기별로 하나의 최종 배율을 공유합니다. PC `wide/medium/close/reaction`은 `1.18/1.62/1.86/1.78`, 모바일은 `1.12/1.58/1.74/1.66`입니다. 세로 원점은 PC `86%`, 모바일 `82%`로 분리해 대화창 위에서 얼굴과 상반신 비중을 확보합니다. `framing.scale`도 보이는 인원수에 따라 감쇠하지 않아 같은 프리셋이 같은 화면 비율을 뜻하고, 타이핑 레이아웃 변화로 카메라를 다시 계산하지 않습니다.
- 엔진의 최초 구도는 `medium`입니다. 명시한 카메라 쇼트와 대상은 다음 카메라 지시까지 유지되며, 화자 변경이나 화자 없는 내레이션만으로 확대가 풀리거나 다른 인물에게 팬하지 않습니다. 대상이 실제로 숨거나 퇴장하면 수평 팬만 그룹 중앙으로 돌아오고 쇼트 배율은 유지합니다. 대화 구도로 돌아가려는 지점에 `camera: medium`을 명시합니다.
- `assets.characters.<id>.calibration(scale/x/y/spacing)`은 쇼트와 독립적으로 원본 캔버스의 인물 크기·여백을 정규화합니다. `spacing`은 PC·모바일 자동 구도에서 현재 보이는 배우들의 평균값으로 계산해 좌우에 대칭 적용합니다. 표정 에셋은 같은 보정값을 공유하므로 감정 교체 때 눈높이가 뛰지 않습니다. 이미 같은 높이의 캔버스로 정규화한 원화에는 중복 `scale/y` 보정을 넣지 않습니다.
- `assets.characters.<id>.placement`는 캐릭터마다 원화 기준선을 선택합니다. 기본 `stage-bottom`은 무대 바닥, `prompt-top`은 현재 대사/선택 프롬프트 상단을 기준으로 하며, 한 게임 안에서도 전신형과 반신형 인물이 서로 다른 값을 가질 수 있습니다. 두 기준선은 좌우 1·2·3인 구도와 카메라 팬을 함께 계산하므로 혼합 장면에서도 중앙에 겹치지 않고, 현재 화자가 속한 레이어가 다른 기준선의 청자보다 앞에 합성됩니다. 이 값은 게임 전역 레이아웃 옵션이 아닙니다.
- `prompt-top`은 `무대 높이 − 읽기 기준선의 하단 inset − HUD 여백`을 실제 표시 가능 높이로 사용합니다. 이 높이를 기기별 최대 카메라 배율과 1 이상의 인물 보정 배율로 나눈 값으로 원본 `52cqh`를 제한하고, 이미지와 Live2D 정사각 캔버스에 함께 적용합니다. 카메라 쇼트·인원수만 바뀔 때는 원본 크기가 유지되어 확대 연출을 보존합니다. 자동 선택/입력 확장은 기존 읽기 기준선을 사용하므로 인물이 축소되거나 위로 밀리지 않습니다. HUD 여백은 PC 100px, 모바일 세로 88px, 높이 520px 이하 가로 화면 70px를 기본으로 노치 안전영역도 반영합니다. 명시한 `x/y` 보정은 유지하며, 작성자가 의도한 화면 밖 이동은 이 크기 제한과 별개입니다. DSL·게임 YAML 마이그레이션은 필요하지 않습니다.
- 같은 캐릭터 ID를 새 `position`에 배치하면 엔진은 해당 ID가 있던 이전 슬롯을 먼저 제거하고, 캐릭터 ID 기반 렌더 키와 위치 독립 Live2D 로드 키로 기존 인스턴스를 유지합니다. 장면 전환에서 `center -> left`처럼 위치를 바꿔도 동일 인물이 두 슬롯에 남거나 `char.enter`를 반복하지 않습니다.
- 동일 인물의 표정은 실제 DOM 이미지 디코드가 완료된 뒤 180ms 크로스디졸브로 바뀝니다. 이전·다음 표정을 함께 합성하고 알파 합을 유지하므로 중간에 배경이 비치며 어두워지지 않습니다. 빠른 연속 요청은 진행 중인 디졸브를 마친 뒤 최신 표정으로 연결하며, 이전 비동기 완료는 무시합니다. 첫 이미지의 비율로 외부 배치 박스를 고정하고 각각의 원화는 같은 높이·하단 중앙에 그려 표정/의상 크기가 달라도 가로 앵커가 튀지 않습니다. 이미지 로딩 실패 시 준비된 이전 표정을 유지합니다.
- `say/choice/input.with`는 실제 노출 인원을 정합니다. 기본 `scene.layout: auto`에서는 3→2→1로 줄어들면 남은 배우가 두 사람의 좌우 구도→한 사람의 중앙 구도로 부드럽게 이동합니다. 퇴장 배우는 마지막 자리·시선에서 페이드를 마치고 DOM·Live2D 인스턴스는 유지합니다. `scene.layout: fixed`에서는 생존 배우도 작성한 좌·중·우 기준을 유지합니다. 화자·표정만 바뀌고 노출 집합이 같으면 재배치하지 않습니다.
- DOM 수명과 자동 구도 계산은 분리됩니다. 숨은 배우가 슬롯에 남아 있어도 `auto` 구도 인원에는 포함하지 않습니다. 전원 퇴장 때만 마지막 구도와 카메라 배율을 유지해 사라지는 중에 축소되지 않으며, 다음 노출 집합이 생기면 새 구도로 배치합니다. `fixed`는 장면 단위 설정이고 다음 장면에서 생략하면 다시 `auto`입니다.
...
- 명시적인 슬롯 이동과 원본 보정/레거시 프레이밍 변화는 `translate` 380ms, 독립 `scale` 360ms로 전환합니다. `layout: push`로 새 배우가 합류하면 작성한 `char.enter.duration/easing/delay`에 맞춥니다. 입장 효과는 내부 레이어에서 재생하므로 시선 반전이나 월드 이동과 충돌하지 않습니다. 카메라 DSL의 `push` 기본 520ms, `pan` 기본 380ms, `cut` 즉시 전환은 별도로 유지합니다.
- `.bg`·`.char-composition-world`·`.char-camera-world`·개별 캐릭터의 합성 힌트는 상시 유지하지 않습니다. 디코드된 배경이 실제로 겹치는 동안에만 `data-background-transitioning`을, 배율·팬·배치·노출 키가 바뀐 레이아웃 커밋에서만 `data-camera-moving`/`data-character-moving`을 켭니다. 캐릭터 전환은 가장 긴 시간에 64ms 여유를 더한 뒤 힌트를 해제하고 배경은 이전 레이어를 제거할 때 함께 해제해, 정지 장면의 GPU 레이어 메모리를 회수합니다. 단순 화자 변경은 캐릭터 모션 키를 갱신하지 않습니다.
- 캐릭터 등장은 이미지 디코드 또는 Live2D 모델 준비가 끝난 뒤 시작합니다. 기본은 최종 자리에서 `fadeIn + push + 360ms`, `none`은 즉시 표시입니다. 월드 배치·시선, 입퇴장, 표정 합성을 각각 분리하여 서로의 변형을 덮지 않습니다. 슬라이드는 최종 자리 근처의 짧은 이동, 팝/스케일은 과장된 반동 없이 98%에서 100%로 연결합니다. 이미 보이는 같은 ID의 표정/위치 변경에는 등장 효과를 다시 실행하지 않습니다. 첫 캐스팅은 최종 카메라 배율에서 등장해 빈 무대에서 확대되는 동작을 없앴습니다.
...
- 수동 진행 입력(클릭/`Enter`/`Space`)이 직전 입력 뒤 620ms 안에 반복되면 런타임은 700ms짜리 `catch-up` 템포를 활성화합니다. 새 상태로 시작하는 배경 크로스페이드·카메라 이동·캐릭터 위치/크기 및 입장·스티커 입퇴장/안전 재배치는 작성된 효과를 유지하면서 지연을 `0ms`, 주요 모션을 최대 140ms의 빠른 ease-out으로 제한합니다. 캐릭터 퇴장 페이드는 최대 90ms, 충돌 회피 안정화 대기는 최대 190ms입니다. 실행 중인 CSS transition/키프레임의 지속 시간을 도중에 다시 쓰면 진행률이 튈 수 있으므로 배경 소스, 카메라 목표, 캐릭터 노출·배치, 스티커 렌더·회피 키별로 시작 당시 템포를 종료까지 고정하고 다음 상태 변화부터 단축값을 적용합니다. AUTO 엔진 호출은 이 템포를 활성화하지 않으며 단발 수동 진행과 AUTO는 YAML에 작성된 원래 시간을 보존합니다. DSL 문법 변경은 없습니다.
- 화자 구분은 크기 1.8%와 앞뒤 합성 순서만 사용합니다. 청자를 어둡게 만들거나 불투명도를 낮추지 않으며 이미지/Live2D는 같은 규칙을 사용합니다.
- 다이얼로그가 수동 숨김 상태면 하단 대화창 inset을 `0`으로 바꿉니다. 이 숨김/복원만으로는 이미 노출된 스티커의 잠긴 좌표와 `stage-bottom` 캐릭터를 그대로 두고, `prompt-top` 캐릭터 레이어만 화면 바닥 기준선으로 부드럽게 이동합니다. 복원 시에는 실측 프롬프트 상단으로 돌아갑니다.
- `prompt-top`의 크기 제한에는 마지막으로 측정한 읽기 inset을 별도로 보존합니다. 따라서 대화창을 수동으로 숨겨도 인물 원본이 갑자기 커지지 않고 기준선만 이동합니다.

## 8-6) 선택/입력 게이트 키보드 동작

- `choice` 게이트가 열리면 첫 번째 옵션 버튼에 자동 포커스됩니다.
- `choice` 게이트는 일반 대사의 높이 예약을 해제하고 최대 높이를 플레이 프레임의 데스크톱 `46cqh`, 모바일 `64cqh`까지 확장합니다. 선택지 수는 렌더러의 `data-choice-count`로 전달되며 데스크톱에서는 1개 전체 폭, 2개 2열, 3개 `2+1`, 4개 `2×2`로 배치됩니다. 768px 이하에서는 모든 기본 선택을 1열로 전환하고 각 옵션에 두 자리 순번·진입 표시·최소 58px 터치 영역·`:active` 눌림 피드백을 제공합니다. 패널을 넘는 긴 묶음만 내부 스크롤합니다.
...
- `sticker.inputLockMs`가 설정된 스티커 액션이 실행되는 동안에는 `input` 제출/`choice` 선택이 잠기며, 지정 시간이 끝난 뒤 다음 액션으로 자동 진행됩니다. 잠금 중 단발 진행 입력은 기존처럼 무시하지만 620ms 안의 반복 입력은 사용자가 빠르게 넘기려는 의도로 보고 마지막 요청 하나만 최대 1.6초 보류합니다. 잠금이 끝난 뒤 대사가 진행 가능한 상태일 때만 실행하며, 새 `choice`/`input`/컷신/게임오버에 도달하거나 마지막 연타가 오래됐으면 보류 요청을 폐기합니다.
- `say.wait`가 설정된 대사는 시작 시점부터 지정 시간 동안 즉시 진행/스킵되지 않습니다. 잠금 중 빠른 반복 입력도 대기 시간을 줄이지 않고 마지막 요청 하나만 최대 1.6초 보류하며, 시간이 모두 지난 뒤 일반 진행 규칙(`unskippable`, 선택/입력 게이트 포함)을 다시 검사합니다.
...
- 캔버스 리사이즈는 `devicePixelRatio`를 반영하되 최대 DPR `2`의 드로잉 버퍼 크기(`clientWidth/Height * min(DPR, 2)`)를 사용해 고해상도 화면의 입력/렌더 좌표를 유지하면서 과도한 픽셀 처리량을 제한합니다. `ResizeObserver`가 프롬프트 기준선 전환 중 연속 호출되어도 80ms quiet window 뒤 최종 크기만 반영해 WebGL 버퍼를 매 프레임 재할당하지 않습니다.
- Live2D ticker는 모델 준비 과정에서는 화면 밖 캐릭터도 계속 실행해 로딩 교착을 피합니다. 모델이 준비된 뒤에는 `.is-camera-hidden`, 준비되지 않은 `prompt-top`, 백그라운드 탭의 ticker를 정지하고 다시 보일 때 같은 인스턴스를 재개합니다. 다인 `close`의 화면 내 주변 인물은 ticker도 활성 상태를 유지합니다.
...
- 자동·수동 저장과 JSON 백업은 현재 배경의 `assets.backgrounds` 에셋 ID를 선택 메타·상태·인벤토리와 함께 기록합니다. 불러오기는 먼저 저장 커서까지 연출을 재생한 뒤 유효한 저장 배경 ID를 최종 적용합니다. 따라서 분기 재생이 저장 scene에 닿지 못하는 경우에도 저장 당시 배경을 복원합니다. 이 필드가 없는 구버전 저장은 같은 챕터의 스토리 로그와 현재 scene에서 각 커서 직전의 마지막 `bg`를 순서대로 유추하고, 유추할 수 없을 때만 기존 커서 재생 결과를 사용합니다.
- 저장의 `gameTitle`은 현재 게임 제목과 일치해야 하며, 저장과 현재 게임에 `version`이 모두 선언되어 있으면 두 버전도 정확히 같아야 재개합니다. 버전이 다른 자동·수동·챕터 저장은 시작 화면의 `이어하기` 대상에서 제외하고 런타임에서도 읽지 않아, 장면 ID와 선택 구조가 바뀐 콘텐츠에 구버전 커서를 억지로 적용하지 않습니다.
- 시스템 탭에서 자동 저장은 최신 시각과 작동 상태만 표시합니다. 일반 플레이 중 자동 저장 직접 불러오기는 숨기고, 수동 저장/불러오기·챕터 복귀·JSON 백업만 명시적 작업으로 제공합니다.
- 자동 저장은 시작 화면 `이어하기`에 사용하고, 게임오버의 `죽음의 원인 선택으로`와 다중 엔딩의 `마지막 선택으로`는 별도 선택 복구점을 우선 사용합니다. 백업은 수동 진행 상태만 포함하며 현재 게임 제목이 다른 파일은 거부합니다.
- 새 진행 payload와 `.yavn-save.json` 백업 envelope는 각각 `schemaVersion: 2`를 기록합니다. 무버전/v1 진행은 최초 슬롯 조회 또는 백업 가져오기 때 누락된 route/inventory/history/log와 최신 메타를 채워 v2로 다시 저장합니다. 현재 엔진보다 큰 저장/백업 버전은 해석하거나 낮은 버전으로 덮어쓰지 않으며, 백업은 업데이트 안내 오류를 반환합니다.
...
- 스와이프·마우스 드래그·좌우 화살표·키보드 `←`/`→`/`Home`/`End`·인디케이터 선택은 같은 캐러셀 상태를 갱신하지만 평상시 주소는 변경하지 않습니다. `선택 링크 복사`에서만 `?demo=<gameId>` URL을 생성하고, 공유 링크에서 다른 데모를 탐색하면 쿼리를 제거합니다. 검색·태그 필터는 아래 직접 실행 목록에만 적용되며 상단 캐러셀 선택에는 영향을 주지 않습니다.
- 캐러셀은 `embla-carousel-react`의 transform 기반 스냅 엔진을 사용합니다. 뷰포트에 `position: relative`를 지정해 좌우 페이지 여백이 스냅 좌표에 더해지지 않도록 합니다. 드래그·관성 이동 중 `scroll` 콜백을 사용하지 않고 `settle` 이벤트에서 선택 스냅을 한 번만 확정하므로 SEO 메타, 권리 고지, 이미지 로딩 속성이 프레임마다 바뀌지 않습니다. 모든 대표 이미지와 권리 고지 DOM은 이동 전에 안정된 구조로 준비합니다.
- 쇼케이스는 PC에서 소개/이미지 2열, 768px 이하에서 이미지/소개 세로 배치입니다. 모든 슬라이드를 가장 긴 콘텐츠 높이에 맞추고 페이지 전체로 스크롤하므로 작은 뷰포트에서도 실행 버튼과 권리 고지를 자르지 않습니다. 대기·빈 목록·실패 상태는 정적인 안내를 사용하며 실패 시 다시 불러올 수 있습니다. OS 모션 감소에서는 버튼/키보드 캐러셀 이동과 태그 행 위치 복귀를 즉시 처리하고 카드 확대를 생략합니다.
- 대표 슬라이드의 `작품 소스` URL은 모든 게임에 공통으로 `<repositoryUrl>/tree/main/public/game-list/<encodedGameId>`에서 계산합니다. 데모 ID별 URL 분기를 추가하지 않습니다.
- 작품 목록은 데스크톱 4열, 태블릿 2열, 휴대폰 1열로 배치하며 카드 전체가 게임 실행 링크입니다. 하단 제작 소개는 집필·연출·플레이 흐름과 제작 가이드·샘플 YAML 링크를 제공합니다. 헤더의 `내 게임 열기`는 ZIP 업로드이며 처리 중 입력을 비활성화합니다. 엔진 스키마·빌드 ID·매니페스트 시각은 방문자용 헤더와 푸터에 표시하지 않습니다. 공통 입력 처리기는 텍스트 노드의 부모 요소를 확인하고 문서 대상 이벤트는 건너뛰므로 런처 텍스트 선택 중 `closest` 오류를 내지 않습니다. 검색은 명시적 지우기와 빈 결과 초기화를 지원하며, 태그는 사용 빈도순 상위 8개를 우선 표시합니다. 모바일에서도 제작 가이드와 키보드 접근 가능한 ZIP 입력을 유지하고 런처에서는 링크·버튼·텍스트 선택 같은 브라우저 기본 상호작용을 차단하지 않습니다.
...
- 시작 화면은 별도의 움직임 켜기/끄기 버튼 없이 배경 연출을 자동 재생합니다. 음악 버튼은 BGM을 전환하며 시작 음악은 덮개가 닫히는 동안 페이드아웃합니다. 본편 효과 `minimal` 설정도 타이틀의 자동 재생을 끄지 않습니다. OS 모션 감소 설정은 존중하고, 숨겨진 탭에서는 캔버스·시차 루프·영상을 일시 정지합니다. 모바일 입자 수를 줄이고 DPR은 최대 1.5로 제한합니다.
...
- 다이얼로그 박스 우측 상단(박스 외부 컨트롤 레이어)에 `AUTO`와 `숨기기` 버튼을 표시합니다.
- `AUTO`는 플레이어 설정이며 일반 `say`의 타이핑이 끝난 뒤 설정 간격만큼 기다려 다음 대사로 진행합니다. `choice`/`input` 게이트에서는 절대 옵션을 대신 선택하거나 입력하지 않고 정지합니다. 작성자가 `say.autoAdvance`를 선언한 대사는 DSL 타이머가 우선하며 플레이어 AUTO 타이머를 중복 예약하지 않습니다.
- CASE FILE/시스템 설정창 또는 수동 숨김 상태에서는 AUTO 타이머를 취소합니다. 다시 닫거나 대화창을 열었을 때 현재 대사가 이미 완성되어 있으면 설정 간격을 새로 시작합니다.
...
- 대사 기본 높이는 `170px`, 입력 기본 높이는 `220px`입니다. 선택지는 PC에서 2~4개일 때 2열, 그 외와 모바일에서는 1열의 행 수에 따라 높이를 계산하고 제한시간 표시 공간을 더합니다. 최대 4행까지 계산하며 화면의 안전 높이를 넘는 내용만 내부 스크롤합니다. 게임의 `ui.promptHeight`와 액션별 `promptHeight`는 자동 높이보다 우선합니다. 타이핑 도중에는 바깥 높이를 바꾸지 않습니다. 자동 확장한 선택지·입력 패널은 `prompt-top` 인물을 위로 밀지 않고 기존 읽기 기준선을 유지하며, 명시한 작성자 높이는 캐릭터 기준선에도 반영합니다.
- `다음`·`완료`·`입력 대기`·`선택 대기`를 표시하던 하단 상태 전용 행과 10px 간격은 렌더링하지 않습니다. PC와 모바일 모두 해당 26px을 본문·입력·선택 영역에 돌려주며, 진행 입력은 기존 화면 클릭/`Enter`/`Space`와 각 게이트 컨트롤을 그대로 사용합니다.
- 모바일 다이얼로그는 화면 좌우·하단에 최소 10px 외곽 여백을 두며, 노치·홈 인디케이터의 safe-area inset이 더 크면 해당 값을 우선합니다. 입력 컨트롤은 최소 48px, 선택 카드는 최소 56px 높이를 사용합니다.
...
- 플레이 감각과 `초기화면 가기`는 인벤토리 탐색에서 분리해 시스템 탭에 배치합니다. ZIP 실행 게임에서는 초기화면 버튼을 비활성화합니다.
- 인게임 UI는 `gameInterface.css`의 공통 색상·간격·타이포그래피를 사용하고 세 가지 `ui.template`의 분위기를 유지합니다. HUD는 기록·소지품·메뉴의 SVG 아이콘과 PC용 짧은 이름을 표시합니다. 대화창은 폭을 제한한 읽기 영역, 선택지는 번호·진입 아이콘을 갖춘 버튼으로 구성합니다. 게임 메뉴는 PC 왼쪽 탐색 메뉴와 모바일 상단 탭을 사용하며 기록은 시간순 목록, 소지품은 이미지 카드와 상세 보기, 저장·설정은 작업별 카드로 구성합니다. 게임오버·엔딩·로딩·컷신 제어도 같은 버튼과 글자 규칙을 따릅니다.
- 메뉴 탭은 방향키와 `Home/End`로 이동합니다. 메뉴/소지품 상세의 `Tab` 초점은 열린 창 안에서 순환하고, `Esc`는 상세→메뉴→플레이 순서로 닫으며 열었던 버튼에 초점을 돌려줍니다. 메뉴가 열린 동안 타이핑·선택 제한시간·대기·공격/효과의 이야기 시계와 CSS 타이머를 함께 멈추고 닫으면 남은 시간부터 이어갑니다. 저장·검색·필터·음량 조절은 그대로 사용할 수 있습니다.
- 시스템 탭은 자동 저장의 작동 상태와 최근 복구 시각을 상단 보호 카드에 표시하고, 수동 저장·챕터 시작점·JSON 백업·플레이 감각·법적 고지를 독립 역할 카드로 분리합니다. 플레이 감각은 텍스트 출력 배속(`0.75/1/1.5/2×`), 플레이어 AUTO와 완료 후 간격(`800/1400/2200ms`), 화면 효과 강도(`full/reduced/minimal`), BGM 활성/음량, SFX 음량을 제공합니다. 데스크톱은 저장 카드 2열과 플레이 감각 2열 컨트롤, 모바일은 모두 1열이며 현재 진행 저장만 주요 작업 색상을 사용합니다.
- 플레이어 텍스트 배속은 `config.settings.textSpeed`의 기본 CPS와 대사 내부 `<speed=...>` 구간 CPS에 같은 비율로 적용합니다. YAML에 작성된 상대적인 빠르기 차이는 유지하면서 전체 읽기 속도만 바뀝니다.
- 모바일 세로 화면의 케이스 파일은 플레이 화면과 같은 전체 게임 뷰포트에서 안전영역을 확보하고 헤더/탭을 고정합니다. 시스템 카드에는 `grid-auto-rows: max-content`를 적용해 각 카드가 컨트롤까지 포함한 내용 높이를 유지하고 카드 목록만 스크롤합니다. `게임 시작 화면으로 가기`와 상태 문구는 하단 고정 행에 유지합니다. 인벤토리 그리드는 수집 헤더와 필터 다음의 `minmax(0, 1fr)` 행을 사용해 남은 공간을 모두 채우며, 상세 이미지는 설명 위에 단일 열로 배치합니다. 짧은 가로 화면은 왼쪽 탐색과 스크롤 가능한 본문을 사용합니다.
- 플레이어 설정은 게임별 키(`vn-engine-settings:<autosave-key>`)에 `schemaVersion: 2`로 저장됩니다. 기존 무버전 payload의 `bgmEnabled/autoSaveEnabled/inventory*` 값은 보존하고 새 속성은 안전한 기본값을 채운 뒤 v2로 다시 기록합니다. 현재 엔진보다 큰 schema는 이해 가능한 값만 메모리에서 사용하고 원본 payload를 덮어쓰지 않습니다.
- 인벤토리 `보기 탭/정렬/카테고리` 마지막 선택도 같은 게임별 키로 저장되어 다음 실행 시 복원됩니다.
- 끄기 상태에서는 현재 BGM을 즉시 정지하고, 켜면 현재 트랙 재생을 재시도합니다.
- BGM 크로스페이드는 32ms interval 대신 `requestAnimationFrame`으로 볼륨을 보간합니다. 설정창이 열린 동안 Live2D `active`를 끄고 캐릭터·스티커·타이핑 CSS 애니메이션을 pause해 모달 뒤 GPU/합성 작업을 줄입니다.
- 모바일 소지품은 2열 카드와 설명 위에 이미지를 둔 상세 시트를 사용합니다. 좁은 가로 화면에서는 메뉴 탐색을 왼쪽에 유지하고 설정 본문을 스크롤해 모든 컨트롤에 접근할 수 있습니다.
...
- 역사/서사: `moonveil(900ms)`, `embers(1100ms)`, `crown(1200ms)`, `eclipse(1400ms)`, `starfall(1200ms)`, `inkstamp(720ms)`
...
- 모든 프리셋은 기본 `full` 모드에서 짧은 중심 bloom과 가장자리 비네트를 공통 envelope로 합성해 타격점의 광량 변화를 분명하게 만듭니다. 플레이어가 `reduced`를 선택하면 흔들림·확대·흐림 진폭과 overlay 광량을 낮추고, `minimal`은 공간 변형/글리치/blur를 짧은 밝기 cue로 대체하며 overlay 광량을 30% 수준으로 낮춥니다. `effect.wait`의 엔진 시간은 설정과 무관하게 기존 프리셋 값을 유지하므로 플레이어 취향이 작성된 게임 흐름을 바꾸지 않습니다. YAML 프리셋 이름과 문법은 바뀌지 않습니다.
...
```yaml
- bg: courtyard # 기존 축약형: 다음 액션 즉시 실행, 기본 620ms 디졸브
- bg:
    id: corridor
    transition: fade
    duration: 900
...
- say:
    channel: narration
    text: "발소리가 멎었다. 문 아래 그림자만 천천히 길어졌다."
- attack:
    attacker: guard
    target: heroine
    from: right
...
| 필드 | 값 / 기본값 | 동작 |
| --- | --- | --- |
| `id` | 배경 에셋 ID, 필수 | `assets.backgrounds` 참조 |
| `transition` | `dissolve` / `fade` / `wipeLeft` / `wipeRight` / `cut` | 기본 `dissolve`. `fade`는 암전을 거치고, 와이프는 지정 방향으로 새 배경을 드러냄 |
| `duration` | 0..5000ms, 기본 620 | `cut`은 항상 0ms. 수동 연타의 기존 적응형 모션 정책 적용 |
| `wait` | 객체형 기본 `true` | 새 이미지의 디코드와 화면 전환 완료 뒤 다음 액션 실행. `false`와 문자열 축약형은 즉시 진행 |

이미 표시 중인 동일 URL은 다시 전환하지 않습니다. 오래된 완료 알림은 최신 장면에 적용하지 않습니다. 실패한 이미지는 직전 배경을 지우지 않으며, 대기형 요청은 오류를 표시하고 다음 대사를 실행하지 않습니다. 렌더러가 응답하지 않는 경우에도 `12초 + duration + 1초` 상한에서 입력 잠금을 해제하고 오류를 표시합니다. 저장 복원은 배경을 복원하되 이전 전환 대기를 다시 재생하지 않습니다.

`attack` 옵션:

| 필드 | 값 / 기본값 |
| --- | --- |
| `attacker` | `assets.characters`의 인물 ID, 필수 |
| `image` | 선택 사항: `assets.backgrounds`의 공격 전용 장면 컷 ID |
| `target` | 다른 인물 ID 또는 `player`; 기본 `player`는 1인칭 피격 |
| `from` | `left` / `center` / `right`, 기본 `right` |
| `style` | `slash`(베기) / `strike`(충돌) / `shot`(발사), 기본 `slash` |
| `strength` | `light` / `heavy`, 기본 `heavy` |
| `anticipation` | 100..3000ms, 기본 520 |
| `impact` | 80..500ms, 기본 160 |
| `recovery` | 100..3000ms, 기본 620 |
| `approachSound`, `sound` | 접근/타격 시점의 `assets.sfx` ID. 생략 시 엔진 자체 제작 효과음 |
| `caption` | 최대 160자의 짧은 현장 묘사 |

`image`가 있으면 디코드를 확인한 뒤 시퀀스를 시작하고, 이미지 실패 시 기존 배우 연출로 이어집니다. 전용 컷은 접근·타격 후 반응 구간에 사라집니다. 모바일 세로 화면에서는 전체 구도를 contain으로 보존합니다.

공격자와 대상은 2인 `medium` 구도로 묶이고, 화면에 없는 인물은 빈 슬롯을 우선 사용해 자동 배치합니다. 이미 배치한 인물의 표정·원본 보정은 유지합니다. `from`은 접근·타격 방향이며 영구 슬롯 위치를 강제로 바꾸지 않습니다. 접근→타격→반응 순서 동안 클릭·AUTO·수동 저장은 잠기고 완료 후 다음 액션으로 이어집니다. 이전의 단발 화면 효과는 공격 시작 시 정리합니다. 연속된 같은 공격도 독립 시퀀스로 재생하며 챕터 변경·저장 복구 시 타이머와 효과음을 정리합니다. 저장 커서 이전의 공격은 배치만 복원하고 효과음·피격을 다시 실행하지 않습니다.

체력이나 피해량을 자동으로 변경하는 액션은 아닙니다. 실제 결과는 뒤의 `say`, `set`, `branch`, `gameOver`로 작성합니다. ‘누가 어디서 접근하는지 → 피할 공간이 왜 없는지 → 충격 뒤 무엇이 달라졌는지’를 앞뒤 대사에 넣으면 동일한 효과도 장면의 원인과 결과를 갖게 됩니다.

SFX 음량 설정은 기본·사용자 효과음 모두에 적용됩니다. 재생 중 음량을 0으로 바꾸거나 장면을 복구하면 기존 효과음도 정지합니다. `reduced`/`minimal` 및 OS 모션 감소 설정은 공격의 흔들림·돌진·섬광선을 제거하고 순서·캡션·입력 잠금 길이는 유지합니다. `minimal`은 배경 애니메이션도 제거합니다.

### 9-0-1) 결말별 연출 메타데이터

```yaml
# config.yaml — 경로 대신 에셋 ID를 참조
endings:
  dawn:
    title: "새벽까지 살아남다"
    message: "두 사람은 함께 성문을 빠져나왔습니다."
    epilogue: "다음 날에도 그 문은 열려 있었습니다."
    background: morning_gate
    music: aftermath
    tone: hopeful
```

`epilogue`, `background`, `music`, `tone`은 선택 사항입니다. `tone`은 `hopeful`/`tragic`/`mystery`로 결말의 강조색을 정합니다. 배경 우선순위는 개별 엔딩의 `background` → 공통 `endingScreen.image` → 마지막 플레이 장면입니다. 엔딩 배경은 챕터 시각 에셋 프리로드에 포함되며 배경·음악 참조 오류는 게임 해석 단계에서 검사합니다. 기존 `title`/`message`만 있는 엔딩과 엔딩 정의가 없는 게임도 새 결과 화면을 사용합니다.
...
원화의 전신·상반신 범위는 `framings`로, 장면 전체의 쇼트와 대상은 `camera`로 설정합니다. `camera`는 독립 액션과 `say/choice/input` 내부에 모두 선언할 수 있습니다.
...
- `assets.characters.<id>.calibration.spacing` (optional): `0.75..1.25`, 기본 `1`. PC 2·3인 구도에서 보이는 에셋들의 평균으로 계산하는 대칭 최대 간격 배율
...
- 엔진의 최초 구도는 `medium`입니다. 명시한 카메라 쇼트와 대상은 다음 카메라 지시까지 유지되며, 화자 변경이나 화자 없는 내레이션만으로 확대가 풀리거나 다른 인물에게 팬하지 않습니다. 대상이 실제로 숨거나 퇴장하면 수평 팬만 그룹 중앙으로 돌아오고 쇼트 배율은 유지합니다. 대화 구도로 돌아가려는 지점에 `camera: medium`을 명시합니다.
- 대사 문자열과 대화창 높이는 카메라 상태에 영향을 주지 않습니다. 같은 인물·같은 `camera`가 이어지면 고정된 쇼트를 유지하고, 좌우 위치 이동은 새 `char.position`, 배율 전환은 다른 `camera` 지시가 있을 때만 수행합니다.
- 연속 액션이 같은 캐릭터 슬롯, 화자 우선순위, `with` 노출 집합, 카메라 상태를 다시 계산해도 스토어는 기존 참조를 유지합니다. 실제 표정·배치·대상·쇼트가 바뀌지 않은 대사는 캐릭터 진입이나 카메라 전환을 재시작하지 않습니다.
- `medium`은 1·2·3인 고정 구성에 동일한 기기별 최종 배율을 적용합니다. 축약형 `camera: close`는 현재 그룹의 수평 구도를 유지한 채 단일 절대 배율만 적용합니다. 객체형 `close`에 `target`을 명시하면 기본 `pan`으로 대상 앵커를 중앙에 맞추며, 명시적 `push`는 수평 이동 없이 확대만 보간합니다. `reaction` 기본값도 `pan`이고, `transform-origin`은 대상마다 바꾸지 않습니다.
- PC 앙상블은 모바일과 같은 2인 25%/75%, 3인 25%/50%/75% 구성을 출발점으로 삼되 유동 간격에 260px 상한을 사용하며, 보이는 배우들의 평균 `calibration.spacing`으로 넓거나 좁은 실루엣을 대칭 보정합니다.
...
- 타이핑 스케줄러는 `requestAnimationFrame`으로 페인트와 동기화합니다. 프레임이 늦어 여러 grapheme의 예정 시각이 지난 경우 최신 단계까지 루프에서 계산하되 스토어에는 한 번만 기록합니다. `DialogueText`만 고빈도 타이핑 필드를 구독하고 앱 본체는 화자·채널·카메라처럼 문장 단위로 바뀌는 필드만 얕게 구독하므로 캐릭터/스티커 계산이 글자마다 반복되지 않습니다.
- 클릭 즉시 완성(`clickToInstant`)과 `say.autoAdvance` 동작은 기존과 동일합니다.
- `prefers-reduced-motion`에서는 마지막 입력 글자 애니메이션을 제거하고 타이핑 시간 규칙만 유지합니다.

### 9-4-1) `say.channel` 대사 표시 채널

`say.channel`은 인물 대사와 화자 없는 문장을 화면 의미에 따라 분리합니다.

```yaml
- say:
    channel: record
    text: "왕실 기록에는 그날의 결정이 한 줄로 남았다."
- say:
    channel: system
    text: "새로운 증언을 확보했습니다."
```
...
허용값:
- `dialogue`: 인물 대사. 화자명이 있는 일반 대화창
- `narration`: 시간·장소·관찰 서술
- `record`: 문서·역사·판결처럼 작품 안에 남는 기록
- `system`: 선택 결과와 엔진 안내
- `thought`: 주인공의 짧은 속마음. 어두운 고대비 표면과 속마음 표식
- `action`: 주인공이 느끼거나 직접 하는 행동. 화자 없는 어두운 고대비 표면

실행 규칙:
- 생략 시 `char`가 있으면 `dialogue`, 없으면 `narration`입니다.
- 채널은 `delivery`, `<speed>`, `wait`, `unskippable`, `autoAdvance`와 독립적으로 함께 사용할 수 있습니다.
- 현재 채널은 다이얼로그 상태와 스토리 로그에 함께 저장합니다. 구형 저장 데이터에 채널이 없으면 기존 화자/서술 표시로 복구합니다.
- `record/system`은 채널 라벨과 전용 테두리·배경을 사용하며, 모션 감소 설정과 무관하게 의미 구분을 유지합니다.
- `narration`은 대화창 위의 중복 `서술` 라벨을 표시하지 않습니다. `narration/record/system` 비대화 채널은 배경 이미지의 밝기를 런타임에 추정하지 않고 템플릿별 고대비 전용 배경을 사용합니다. `paper-stage`는 세 채널 모두 완전 불투명한 종이색 바탕을 적용하며, 스토리 로그의 채널 구분은 그대로 보존합니다.

### 주인공 시점과 직접 행동 (2026-09-10)

`thought/action`은 타이핑·기록·저장 기능을 그대로 사용합니다. `char`를 지정하면 이름과 카메라 대상은 유지하지만, 실제 발화자 `speakerId`를 비워 말하는 캐릭터의 확대 강조를 적용하지 않습니다. 주인공의 눈으로 주변을 보는 장면은 `char`를 생략하고 `with: []`, `camera: wide`를 지정합니다. 화자 없는 `choice/input`에서도 명시한 `with: []`가 적용됩니다.
...
- say:
    ...
    with: []
    camera: wide
    text: "문밖의 발소리가 멎었다. 나를 기다리는 건가?"
    continueLabel: "문틈에 귀를 댄다"
- say:
    channel: action
    with: []
    text: "뺨에 나무의 찬 기운이 닿는다. 바깥에서 고리가 걸린다."
```

`continueLabel`은 1~60자의 행동 버튼입니다. 타이핑이 끝나고 입력 잠금이 풀린 뒤 버튼 클릭 또는 버튼에 초점을 둔 Enter/Space로 진행합니다. 배경 클릭은 타이핑을 완성할 수 있지만 행동을 대신 실행하지 않습니다. 플레이어 AUTO는 이 지점에서 기다리고 다음 일반 대사부터 다시 진행합니다. 작성자 `autoAdvance`와 함께 지정하면 파싱 오류입니다. 메뉴·가려진 화면·로딩 중에는 행동도 진행되지 않습니다. 자동 높이는 210px이며 명시적 `promptHeight`가 우선합니다.

장소를 옮길 때는 이동 이유를 먼저 보여 주고 행동 버튼 다음에 `bg/goto`를 둡니다. 매 문장에 버튼을 붙이지 말고 문을 열기, 위험한 곳으로 들어가기, 다음 장의 목적지를 결정하기처럼 의도가 필요한 순간에 사용합니다. `action`에는 감각과 관찰, `thought`에는 아직 모르는 것과 망설임을 두며 조사 전에 정답을 설명하지 않습니다.

### 화면에서 직접 살펴보기: `choice.presentation: explore`

```yaml
# base.yaml의 state에 read_letter: false, read_seal: false 선언
- choice:
    key: inspect_desk
    presentation: explore
    prompt: "종이와 인장. 어느 쪽부터 볼까?"
    with: []
    camera: wide
    options:
      - text: "접힌 편지"
      ...
      - text: "살펴본 흔적을 대조한다"
        when:
          all:
            - { var: read_letter, op: eq, value: true }
            - { var: read_seal, op: eq, value: true }
        goto: deduction
```

- `at.x/y`는 0~100이며 HUD와 대화창을 제외한 **조사 영역**의 비율입니다. 배경 이미지 원본의 픽셀 좌표가 아닙니다. `at`가 있는 선택은 화면 버튼, 없는 선택은 대화창 안의 돌아가기/대조하기 버튼으로 표시됩니다. 좌표 없는 기존 선택은 그대로 목록형입니다.
- `at`는 `presentation: explore`에서만 허용합니다. 작성된 옵션 중 하나 이상은 좌표가 있어야 합니다. `when`으로 모든 선택을 숨기지 않도록 돌아가기나 다음 행동을 남겨 둡니다.
- 버튼은 화면 안으로 제한하고 44px 이상의 터치 영역과 안쪽 포커스 테두리를 유지합니다. 여러 버튼의 상호 겹침까지 자동 배치하지는 않으므로 짧은 이름과 충분히 떨어진 좌표를 사용하고 모바일 세로·가로에서 확인합니다. 권장 대상 수는 한 화면에 두 개입니다.
- 관찰 장면은 `say`로 물리적 사실을 보여 준 뒤 `set`으로 확인 플래그를 기록하고 같은 조사 선택으로 돌아옵니다. 두 물건을 모두 본 뒤 `when`으로 추론을 엽니다. 반복해서 볼 수 있는 장면에는 `add` 보상을 두지 않습니다.
- 조사 상태와 선택 이력은 기존 `routeVars/routeHistory`로 저장합니다. 새 저장 형식이나 별도 인벤토리는 없습니다. 구형 저장에 없는 플래그는 `base.yaml.state`의 기본값으로 채웁니다. 덕만 기존 장면 ID·액션 개수·게임 버전은 유지하며, 첫 조사를 지난 저장에서는 1장 다시 시작으로 새 도입부를 볼 수 있습니다.
- 덕만은 첫 방과 네 증거상에 이 흐름을 사용하고, 코난 초반은 관찰·속마음·밤의 행동 버튼을 사용합니다. 다른 작품은 새 속성을 작성하지 않아도 기존 동작을 유지합니다.

### 9-4-2) `say.when` 조건부 대사

이전에 확보한 단서, 관계, 정책에 따라 같은 scene 안에서 다른 반응을 재생하려면 `say.when`을 사용합니다.

```yaml
- say:
    when: { var: clue_note, op: eq, value: true }
    char: 코난.think
    text: "아까 찾은 메모와 이 시간이 맞아."
```

실행 규칙:
- 조건 문법과 검증은 `branch.cases[].when`, `choice.options[].when`과 동일합니다. `eq/ne/gt/gte/lt/lte/in`, `all/any/not`을 조합하고 상태값과 인벤토리 ID를 조회할 수 있습니다.
- 조건을 만족하지 않으면 해당 action은 대사창·입력 대기·스토리 기록 없이 즉시 건너뜁니다. 화자, 표정, `with`, 카메라도 적용하지 않습니다.
- 저장 커서를 복원할 때도 복원된 상태·인벤토리로 조건을 다시 평가해, 실제로 재생되지 않은 조건부 대사가 화면 상태에 영향을 주지 않게 합니다.
- 선언되지 않은 상태 키나 인벤토리 ID는 `say.when`의 위치가 포함된 파싱 오류입니다.
...
### 9-7) `say.unskippable` 타이핑 완료 전 진행 금지

반드시 끝까지 출력해야 하는 문장은 `unskippable: true`로 선언합니다.
...
실행 규칙:
- 타이핑 중 클릭/`Enter`/`Space` 입력을 무시하며 `clickToInstant`의 즉시 완성도 허용하지 않습니다.
- 타이핑이 끝나면 잠금은 자동 해제됩니다. 따라서 문장 완료 후의 일반 진행 입력까지 막는 영구 잠금은 아닙니다.
- `wait`와 함께 선언하면 타이핑 완료와 지정 시간 경과 조건을 모두 만족해야 수동 진행할 수 있습니다.
- `autoAdvance` 시간이 타이핑보다 먼저 끝나면, 남은 글자를 생략하지 않고 타이핑 완료 직후 다음 액션을 실행합니다.
- 생략하면 `false`로 동작하므로 기존 게임의 대사 진행 방식은 바뀌지 않습니다.

### 9-8) `say.autoAdvance` 대사 자동 진행
...
- 시간이 끝나면 `timeoutOptionIndex`의 옵션을 선택합니다. 생략 시 첫 옵션을 사용하며, 옵션 범위를 벗어난 인덱스는 실행 중 보정하지 않고 파싱 오류로 보고합니다. 지정한 옵션이 `when` 조건 때문에 숨겨졌다면 현재 보이는 첫 옵션으로 안전하게 대체합니다.
- 만료 선택은 자동 흐름이 멈추지 않도록 `forgiveOnce`를 건너뜁니다.
- 플레이어가 먼저 선택하면 예약 타이머를 취소합니다.

### 9-9-1) `choice.options[].when` 조건부 선택지

```yaml
- choice:
    prompt: "천문 기록을 어떻게 다룰까?"
    options:
      - text: "일식 계산표를 공개한다"
        when: { var: clue_eclipse_chart, op: eq, value: true }
        goto: reveal_chart
      - text: "기록을 더 확인한다"
        goto: inspect_archive
```

실행 규칙:
- 조건 문법과 검증은 `branch.cases[].when`과 동일하며 상태값과 인벤토리 ID를 모두 조회할 수 있습니다.
- 조건을 만족하지 않은 옵션은 비활성 버튼으로 노출하지 않고 선택 목록에서 제외합니다.
- 보이는 옵션은 YAML 작성 순서를 유지합니다. 선택 기록은 옵션 텍스트로 저장되므로 선택 복구와 저장 재생도 기존 방식으로 동작합니다.
- 모든 옵션이 숨겨지면 빈 선택 게이트를 열지 않고 `sceneId:actionIndex`가 포함된 런타임 오류를 표시합니다.
    ...
    options:
      ...
      - text: "붉은 전선"
        gameOver:
          ...
          recoverToChoice: cut_the_wire
```

- `gameOver.title`, `gameOver.message`는 선택이며 생략하면 엔진 기본 문구를 사용합니다.
- `gameOver.recoverToChoice`에는 죽음의 실제 원인이 된 앞선 `choice.key`를 적습니다. 엔진은 선택 키별 체크포인트 이력을 챕터 전환 뒤에도 유지하고 해당 선택 직전의 상태·인벤토리·기록을 복원합니다. 키가 없거나 체크포인트를 찾지 못하면 가장 최근 선택을 사용합니다.
- 한 선택지에 `goto`와 `gameOver`를 동시에 선언할 수 없습니다.
- 선택지의 `set/add`는 게임오버 전에 반영되지만 게임오버 시점에는 자동저장을 덮어쓰지 않습니다.
- 모든 확정 선택은 `set/add`와 선택 기록을 반영하기 전에 최신 `:choice-recovery`와 키별 `:choice-recovery-trail` 복구점을 만듭니다.
...
- 게임오버 패널은 복구 메타에 실제 선택 문구가 있으면 제목·결과 설명 아래의 `결과를 만든 선택` 카드에 그 문구를 표시합니다. 이어서 선택 복구점을 우선 쓰는 `죽음의 원인 선택으로`, `수동 저장으로`, 자동으로 남긴 `챕터 처음으로`, 백업 파일 불러오기를 제공합니다.
- 선택 복구점은 선택 키·문구·인덱스도 별도 기록하고 실제 `gameOver`가 발생했을 때만 실패 이력으로 확정합니다. 복귀해 같은 선택 게이트에 도달하면 해당 항목을 저채도 회색 카드와 `원인 선택 · GAME OVER` 배지로 표시하되 비활성화하지 않습니다. 현재 키보드 포커스는 밝은 테두리로 구분하고 가능한 경우 다른 선택지에서 시작합니다. 직접 `gameOver` 옵션과 `goto` 뒤 독립 게임오버 모두 같은 규칙을 사용합니다.
...
장편 역사극의 채널·조건부 대사/선택·누적 상태·다중 실패/엔딩 샘플은 `public/game-list/deokman/`을 참고합니다. 12개 챕터에서 `dialogue/narration/record/thought/action`, `say.when`, `choice.options[].when`, 외부 챕터 `goto`, 누적 상태 `branch`, 장면형 `gameOver`, 다중 `ending`을 함께 사용합니다.
...
- `public/game-list/deokman/CHARACTER_VOICE.ko.md`
- 샘플 YAML: `public/game-list/conan/*.yaml`
- 역사극 분기 샘플: `public/game-list/deokman/*.yaml`
- DSL 축약 샘플: `sample.yaml`

### 비주얼노벨 연출 마이그레이션 (2026-09-08)

`CharacterPresence`는 디코드/모델 준비와 입퇴장, `StageImageCharacter`와 `portraitTransition`은 표정 합성, `retainedCast`는 교체 배우의 퇴장 수명을 담당합니다. `characterPresentation.css`는 월드 transform과 독립된 로컬 모션만 정의합니다. 기존 `char.enter`와 `camera` 문법은 그대로 호환됩니다.

모바일 버튼 포커스는 `outline-offset: -4px`로 컨트롤 안에 그립니다. 타이틀 버튼은 `-5px`이며 키보드 초점 표시를 없애지 않습니다. 모바일 세로 타이틀은 제목·시작/이어하기·고지 순서의 흐름 배치를 사용하고, 남는 높이를 버튼 위에 배분합니다. `safe-area-inset-*`와 스크롤 안쪽 여백을 확보하며 고지 유무에 따른 고정 `bottom` 값은 적용하지 않습니다.

기존 작품의 대사마다 반복된 `camera: close/reaction`은 대화 시작의 `medium`으로 정리하고, 중요한 순간에만 명시 대상 컷을 남깁니다. `with`로 노출 인원이 바뀌면 기본 자동 구도가 적용됩니다. 예전처럼 빈자리를 유지할 장면에만 `layout: fixed`를 추가하고, 나레이션에서 확대를 풀려면 `camera: medium`을 지정합니다. 예전 전신 구도가 필요한 게임은 첫 장면에 `camera: wide`를 명시합니다. 덕만의 2026-09-09 콘텐츠 개편은 기존 scene/action 주소를 보존하고 새 조사 장면을 추가합니다. 인물 기본 배치는 `base.yaml`의 `placement: prompt-top`으로 맞춥니다. 이전 저장도 열리며, 이미 지나간 새 수집 구간은 해당 장을 다시 시작해 볼 수 있습니다. 코난/코난 데모/Live2D는 공통 화자·모바일 UI 개선을 자동 적용받습니다.

## 14) 문서 변경 로그

- 2026-09-11: 상반신 크롭 확대를 높이 애니메이션에서 transform 보간으로 바꿔 반복 레이아웃·래스터 작업을 줄였습니다. 타이핑 레이아웃은 크기가 지정된 대화창 내부에 격리하고, 마지막 글자 추출은 Segmenter 재사용과 위치 조회로 변경했습니다. 자동 인물 배치·그림 DOM·표정 디졸브·모션 감소 동작을 유지하며 기존 게임에 문법 변경 없이 적용합니다.

- 2026-09-11: `scene.layout: auto|fixed`를 추가하고 기본을 실제 노출 인원에 맞춘 자동 재정렬로 바꿨습니다. `char.position` 생략/`auto`는 같은 인물의 슬롯을 유지하고 빈 슬롯을 배정하며 저장 복원에도 동일하게 적용합니다. 빈자리 유지가 필요한 장면만 `fixed`를 지정합니다. 모바일 CSS의 중복된 고정 앵커를 제거해 배치·간격·카메라가 동일한 좌표 계산을 사용합니다. 정적 `framings.cropBottom`과 이미지 재사용을 통한 전신↔상반신 전환을 추가했습니다. 덕만 10명은 대화용 portrait(성인 하단 40%, 어린 덕만 32% 크롭)와 full 프리셋을 갖추고 PC·모바일 간격을 조정했습니다. 원화·스토리·장면 액션 주소·저장 형식은 유지합니다.

- 2026-09-11: 대화창 높이·카메라·등장 인물 변경 때 기존 스티커를 숨겼다가 재노출하던 동작을 제거했습니다. 실제 스티커를 임시로 옮기던 크기 측정도 별도 요소로 분리해 위치 보간이 끊기지 않게 했습니다. 같은 ID의 재선언·그림 교체는 DOM 수명을 유지하고, 디코딩 후 크로스디졸브와 부분 배치 상속을 적용합니다. 완전히 같은 재선언의 입력 잠금도 반복하지 않습니다. 캐릭터 슬롯 교환 시 렌더 배열의 DOM 순서를 고정해 이동·표정 전환이 끊기지 않게 했고, prompt-top의 마지막 교체 배우도 퇴장을 마치도록 유지합니다. 덕만 족보 조사 구간의 212프레임 재현에서 숨김 프레임이 55→0으로 줄었습니다. DSL 필드·게임 데이터·저장 형식 변경 없이 기존 게임에 적용됩니다.

- 2026-09-10: `say.channel: thought/action`, 플레이어 확인이 필요한 `say.continueLabel`, `choice.presentation: explore`와 `options[].at`를 추가했습니다. 실제 발화와 속마음의 화자 강조를 분리하고 화자 없는 choice/input의 `with: []` 적용을 수정했습니다. 행동 시 AUTO 대기, 화면 조사와 기존 조건·저장·기록을 연결했습니다. 덕만 12장의 주관적 감각·이동·판단과 첫 방/네 증거 조사를 개편하고 코난 초반 3장의 관찰을 주인공 시점으로 바꿨습니다. 기존 저장 주소와 엔딩 분기를 유지하며 샘플 YAML에 작성 예를 추가했습니다.

- 2026-09-09: 화자의 밝기/외곽 그림자를 제거하고 340ms의 작은 확대와 앞쪽 합성으로 바꿨습니다. 빈 액션 구간에서 같은 화자의 강조가 풀리지 않으며, 모션 감소 환경에서는 크기를 고정합니다. 인게임 버튼·입력·탭의 포커스는 안쪽 2px 테두리로 표시해 스크롤 경계에서 잘리지 않고, 시작 화면의 모바일 세로 메뉴는 모든 높이에서 흐름 배치와 안전영역을 사용합니다. 고지문은 실제 공간을 차지하며 짧은 화면은 메뉴를 스크롤합니다. 덕만은 32개 즉사 분기를 통치/조사의 후속 결과로 개편하고 증거 비교 4곳·아이템 2종을 추가했습니다. 11개 결말의 도달 경로를 검사합니다. 기존 장면의 액션 개수·선택 키·저장 식별자를 유지했고 DSL/저장 형식 변경은 없습니다. 새 조사 장면과 플래그는 기존 `sticker`, `choice`, `set`, `branch`로 작성했습니다.

- 2026-09-08: 코난·덕만을 먼저 작품 단위로 보강했습니다. 코난은 장소 전환·대상 지정 증언/자백 컷·선택의 중간 구도·차분한 코고로 원화·엔딩 세 가지의 배경/음악/후일담을 적용했습니다. 덕만은 외부 샘플 없는 24초 루프 세 곡을 타이틀·12장·11개 결말에 배치하고 3장 칠숙의 협공을 공격 시퀀스로 바꿨습니다. 이어 코난 데모의 화면 밖 화자를 복귀시키고 `prompt-top`으로 정렬했으며, 렌 데모에 표정/시선/카메라 체험과 작별을 추가했습니다. 기존 장면의 액션 개수·선택 키·분기 결과·작성 버전을 유지하므로 본편 저장을 초기화하지 않습니다. 새 DSL 문법이나 엔진의 게임별 예외는 없습니다. 음원 재생성: `python3 scripts/generate-deokman-score.py`.

- 2026-09-08: PC와 짧은 가로 화면에서 `prompt-top` 인물의 머리가 상단으로 벗어나던 문제를 수정했습니다. 대화창·HUD를 제외한 높이와 최대 카메라 배율로 이미지/Live2D 크기를 제한하며 쇼트 간 확대와 자동 선택지 기준선을 유지합니다. 모바일 세로 프레임을 전체 뷰포트 높이로 바꿔 대화창 아래 빈 공간을 없애고 HUD 안전영역을 반영했습니다. DSL·개별 게임·저장 형식 변경은 없습니다.

- 2026-09-08: 프리로드와 렌더링의 이미지 리소스를 통합했습니다. 디코딩된 DOM 이미지 재사용, 동일 출처 바이트 공유, 동시 요청 병합, 예산에 따른 미사용 리소스 정리, 실패 재시도·ZIP 무효화를 적용했습니다. 다음 챕터는 바이트만 예열합니다. 빠른 장면 준비 중 첫 rAF 타임스탬프가 BGM 페이드 시작보다 앞설 때 음량이 음수가 되던 예외도 보정했습니다. 4개 이미지의 반복 전환 재현에서 디코딩 호출 22→4회, 이미지 요소 21→4개를 확인했습니다. DSL·게임 데이터·저장 형식 변경은 없습니다.

- 2026-09-08: 인게임 HUD·대화/선택/입력·기록·소지품 상세·저장/설정·게임오버/엔딩을 공통 인터페이스로 개편했습니다. PC 탐색 메뉴와 모바일 탭, SVG 아이콘, 키보드 탐색/초점 가두기·복귀를 적용했습니다. 메뉴를 열면 이야기 시계도 정지하며 선택/입력은 자동 높이를 사용하되 작성자 `promptHeight`와 캐릭터의 읽기 기준선을 보존합니다. 기존 작품과 세이브는 그대로 적용하며 DSL 문법 변경은 없습니다.

- 2026-09-08: 캐릭터 배치·입퇴장·표정 레이어를 분리했습니다. 디코드 후 360ms 등장, 180ms 표정 크로스디졸브와 최신 요청 병합, 260ms 제자리 퇴장 및 슬롯 교체 보존을 적용했습니다. 화자 호흡 확대를 밝기 전환으로 교체하고 `with`의 불필요한 재배치·시선 반전, 내레이션에서의 자동 줌 해제를 제거했습니다. 초기 카메라는 `medium`, 확대 컷은 완만한 배율을 사용합니다. 타이틀 움직임 버튼을 제거하고 자동 재생으로 고정했습니다. 덕만 전 12개 챕터의 반복 확대/반응 컷 243곳과 등장 4곳을 마이그레이션했으며 이야기·분기·장면 액션 인덱스와 저장 형식은 보존했습니다. DSL 문법 변경은 없습니다.
...
- 2026-09-07: 배경 객체형 전환(`dissolve/fade/wipeLeft/wipeRight/cut`, 디코드·완료 대기)과 `attack`의 접근·타격·반응 시퀀스, 인물 모션·방향성 효과·자체 제작 효과음을 추가했습니다. 게임오버와 엔딩은 플레이어가 읽고 넘기는 결과 장면을 먼저 보여 주고 크레딧의 강제 자동 스크롤을 제거했습니다. 엔딩별 배경·음악·후일담·분위기, 모션 감소·SFX 음소거, 연출 중 저장 제한과 복구 시 타이머/오디오 정리도 적용했습니다. 덕만 V10.4는 12장 배경 전환과 네 곳의 칠숙 공격(검·활 전용 장면 컷 포함), 11개 엔딩 후일담을 사용합니다. 액션 커서 변경으로 작성 버전을 올려 이전 버전 저장과 구분하며 엔딩 수집 기록은 유지합니다.

- 2026-08-31: 정적 캐릭터의 새 소스를 같은 `<img>`에 즉시 적용해 디코딩 중 빈 프레임이 생기던 경합을 수정했습니다. 첫 DOM 이미지는 디코드 전까지 숨기고, 표정·소스 교체는 직전 디코딩 이미지를 유지한 채 최신 요청만 다음 페인트에 커밋합니다. 3→2·2→1 퇴장에서는 사라지는 배우의 마지막 앵커·시선을 스냅샷하고 생존 배우 재배치를 같은 페인트에 시작해 `퇴장 대기 → 뒤늦은 이동`의 두 단계 멈춤을 제거했습니다. DSL 문법 변경은 없습니다.
- 2026-08-31: `close` 확대에서 캐릭터 실루엣이 중앙 `stage-content-frame`과 캐릭터 합성 레이어의 중첩된 paint/overflow 경계에 잘리던 문제를 수정했습니다. 캐릭터의 기기별 배율과 프레임 기준 좌표는 유지하면서 중앙 프레임 바깥 풀블리드 영역까지 렌더링하고, 실제 화면 끝의 `.effect-viewport`만 최종 클리핑합니다. DSL 문법 변경은 없습니다.
- 2026-08-31: 다인 `camera: close`에서 화자가 바뀔 때 주변 인물이 매번 퇴장·재등장하던 동작을 제거했습니다. 축약형 `close`는 기존 `with` 그룹을 유지한 채 수평 이동 없이 확대하고, 명시적 대상이 있는 `close`는 기본 `pan`을 사용합니다. 단순 화자 변경을 캐릭터 모션·스티커 회피 재계산 키에서 제외하고 화면 안의 Live2D 주변 인물 렌더러를 계속 활성화했습니다. 캐릭터 퇴장은 실제 `with` 제외에서만 발생하며 DSL 문법 변경은 없습니다.
- 2026-08-25: 자동재생의 작성 전환은 유지하면서 수동 연타에만 적응형 `catch-up` 템포를 적용했습니다. 620ms 안의 연속 클릭/키 입력은 700ms 동안 새 배경·카메라·캐릭터·스티커 전환의 지연을 제거하고 주요 모션을 최대 140ms, 캐릭터 퇴장 페이드를 최대 90ms로 단축합니다. 이미 진행 중인 전환은 배경·카메라·캐릭터·스티커별 모션 키가 시작될 때의 템포를 고정해 속도 변경 순간의 위치 점프를 막고, 짧은 연출 잠금 중 연타는 하나만 최대 1.6초 보류해 해제 뒤 안전한 대사 상태에서 실행합니다. 선택·입력·컷신·게임오버와 오래된 입력은 보류 요청을 소비하지 않습니다. DSL 문법 변경은 없습니다.
- 2026-08-24: 캐릭터 배치 액션에 `char.enter`를 추가했습니다. `none/fadeIn/wipeLeft/scaleIn/popIn/slideUp/slideDown/slideLeft/slideRight/wipeCenterX/wipeCenterY/blurIn/rotateIn` 효과와 `layout: cut|push`, `duration/easing/delay`를 장면별로 지정할 수 있습니다. 기본 `fadeIn + push + 240ms`는 기존 작품을 유지하고, `cut`은 팟 등장처럼 기존 배우 자리도 즉시 확정하며, `push`는 기존 배우가 새 인물에게 자연스럽게 자리를 내줍니다. 덕만 V10.2는 320개 인물 발화와 36개 선택 프롬프트를 전수 교정해 설명 낭독을 짧은 질문·반응·명령으로 바꾸고, 인물별 보이스 가이드와 발화 길이·답안형 문구 회귀 검사를 추가했습니다. `README.md`, `sample.yaml`, 덕만 게임 바이블을 함께 갱신했습니다.
- 2026-08-24: 덕만 V10.1에서 앞 선택은 살아남았지만 뒤 위기의 생존 옵션이 모두 조건으로 닫히던 막다른 분기를 제거했습니다. 36개 선택마다 생존 정답을 정확히 하나로 고정하고 기존 복수 생존 8개를 선택 원인에 맞는 장면형 죽음으로 완결해 112개 보기 중 76개가 실패 장면으로 이어집니다. 구버전 진행이 바뀐 장면 커서로 재개되지 않도록 게임 제목과 작성 버전이 맞는 저장만 `이어하기`에 사용하며, URL YAML 요청은 HTTP 캐시를 우회합니다. DSL 문법 변경은 없습니다.
- 2026-08-24: 덕만 첫 모란 선택의 `medium -> close` 전환 중 이미 노출된 스티커와 어린 덕만이 약 0.5초 겹친 뒤 분리되던 경합을 수정했습니다. 카메라·등장 인물 변경이 시작되면 이전 안전 배치를 같은 페인트에서 미확정 처리해 숨기고, 안정된 캐릭터 경계에서 충돌 없는 좌표를 확정한 뒤 160ms 페이드로 다시 노출합니다. 퇴장 중인 스티커는 마지막 배치와 렌더 인스턴스를 그대로 유지하며 DSL 문법 변경은 없습니다.
- 2026-08-24: 덕만 완결판을 V10.0으로 개편했습니다. 핵심 생존 경로의 대사를 짧은 질문·망설임·반박·감정 반응 중심으로 다시 쓰고, 플레이어가 대화 속 단서를 읽어 덕만을 선덕왕으로 만들어야 하는 구조로 복원했습니다. 36개 선택·112개 보기 중 68개가 최소 네 번의 대사·서술·기록을 거치는 장면형 죽음으로 이어지며, 독살·모함과 처형·매복·화재·압사·반란 등 각 오답의 정치적 인과를 별도 장면으로 회수합니다. 관찰·증언·조건부 표결 같은 조사·관계 선택은 복수 생존을 허용하고 모든 선택에는 최소 한 개의 생존 경로를 보장합니다. DSL 문법 변경은 없습니다.
- 2026-08-24: 비주얼노벨 플레이 흐름을 기준으로 플레이어 AUTO·텍스트 배속·AUTO 간격·화면 효과 강도·BGM/SFX 음량을 시스템 탭과 대화창 퀵 컨트롤에 추가했습니다. AUTO는 타이핑 완료 뒤에만 진행하고 선택/입력·작성자 자동대사·설정창·대화창 숨김에서 정지합니다. 대화창 바깥 컨트롤이 부모 clipping에 가려져 선택지 화면에서 보이지 않던 기존 문제도 함께 수정했습니다. 화면 효과에는 공통 bloom/비네트 envelope와 `full/reduced/minimal` 완급을 적용하고 호흡 글로우를 보강했습니다. 다음 챕터 예열을 idle 작업으로 옮기고 BGM fade를 rAF로 전환했으며, 설정창 뒤 Live2D/CSS 모션을 멈춥니다. 진행/백업과 게임별 설정을 `schemaVersion: 2`로 올려 무버전/v1 데이터를 자동 마이그레이션하고 미래 버전은 덮어쓰지 않습니다. DSL 문법 변경은 없습니다.
- 2026-08-24: 캐릭터 충돌 회피로 이미 노출된 스티커의 안전 위치가 바뀔 때 좌표·크기·배율을 280ms로 보간하도록 개선했습니다. 캐릭터 노출 집합과 앙상블 레이아웃은 등장 전에 같은 페인트에 확정하고 등장·재등장·퇴장 중 위치 전환을 끄며, 다인 `close`는 비대상 인물의 제자리 페이드를 마친 뒤 카메라를 이동합니다. 기본 `push`는 확대만 보간하고 명시적 `pan`만 횡이동하도록 분리해 좌우 화자가 화면 밖에서 날아오거나 멀리 사라져 보이던 문제를 수정했습니다. DSL 문법 변경은 없습니다.
- 2026-08-24: 화자 호흡 모션의 변형 기준을 하단 중앙(`50% 100%`)으로 명시하고 좌우 반전 `transform`과 호흡용 `translate/scale`을 분리했습니다. 화자 변경 시 직전 화자는 진행 중인 호흡을 중립 프레임까지 마친 뒤 정지해 이미지가 중간 변형값에서 꺾이듯 튀지 않으며, 호흡 정점에는 최대 `3%` 밝기와 옅은 웜 글로우를 추가했습니다. DSL 문법 변경은 없습니다.
- 2026-08-24: `이어하기`가 현재 배경을 커서 재생에만 의존해 복잡한 분기·레거시 진행에서 빈 무대로 복원될 수 있던 경로를 보강했습니다. 새 자동·수동 저장과 백업에 현재 배경의 안정적인 에셋 ID를 함께 기록하고, 커서 연출 재생 후 해당 배경을 최종 적용합니다. 배경 필드가 없는 기존 저장도 같은 챕터의 스토리 로그와 현재 scene에서 마지막 `bg`를 유추해 복구합니다. DSL 문법 변경은 없습니다.
- 2026-08-21: 노출된 스티커가 처음 계산한 좌표를 무조건 고정해, 덕만 첫 선택지의 중앙 클로즈업에서 모란 그림이 어린 덕만 얼굴을 가리던 문제를 수정했습니다. 카메라·등장 인물 변경 후 경계가 안정되면 기존 좌표의 충돌 여부를 다시 확인하고, 안전하면 유지하며 새 충돌이 있을 때만 가장 가까운 빈 공간으로 보정합니다. DSL 문법 변경은 없습니다.
- 2026-08-21: 실제 대사 화자의 기본 호흡 주기를 `3.2s`에서 `2.2s`로 단축해 미세 모션을 더 쉽게 인지하도록 조정했습니다. 프롬프트 하단의 `다음`·`완료`·`입력 대기`·`선택 대기` 상태 전용 행과 10px 간격도 제거해 PC·모바일에서 26px을 본문·입력·선택 영역으로 환원했습니다. DSL 문법 변경은 없습니다.
- 2026-08-20: 다인 대화에서 `speakerOrder` 순위에 따라 비화자 불투명도를 낮추던 화자 강조를 제거하고, 실제 대사 화자에게만 `3.2s` 미세 상하·스케일 호흡 모션을 기본 적용했습니다. 카메라 포커스와 발화 상태를 분리해 `reaction` 대상과 화자가 달라도 올바른 인물이 움직이며, `prefers-reduced-motion`에서는 모션을 끕니다. 다인 `close`와 `with`의 퇴장 페이드는 쇼트 전환 동작으로 유지합니다. DSL 문법 변경은 없습니다.
- 2026-08-20: 새 배경의 디코드 동기화 크로스페이드와 스티커 DOM 유지 동작을 보존하면서, 배경·카메라·캐릭터 합성 레이어의 상시 `will-change`를 실제 전환 중에만 유지하도록 통합했습니다. 대사 타이핑은 `requestAnimationFrame` 기반 프레임당 1회 갱신으로 합치고 고빈도 본문 구독을 앱 본체에서 분리해 타이핑 중 무대 재계산을 제거했습니다. Live2D는 준비 완료 후 화면 밖·백그라운드 renderer를 정지하고, DPR 2 상한과 80ms 리사이즈 안정화를 적용해 모바일 전환 중 WebGL 픽셀 처리·버퍼 재할당을 줄였습니다. DSL 문법 변경은 없습니다.
- 2026-08-20: 플레이 중 `bg` 교체가 같은 `<img>`의 `src`만 즉시 바꿔 디코드가 늦으면 이전 프레임이 사라지던 문제를 수정했습니다. 새 배경은 숨김 레이어에서 로드·디코드한 뒤 직전 배경 위로 360ms 크로스페이드하며, 빠른 연속 요청의 오래된 완료와 실패 에셋은 현재 화면을 덮지 않습니다. `clearSticker.leave`도 렌더 키를 새로 발급해 안전 배치를 잃던 동작을 제거하고 같은 스티커 DOM·확정 좌표에서 퇴장 이펙트를 끝내도록 변경했습니다. DSL 문법 변경은 없습니다.
- 2026-08-20: `assets.characters.<id>.placement`를 추가했습니다. 각 캐릭터 원화는 게임 전역 설정과 무관하게 기본 무대 하단 기준선(`stage-bottom`, 생략 시 기본값) 또는 대사/선택 프롬프트 상단 기준선(`prompt-top`)을 선택하며, 계층 YAML 병합과 상대 경로 정규화 뒤에도 선언값을 보존합니다. `sample.yaml`에 반신형 인물 선언 예시를 추가했습니다.
- 2026-08-20: `say.when`을 추가해 상태·인벤토리 조건을 만족하는 대사만 재생하도록 했습니다. 거짓인 대사는 대화창·로그·화자·카메라 상태를 남기지 않고 건너뛰며 저장 화면 복원도 같은 조건을 평가합니다. 현재 준비된 번호 챕터 안의 `goto`와 URL/ZIP 저장 재개는 기존 시퀀스·진행도·다음 챕터 프리로드를 보존합니다. 게임오버 패널은 복구 메타가 있을 때 실패를 만든 실제 선택 문구를 `결과를 만든 선택` 카드로 먼저 보여 줍니다. `sample.yaml`에 조건부 대사 예시를 추가했습니다.
- 2026-08-20: 덕만 V9.6은 V9.5의 과반 사망형 선택 구조를 전략형 위기로 다시 설계했습니다. 11개 위기마다 최소 두 생존 전략과 하나의 극화된 실패를 두고, 일반 정치 선택도 여러 실행 가능한 대가를 남깁니다. 36개 선택·장면형 게임오버 13개·누적 엔딩 11개를 유지하며, `say.when`과 조건부 선택으로 앞 장의 관찰·관계·정책을 뒤 장의 대사와 선택에 회수합니다.
- 2026-08-20: Conan V10.8은 인물 방문 수와 증거 완성도를 분리해 네 탐문 뒤에도 놓친 단서를 라인별 후속 대면으로 회수할 수 있게 했습니다. 실제로 찾은 증거만 조건부로 회상하며, TRUE END는 두 번 난 뚜껑 소리의 미끼와 켄지가 들은 거래 거절까지 요구합니다. 장면 중 처음 말하는 인물을 명시적으로 무대에 배치하고, 찾기 전 단서를 확정적으로 말하던 문장도 제거했습니다.
- 2026-08-19: 덕만 V9.5에서 선택지 내부의 즉시 게임오버 16개를 모두 별도 실패 후일담 장면으로 전환했습니다. 선택 행동, 덕만의 독백 또는 상대 반응, 사망을 확정하는 기록을 최소 네 단계로 보여 준 뒤 게임오버를 표시하며, 콘텐츠 회귀 테스트가 즉시 `gameOver` 옵션의 재도입과 설명 없는 실패 장면을 차단합니다. DSL 문법 변경은 없습니다.
- 2026-08-19: 진행 중 캐릭터 슬롯 이동이 끝나기 전에 스티커 충돌 회피 좌표가 잠겨 PC에서 아이템이 얼굴을 가리지만 같은 화면 새로고침 후에는 정상화되던 경합을 수정했습니다. 스티커는 정적 캐릭터 이미지 준비와 캐릭터 이동 최대 시간을 기다린 뒤, 48ms 간격의 연속 경계가 안정됐을 때만 최종 배치·노출합니다. DSL 문법 변경은 없습니다.
- 2026-08-19: 덕만 V9.4의 생존 위기와 일반 판단 게이트를 `생존 정답 1개 + 나머지 게임오버` 원칙으로 재조정해 총 28개 실패 판정을 구성했습니다. 복수 생존은 정책·관계 클루, 최종 엔딩, 1장 북문처럼 다음 장의 지연 사망 원인이 되는 명시적 예외에만 허용하며, 원인 선택 복구와 11번째 히든 엔딩은 유지합니다. DSL 문법 변경은 없습니다.
- 2026-08-19: `gameOver.recoverToChoice` DSL과 선택 키별 복구 이력을 추가했습니다. 죽음이 여러 선택·장면·챕터 뒤에 발생해도 마지막 선택이 아닌 실제 원인 선택 직전으로 돌아가며, 대상 키가 없으면 최근 선택으로 대체합니다. 게임오버 버튼과 실패 배지는 `죽음의 원인 선택` 의미로 정리했고 `sample.yaml`에 사용 예시를 추가했습니다. 덕만 V9.3은 위기별 생존·사망 비율을 균형화하고 1장 탈출 선택의 지연 결과 복구와 11번째 히든 엔딩을 적용했습니다.
- 2026-08-19: 모바일 Start Gate와 게임 실행 화면을 `100dvh` 고정 앱 셸로 전환하고, iOS 당겨 새로고침·바운스 및 Android 문서 스크롤을 차단했습니다. 실제 내부 스크롤은 경계 안에서 유지합니다. 모바일 선택지는 1열·58px 터치 카드·순번·눌림 피드백으로 바꾸고 선택 패널을 `64cqh`까지 확장했습니다. Start Gate 제목에는 테마 공통 고대비 표면과 긴 한글 줄바꿈을 적용해 덕만의 금색 제목을 포함한 밝은 키아트 위 글자 가독성을 보강했습니다. DSL 문법 변경은 없습니다.
- 2026-08-19: 덕만 V9.2에서 1~11장 고위험 선택의 과반 옵션을 실제 사망 분기로 바꾸고, 서로 다른 위험 행동이 같은 실패 장면으로 합류하도록 선택 그래프를 재설계했습니다. 조사·정치 선택과 12장 복수 엔딩은 비치명 장기 분기로 유지합니다. DSL 문법 변경은 없습니다.
- 2026-08-19: 덕만 완결판을 V9.1로 갱신했습니다. 어린 덕만 이후 하나의 성인 원화만 사용하던 구성을 `국경 유랑자 3표정 → 신분을 되찾은 공주 3표정 → 선덕왕 3표정`으로 분리하고, 3장의 시녀 잠입용 무장식 원화를 추가했습니다. 10개 신규 원화는 같은 얼굴 계보와 전신 체급을 유지한 RGBA PNG이며 2장 국경 장터, 3장 귀환 분기, 9장 즉위, 10~12장 통치 장면에서 시점에 맞게 전환됩니다. DSL 문법 변경은 없습니다.
...
- 2026-08-18: 정적 캐릭터의 원본 높이를 `58cqh`에서 `52cqh`로 낮춰 새 카메라 쇼트 비율을 유지하면서도 PC·모바일 `close`에서 머리가 상단을 넘지 않도록 조정했습니다. 스티커는 첫 페인트 전에 보이는 캐릭터 경계를 측정해 겹칠 때만 가장 가까운 빈 공간으로 이동하고, 카메라 전환·캐릭터 교체·프레임 리사이즈에도 자동 재배치합니다. 덕만 V8 아이템은 반응형 최대 폭으로 제한했으며, 어린 덕만 세 표정은 원본 인물 픽셀을 유지한 채 알파 외곽의 흰 매트만 걷어 낸 V7 투명 WebP로 교체했습니다. DSL 문법 변경은 없습니다.
- 2026-08-18: 캐릭터 카메라를 중첩 배율·대상별 `transform-origin` 방식에서 단일 절대 배율과 명시적 수평 팬으로 변경하고, PC `1.18/1.62/2.08/1.90`·모바일 `1.12/1.58/1.84/1.72` 쇼트 프로필로 배우 비중을 높였습니다. 다인 close의 퇴장 페이드와 카메라 이동은 72ms 뒤부터 겹치며, 슬롯 이동은 380ms, 첫 등장은 이동 없는 240ms 페이드로 정리했습니다. Start Gate는 화면 밖 버튼 비행·반복 질감 모션 대신 짧은 opacity 등장과 220ms 검정 크로스페이드를 사용합니다. PC·모바일 시스템 탭은 저장 카드만 스크롤하고 시작 화면 복귀 컨트롤을 하단에 고정하며, 내레이션의 중복 `서술` 라벨을 제거했습니다. 덕만 V8은 공간 제시 `wide` 뒤 첫 인물 서술에서 `medium`으로 복귀합니다. DSL 문법 변경은 없습니다.
- 2026-08-18: 구버전 8장 덕만 게임과 전용 문서·회귀 테스트를 제거하고, 당시 1장 수직 슬라이스에 필요한 배경·인물·아이템을 독립 패키지로 이관했습니다. 이후 완결판 승격으로 해당 자산은 `public/game-list/deokman/assets/`에서 이어 사용합니다.
- 2026-08-18: `narration/record/system` 채널이 그라디언트 토큰을 `color-mix()` 색상 인자로 사용해 전용 배경 선언이 무효가 되던 문제를 수정했습니다. 템플릿별 불투명 고대비 표면을 분리하고 `paper-stage`의 세 비대화 채널에 종이색 바탕을 적용해 밝고 어두운 장면 모두에서 본문 대비를 유지합니다. DSL 문법 변경은 없습니다.
- 2026-08-18: 선택 게이트가 4개 보기를 한 열로 쌓아 대화창 내부 스크롤을 만들던 레이아웃을 선택지 수 기반 반응형 그리드로 변경했습니다. 1개는 전체 폭, 2개는 2열, 3개는 `2+1`, 4개는 `2×2`로 표시하고 긴 문구의 줄바꿈과 동일 행 카드 높이를 안정화했습니다. 5개 이상은 기존 한 열 스크롤 동작을 유지하며 DSL 문법 변경은 없습니다.
- 2026-08-14: 게임오버 뒤 선택 또는 챕터 복구 시 실패를 만든 직전 선택을 확인할 수 있도록 선택 복구점에 선택 키·문구·인덱스와 실제 게임오버 도달 여부를 기록합니다. 같은 게이트에 돌아오면 해당 항목을 회색 저채도 카드와 `직전 선택 · GAME OVER` 배지로 표시하고 다른 항목에 기본 포커스를 두며, 과거 선택도 비활성화하지 않아 의도적으로 다시 선택할 수 있습니다. 직접 실패 옵션과 `goto` 후일담 뒤 독립 게임오버를 함께 지원하며 DSL 문법 변경은 없습니다.
- 2026-08-14: `with`에서 제외된 캐릭터만 `.is-camera-hidden`의 `transition: none`으로 즉시 사라지고 다인 `close` 주변 인물은 별도 페이드를 사용하던 퇴장 불일치를 수정했습니다. 이미지와 Live2D 모두 공통 180ms opacity 페이드 뒤 `visibility: hidden`이 되며, 퇴장 동안 마지막 PC·모바일 앵커와 시선 방향을 보존해 인원수 재배치 위치로 미끄러지거나 뒤집히는 현상을 막습니다. 빠른 진행의 80ms 노출 안정화로 건너뛴 중간 배우의 플래시를 제거하고, `wide/medium`의 2→1·3→2에서는 퇴장 완료 뒤 생존 배우를 재정렬하도록 전환 단계를 분리했습니다. 로딩 최초 노출과 `prefers-reduced-motion`은 지연하지 않으며, 최초 등장 도중 제외되는 한 프레임 노출과 재등장 시 `characterEnter` 반복도 함께 방지했습니다. DSL 문법 변경은 없습니다.
- 2026-08-13: 덕만 v6.1.0에 사건 사이의 관계 호흡과 선택 뒤 실행 장면을 보강했습니다. 가족의 늦은 저녁, 자매가 혼인 끈을 푸는 장면, 유신·비담의 역할 신경전, 기록관 무진의 이름, 덕만·진운의 식은 저녁상, 진평왕 사후 자매 애도, 즉위 뒤 첫 공개 재판을 추가했으며 마지막 네 전략은 법 문서·성벽 연설·항복 규칙·비담의 편지 장면을 거친 뒤 판정됩니다. 동일 캐릭터 슬롯·화자 우선순위·노출 집합·카메라 상태의 반복 스토어 갱신을 생략해 실제 구도 변화가 없는 연속 대사에서 등장·카메라 전환이 재시작될 여지를 제거했습니다. DSL 문법 변경은 없습니다.
- 2026-08-13: 덕만 v5.8.0의 전 8장 대사를 다시 감수하고 장면 인과를 확장했습니다. 덕만이 자기 방석을 요구하던 도입은 없는 왕자를 기다리며 오늘의 국사를 미루는 질서를 공격하는 장면으로 바꿨고, 진평왕은 공개석상마다 사실 확인 뒤 창고 개방·왕명 보호·혼인 거부·국경 지휘·후보 심문·판결을 직접 명령하도록 재구성했습니다. 4장에는 부러진 수레에서 의장품 대신 곡식과 약을 지키는 이동 장면을, 5장에는 왕이 직접 후보들의 말을 확인하는 장면을, 최종장에는 645년 승만 후계 공표와 646년 선덕왕의 직접 협상을 추가했습니다. 즉위한 선덕왕의 공식 말투는 `한다/하라`로 통일하고 사적인 자매 대화만 편한 말투를 유지했으며, 핵심 통치 선언 15곳에는 느린 타이핑·대기·스킵 방지를 선별 적용했습니다. DSL 문법 변경은 없습니다.
- 2026-08-13: 다인 `medium -> close`에서 비대상 인물의 180ms 페이드와 520ms 카메라 확대가 동시에 시작되어, 특히 모바일에서 사라지는 인물이 화면 가장자리로 밀리며 반만 보이는 문제를 수정했습니다. 비즉시 다인 `close`는 전체 전환 시간의 절반 이하·최대 160ms 안에 비대상 인물의 페이드와 `visibility: hidden`을 먼저 완료하고 남은 시간에 확대하며, `with` 제외 인물도 `opacity: 0`을 함께 적용해 합성기 한 프레임 노출을 방지합니다. 1인 `close`, `cut`, 전체 전환 시간은 그대로이고 DSL 문법 변경은 없습니다.
- 2026-08-18: `say.channel(dialogue/narration/record/system)`을 추가해 대사창과 스토리 로그에서 서술·기록·시스템 안내를 구분했습니다. `choice.options[].when`으로 상태·인벤토리 기반 조건부 선택지를 지원하고, 조건으로 타임아웃 대상이 숨겨진 경우 보이는 첫 옵션으로 안전하게 대체합니다. 역사 서사용 `eclipse/starfall/inkstamp` 화면 이펙트와 모션 감소 처리를 추가했습니다.
- 2026-08-13: pnpm 보안 감사에서 확인된 Vite 개발 서버 파일 읽기, js-yaml 비정상 입력 CPU 소모 및 연쇄 PostCSS/picomatch/nanoid 취약점을 해소했습니다. 직접 의존성 하한을 Vite `6.4.3`, js-yaml `4.3.1`로 올리고 호환 범위의 잠금 파일을 갱신했으며 `pnpm audit --audit-level low`에서 알려진 취약점 0건을 확인했습니다.
- 2026-08-13: 덕만 v5.7.0의 전 8장을 인과관계와 비주얼노벨 호흡 기준으로 다시 감수했습니다. 진평왕의 열병 후유증과 귀족 사병·창고 때문에 생긴 정치적 한계, 천명의 공포가 생긴 경험, 작은 서재 관리와 인질 단서, 진운의 빌린 글, 비담의 반복된 궂은일과 646년 다음 왕 조건 협상을 결과보다 먼저 배치했습니다. 최종 반란은 비담이 구호가 거짓임을 알면서도 군사를 택한 책임으로, 진엔딩은 덕만이 자기 과오를 인정하면서 상처와 죄를 분리해 판단하는 장면으로 보강했습니다. 화자 없는 내레이션·24개 선택 프롬프트·35개 실패 문구·10개 엔딩 문구·3개 증거 설명은 `-습니다/-습니까`의 시스템 존댓말로 통일했으며 DSL 문법 변경은 없습니다.
- 2026-08-13: `camera: close/reaction`의 의도적인 확대는 유지하되 PC 배율을 모바일 세로 화면에 그대로 적용해 `medium ↔ close` 반복 시 캐릭터 이미지가 약 28%씩 펌핑되던 문제를 수정했습니다. PC 최종 배율 `2.02/1.83`은 유지하고 모바일을 `1.84/1.72`로 완화했으며, pnpm을 `11.21.0`으로 올리고 삭제된 `package.json.pnpm.onlyBuiltDependencies`를 `pnpm-workspace.yaml`의 `allowBuilds`로 이전했습니다. DSL 문법 변경은 없습니다.
- 2026-08-13: 반드시 끝까지 출력해야 하는 대사를 위한 `say.unskippable` 불리언을 추가했습니다. 활성화된 문장은 타이핑 중 클릭/`Enter`/`Space`와 `clickToInstant`를 무시하고, `autoAdvance` 시간이 먼저 만료되어도 남은 글자를 생략하지 않은 채 타이핑 완료 직후 진행합니다. `wait`와 함께 쓰면 두 잠금 조건을 모두 적용하며, 기존 게임 콘텐츠에는 값을 자동 적용하지 않습니다.
- 2026-08-13: 인원수마다 정적 원본 높이·구성 배율·세로 원점을 다르게 조합해 최종 높이만 비슷하고 실제 머리 위치와 하체 크롭은 달랐던 문제를 수정했습니다. 덕만 v5.4.2와 엔진은 1·2·3인 모두 `58cqh × 1.58`, 원점 80%의 동일한 `medium` 구도를 사용하고, `wide/close/reaction` 및 `framing.scale`도 인원수 독립 배율로 통일합니다. 390×844와 1440×900에서 1·2·3인 이미지 박스의 y·높이·하단 좌표가 동일함을 실측했으며 DSL 문법 변경은 없습니다.
- 2026-08-13: 높이 기준 정규화 이전의 캐릭터 `scale/y` 값이 새 구성 배율과 중복 적용되던 문제를 수정했습니다. 덕만 v5.4.1은 10명 모두 `scale: 1`, `y: 0`으로 통일해 원화 자체의 체격만 남기고, PC 다인 배치는 보이는 배우들의 `spacing` 평균을 좌우에 대칭 적용하며 최대 간격을 260px로 조정했습니다. 기본·표정 원화 30종과 390×844·1440×900의 1·2·3인 구도를 검증했으며 DSL 문법 변경은 없습니다.
- 2026-08-13: 캐릭터의 고정 구성과 쇼트 확대를 `.char-composition-world`/`.char-camera-world`로 분리하고 대상 중앙 이동용 수평 팬을 제거했습니다. 1인은 중앙, 2인은 25%/75%, 3인은 25%/50%/75% 앵커를 유지한 채 `wide/medium/close/reaction`이 배율만 바꾸며, 정적 인물 높이와 PC 최대 간격을 플레이 프레임에 맞춰 정규화했습니다. 덕만 v5.4.0의 전 8장 기본 컷은 화면을 채우는 `medium`으로 이전하고 빈 무대·공간 설명만 `wide`로 남겼습니다. 390×844와 1440×900에서 2·3인 구성 및 쇼트 전환을 검증했으며 DSL 문법 변경은 없습니다.
- 2026-08-12: 캐릭터 원본 보정의 변형 원점을 캔버스 상단에서 무대 바닥으로 옮기고, 정적 이미지 폭을 플레이 프레임의 `cqw/cqh`에 함께 반응하도록 변경했습니다. 좌우 기본 슬롯과 카메라 이동을 동일한 25%/75% 중심 좌표로 통일하고, 단독 배우의 `medium/close/reaction`을 자동 중앙 정렬합니다. 표적 없는 `close/reaction` 완화는 실제 쇼트·포커스·비대상 배우 노출을 모두 `medium`으로 맞춥니다. 덕만 v5.3.1의 표정 원본 30개와 375×667, 390×844, 768×1024, 900×1600, 1440×900 구도를 검증했으며 DSL 문법 변경은 없습니다.
- 2026-08-12: 데스크톱과 모바일의 카메라 확대 배율을 분리하고 모바일 `medium/close/reaction`을 현재 보이는 1·2·3인에 맞춰 조정했습니다. 현재 대사 단위 카메라 표적을 별도 추적해 화자 없는 내레이션이 직전 `close/reaction` 화자를 계속 확대하는 문제를 막고, 표적이 없을 때는 현재 인원의 `medium` 그룹 구도로 안전하게 완화합니다. 덕만 v5.2.4의 기둥 은신·엿듣기·발각 장면을 375×667, 390×844, 1440×900에서 검증했으며 DSL 문법 변경은 없습니다.
- 2026-08-12: 세로 모바일의 `medium/close/reaction`이 전신 캐릭터를 화면 상단 쪽 원점에서 확대해 얼굴이 대사창까지 내려가던 문제를 수정했습니다. 모바일 카메라 원점을 현재 보이는 인원에 따라 1인 55%, 2인 80%, 3인 92%로 분리하고, 768px 이하 세로 프레임 판별을 포인터 종류와 무관한 반응형 조건으로 통일했습니다. 덕만 v5.2.3의 칠숙·진평왕 대화와 덕만 클로즈업을 390×844에서 검증했으며 DSL 문법 변경은 없습니다.
- 2026-08-12: 화면 밖 캐릭터의 DOM을 보존하면서 앙상블 레이아웃까지 전체 배치 인원으로 계산해 모바일 2인 컷이 3인용 15%/85% 슬롯으로 밀리던 회귀를 수정했습니다. 렌더 인스턴스는 계속 보존하되 무대 레이아웃·프레이밍·카메라 인원수는 현재 `with`에 보이는 배우만 사용합니다. 덕만 v5.2.2에 적용했으며 DSL 문법 변경은 없습니다.
- 2026-08-12: `with`에서 잠시 제외된 캐릭터를 React 트리에서 제거했다가 다음 대사에 다시 마운트해 `characterEnter`가 반복되던 문제를 수정했습니다. 배치된 배우는 무대에 유지하고 현재 카메라 밖에서는 `.is-camera-hidden`으로만 숨기며, 무대 레이아웃도 보이는 대사 상대가 아니라 실제 배치 상태를 기준으로 고정합니다. 덕만 v5.2.1은 프롤로그에서 진평왕의 반복 등장 모션을 제거하고 젖은 관리의 도착을 문·질문·청원서의 행동으로 보강했습니다. DSL 문법 변경은 없습니다.
- 2026-08-12: 대화 타이핑 중 높이가 바뀌는 다이얼로그를 따라 `.char-layer`의 `bottom`을 매번 갱신하던 결합을 제거했습니다. 캐릭터 무대는 중앙 플레이 프레임에 고정하고, `ResizeObserver`로 측정한 안전 여백은 스티커 레이어에만 적용합니다. 덕만 v5.2.0은 대관식의 불필요한 위치 이동을 고정 카메라 컷으로 바꾸고, 첫 등장 역할 카드·게임 설명 내레이션·추상적인 결론형 문장을 행동과 상호 대사로 교체했습니다. DSL 문법 변경은 없습니다.
- 2026-08-12: 런처의 수동 `overflow`/`scroll` 캐러셀을 Embla Carousel React 8.6.0으로 교체했습니다. 이동 프레임마다 전체 슬라이드 위치를 순회하고 선택 데이터·SEO·고지·이미지 로딩 속성을 다시 렌더링하던 경로를 제거하고, 선택 스냅 변경 시 상태를 한 번만 확정합니다. 대표 이미지 eager 로딩, 고지 DOM 안정화, 정적 manifest 대기 높이를 적용하고 시머·런처 진입 애니메이션을 제거했습니다. DSL 문법 변경은 없습니다.
- 2026-08-12: 엔진 홈 진입 시 이전 탭 선택을 복원하면서 캐러셀이 자동 이동해 첫 게임을 건너뛰던 동작을 제거했습니다. 일반 루트 진입은 항상 manifest 첫 게임을 정지 상태로 표시하고, 명시적 `?demo=` 공유 링크만 해당 슬라이드에 초기 애니메이션 없이 배치합니다. DSL 문법 변경은 없습니다.
- 2026-08-12: 런처 캐러셀을 헤더를 제외한 `100svh` 안에 고정해 PC·모바일 첫 화면에서 쇼케이스와 조작부가 함께 보이도록 했습니다. 매니페스트 로딩 상태도 같은 높이를 예약하며 긴 슬라이드는 외곽 레이아웃 대신 내부에서만 스크롤합니다. 활성 선택은 Embla `select`가 아니라 이동 완료 `settle`에서 확정하고, 페이지 점과 라벨 레일의 폭을 고정해 클릭·스와이프 중 데이터 교체와 컨트롤 레이아웃 시프트를 제거했습니다. 시스템 탭의 `게임 시작 화면으로 가기`를 명확히 복구하고 URL·ZIP 게임 모두 지원하며, 시작 직후 기본 버튼 문구가 바뀌거나 `이어하기`가 전환 중 노출되지 않도록 했습니다. DSL 문법 변경은 없습니다.
- 2026-08-12: 엔진 홈의 평상시 캐러셀 탐색에서 URL 해시 갱신을 제거하고, 명시적인 `선택 링크 복사`에서만 `?demo=<gameId>` 공유 주소를 생성하도록 변경했습니다. 기존 `#demo=` 링크는 자동 이전하며 다른 데모 탐색 시 오래된 선택 쿼리를 제거합니다. 데모별 `GitHub 폴더`를 `public/game-list/<gameId>`로 직접 연결하고, 엔진 제작 개요·가이드·샘플 YAML 진입점, 태그 축약/검색 복구, 접근 가능한 ZIP 입력, 모바일 가이드 노출과 런처 기본 브라우저 상호작용을 보강했습니다. DSL 문법 변경은 없습니다.
...
- 2026-08-11: 덕만 v3.1.0의 전 8장 대사를 쉬운 한국어로 다시 감수하고, 열 인물 모두 말보다 시선·손·말끝·소품으로 속마음이 먼저 새도록 연기 규칙을 통일했습니다. 사건을 해결하지 않는 안부·입맛·잠·옷매무새·농담의 생활 대사 24줄을 긴장 사이에 배치했으며, 평상시 27 CPS와 고백·협박·상실·결단 구간의 17~22 CPS를 대비했습니다. DSL 문법과 런타임 동작은 변경하지 않았습니다.
- 2026-08-11: 기록 보관소 모바일 레이아웃을 보정했습니다. 인벤토리 상단을 묶은 뒤 남아 있던 빈 그리드 행을 제거해 목록이 남은 높이를 채우도록 수정하고, 저장 보호 카드를 한 줄로 압축했으며 단서 상세를 단일 열로 전환했습니다. 좁은 화면의 탭 밀도와 짧은 가로 화면의 전체 높이 바텀시트도 함께 조정했습니다.
