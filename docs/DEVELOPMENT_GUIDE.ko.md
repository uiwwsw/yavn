...
- 모달 상단은 수집 진행 막대와 검색(이름), 카테고리, 정렬(`order` 우선/이름순)을 제공하며 두 탭에서 동일하게 적용합니다.
- `도감` 탭의 미획득 아이템은 잠금 카드(`미확인 아이템`)로 표시하고, 상세보기 팝업에서는 설명/이미지를 노출하지 않습니다.
- 모달 본문 그리드는 반응형 고정 열 수를 사용합니다(데스크톱 5열, 태블릿 4열, 모바일 3열, 400px 이하 2열).
- 슬롯 그리드 영역만 스크롤하며, 슬롯을 누르면 즉시 상세 팝업에 소지 여부, 카테고리, 설명(`description`), 이미지(`image`)를 표시합니다.
...
- 모바일에서는 상세 팝업을 하단 시트로 표시하고, 400px 이하에서 인벤토리 그리드를 2열로 줄입니다.
