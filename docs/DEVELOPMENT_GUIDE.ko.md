...
- 스티커·증거물은 화면 전체가 아니라 HUD·다이얼로그·기기 safe-area를 제외한 `.sticker-safe-frame`을 좌표 기준으로 사용합니다. 스티커·캐릭터 이미지 준비와 초기 캐릭터·카메라 전환의 최대 시간을 기다리고, 보이는 캐릭터의 실제 렌더 경계를 연속 측정해 같은 위치가 확인될 때까지 숨깁니다. 안정된 `.char-layer .char`와 겹치면 작성 위치에서 가장 가까운 좌·우·상단 빈 후보를 최종 위치로 선택합니다. 빈 공간이 없을 때만 단계적으로 균일 축소하며, 최초 안전 좌표는 전환 없이 확정한 뒤 입장 이펙트를 시작합니다. 이미 보이는 스티커에서 카메라 쇼트·실제 팬 이동이나 등장 인물 집합이 바뀌면 기존 배치를 같은 페인트에서 미확정 처리해 숨기고, 캐릭터 경계가 안정된 뒤 기존 좌표가 안전하면 유지하며 충돌하면 가장 가까운 안전 좌표를 다시 계산합니다. 확정된 위치에서는 160ms 페이드로 다시 노출해 이동 중인 캐릭터와 스티커가 한 프레임이라도 겹쳐 보이지 않게 합니다. 대사 타이핑과 단순 포커스·화자 변경은 재측정 키에서 제외합니다. `clearSticker.leave` 실행 시에는 `renderKey`와 마지막 안전 배치를 바꾸지 않고 같은 DOM의 퇴장 클래스만 갱신해, 재마운트·숨김 재측정·퇴장 타이머 사이의 경합을 방지합니다. 기기 회전이나 큰 PC 프레임 변경 때만 다시 숨기고 한 번 재배치하며, 모바일 주소창에 의한 높이 변화는 무시합니다.
...
- 스티커 안전 프레임은 중앙 프레임 안의 실제 다이얼로그 상단을 `ResizeObserver`로 측정해 `stickerSafeInset`을 반영합니다. 스티커는 초기 안정화가 끝난 시점의 대화창·보이는 인물 경계를 기준으로 충돌 없는 위치를 정합니다. 노출 후 카메라 쇼트·실제 팬 이동·등장 인물이 바뀌면 이전 안전 좌표를 즉시 미확정 처리하고 숨긴 채 전환 안정화를 기다립니다. 안정된 경계에서 기존 좌표가 안전하면 그 위치를 유지하고, 실제로 겹치면 가장 가까운 빈 공간으로 한 번 보정한 뒤 160ms 페이드로 재노출합니다. 기기 회전이나 의미 있는 PC 무대 크기 변경도 숨김-재측정-재노출 순서로 갱신하고, 대사 타이핑·단순 화자 변경·모바일 주소창 변화에는 좌표를 유지합니다.
