...
- 모바일과 데스크톱, 1·2·3인은 하나의 최종 배율을 공유합니다. `medium` 구성 배율은 `1.58`, `wide`는 `1.00`, `close`는 `2.02`, `reaction`은 `1.83`이며 구성 레이어의 세로 원점은 항상 80%입니다. `framing.scale`도 보이는 인원수에 따라 감쇠하지 않아 같은 프리셋이 같은 화면 비율을 뜻합니다. 쇼트 레이어는 대상 앵커에서 배율만 바꾸며 짧은 세로 화면에서도 타이핑 레이아웃 변화로 인물 위치를 다시 계산하지 않습니다.
