...
- `narration`은 대화창 위의 중복 `서술` 라벨을 표시하지 않고, 밝은 배경 이미지 위에서도 본문 대비를 유지하도록 템플릿별 고대비 전용 배경을 사용합니다. `paper-stage`는 완전 불투명한 종이색 바탕을 적용합니다. 스토리 로그의 채널 구분은 그대로 보존합니다.
...
- 2026-08-18: `narration` 채널이 그라디언트 토큰을 `color-mix()` 색상 인자로 사용해 전용 배경 선언이 무효가 되던 문제를 수정했습니다. 템플릿별 `--ui-narration-bg`를 분리하고 `paper-stage` 서술창에는 불투명한 종이색 바탕을 적용해 밝은 장면에서도 본문 대비를 유지합니다. DSL 문법 변경은 없습니다.
