...
- 화자 강조는 데스크톱/모바일 공통 규칙으로 동작합니다. 현재 화자는 가장 앞의 z축·원래 불투명도를 사용하고 비화자는 `speakerOrder` 순위에 따라 불투명도를 낮추지만, 같은 쇼트 안의 개별 캐릭터 크기는 바꾸지 않습니다. 다인 `close`의 `push/pan`은 전체 카메라 `duration`의 절반 이하이면서 최대 160ms를 비대상 인물 페이드에 먼저 배정하고, `visibility: hidden`이 된 뒤 남은 시간에 확대합니다. 퇴장 인물이 확대와 동시에 가장자리로 밀려 반만 남는 프레임과 투명 인물의 불필요한 합성을 막으며, 1인 `close`와 `cut`은 지연하지 않습니다. `reaction`은 듣는 카메라 대상을 전면에 둡니다.
...
- `say/choice/input.with`가 현재 카메라의 인물 구성을 좁혀도 배치된 캐릭터 DOM과 Live2D 인스턴스는 제거하지 않습니다. 화면 밖 인물에는 `.is-camera-hidden`의 `opacity: 0`과 `visibility: hidden`을 함께 적용해 즉시 숨기고 위치·표정 상태를 유지합니다. 다음 컷에서 같은 인물을 다시 보여 줄 때 `characterEnter`와 슬롯 이동 전환을 재실행하지 않으며, 실제 `char` 액션으로 새 슬롯이 배치되거나 교체될 때만 최초 등장으로 처리합니다.
