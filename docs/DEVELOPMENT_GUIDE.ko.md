...
- 실제 플레이 UI는 `.stage-content-frame` 안에서 렌더링합니다. PC에서는 `width: min(100cqw, 100cqh × 16 / 9)`와 `height: min(100cqh, 100cqw × 3 / 4)`를 함께 사용해 화면 안에서 가장 큰 `4:3 ~ 16:9` 프레임을 만들고 가로·세로 중앙에 놓습니다. 세로 모니터는 4:3, 중간 비율은 전체 화면, 울트라와이드는 16:9에서 멈춥니다. 터치 기반 세로 모바일(`hover: none`, `pointer: coarse`)은 기존 최대 9:16 프레임을 유지합니다.
...
- 데스크톱 확대는 캐릭터 원본이 화면 높이를 이미 채우므로 카메라 세로 원점을 상단 `0%`에 둡니다. 모바일 1인 `medium/close/reaction`도 상단 `0%` 원점과 아래 방향 `4cqh` 안전 헤드룸을 사용해 세로를 가득 채우는 에셋의 머리·장신구가 잘리지 않게 합니다. 모바일 2·3인은 폭 제약으로 원본 상단 여백이 생기므로 기존 `23%` 눈높이 원점을 유지합니다.
