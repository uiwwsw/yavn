...
- 스티커·증거물은 화면 전체가 아니라 HUD·다이얼로그·기기 safe-area를 제외한 `.sticker-safe-frame`을 좌표 기준으로 사용합니다. 스티커·캐릭터 이미지 준비와 초기 캐릭터·카메라 전환의 최대 시간을 기다리고, 보이는 캐릭터의 실제 렌더 경계를 연속 측정해 같은 위치가 확인될 때까지 숨깁니다. 안정된 `.char-layer .char`와 겹치면 작성 위치에서 가장 가까운 좌·우·상단 빈 후보를 최종 위치로 선택합니다. 빈 공간이 없을 때만 단계적으로 균일 축소하며, 최초 안전 좌표는 전환 없이 확정한 뒤 입장 이펙트를 시작합니다. 이미 보이는 스티커에 카메라·캐릭터 변경으로 새 충돌이 생기면 다음 안전 좌표·크기·배율을 다시 계산하고 280ms로 보간해 순간이동을 막습니다. `clearSticker.leave` 실행 시에는 `renderKey`와 마지막 안전 배치를 바꾸지 않고 같은 DOM의 퇴장 클래스만 갱신해, 재마운트·숨김 재측정·퇴장 타이머 사이의 경합을 방지합니다. 기기 회전이나 큰 PC 프레임 변경 때만 다시 숨기고 한 번 재배치하며, 모바일 주소창에 의한 높이 변화는 무시합니다.
...
- 스티커 안전 프레임은 중앙 프레임 안의 실제 다이얼로그 상단을 `ResizeObserver`로 측정해 `stickerSafeInset`을 반영합니다. 스티커는 초기 안정화가 끝난 시점의 대화창·보이는 인물 경계를 기준으로 충돌 없는 위치를 정합니다. 노출 후에는 안전한 좌표를 유지하되 카메라 쇼트·표적·등장 인물이 바뀐 뒤 현재 스티커와 새 캐릭터 경계가 실제로 겹칠 때만 전환 안정화를 기다려 가장 가까운 빈 공간으로 한 번 보정합니다. 기기 회전이나 의미 있는 PC 무대 크기 변경에서는 숨김-재측정-재노출 순서로 갱신하고, 대사 타이핑·충돌 없는 화자 전환·모바일 주소창 변화에는 좌표를 유지합니다.
...
- 계속 보이는 동일 인물의 슬롯 이동과 원본 보정/레거시 프레이밍 오프셋은 레이아웃을 일으키는 `left/width` 대신 합성 가능한 `translate`로 380ms 전환하고 크기는 독립 `scale`로 360ms 전환합니다. 등장·재등장·퇴장 중에는 두 전환을 끄고 불투명도만 처리합니다. 화자 호흡은 시선 반전용 `scaleX`를 보존한 합성 `transform`만 사용하므로 불투명도와 프레이밍 배율 전환을 덮어쓰지 않습니다. DSL의 `push` 기본 520ms, `pan` 기본 380ms, `cut` 즉시 전환은 유지하되, `push`는 수평 좌표를 즉시 맞춘 뒤 배율만 보간하고 명시적 `pan`만 카메라 수평 이동을 보간합니다.
...
- 캐릭터가 새로 노출되거나 숨김 상태에서 다시 잡힐 때 공통 `characterEnter`가 최종 배치 위치에서 240ms opacity 페이드를 실행합니다. 위로 튀는 이동이나 크기 바운스는 섞지 않으며, 계속 보이는 같은 ID의 위치·표정·구도 변경은 같은 렌더 키를 사용하므로 등장 애니메이션을 다시 실행하지 않습니다.
