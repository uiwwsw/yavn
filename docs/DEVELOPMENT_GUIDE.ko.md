...
- 초기 구성의 캐릭터가 한 명이면 원래 `left/center/right` 슬롯과 관계없이 50% 중앙 앵커를 사용합니다. 정확히 두 명이면 `.char-layout-duo`가 적용되어 25%/75%, 세 명이면 `.char-layout-trio`가 적용되어 25%/50%/75%의 구성 앵커를 사용합니다. 좌우 역할은 `left -> center -> right` 순서로 결정되어 화자 변경으로 뒤집히지 않습니다.
- PC는 모바일과 같은 구성 비율을 출발점으로 사용하되, 좌우 간격을 `min(25cqw, 260px) × 보이는 배우들의 평균 calibration.spacing`으로 제한합니다. 같은 평균값을 좌우에 적용하므로 에셋별 보정이 달라도 앙상블 중심이 기울지 않고, 넓은 화면에서는 배우들이 대화권 밖으로 지나치게 벌어지지 않습니다. `with`로 한 명을 숨기는 동안에는 기존 앙상블 간격을 유지합니다.
...
- `assets.characters.<id>.calibration(scale/x/y/spacing)`은 쇼트와 독립적으로 원본 캔버스의 인물 크기·여백을 정규화합니다. `spacing`은 PC 다인 구도에서 현재 보이는 배우들의 평균값으로 계산해 좌우에 대칭 적용하며 모바일 분할에는 영향을 주지 않습니다. 표정 에셋은 같은 보정값을 공유하므로 감정 교체 때 눈높이가 뛰지 않습니다. 이미 같은 높이의 캔버스로 정규화한 원화에는 중복 `scale/y` 보정을 넣지 않습니다.
...
- `say/choice/input.with`는 노출만 바꿉니다. 숨길 인물은 원래 자리·시선에서 260ms 페이드아웃하고, 계속 보이는 인물도 기존 자리를 유지합니다. 숨은 인물의 DOM·Live2D 인스턴스는 보존하며 다시 보이면 같은 자리에서 등장합니다. 슬롯을 다른 ID로 교체할 때도 퇴장 배우를 마지막 배치와 함께 보존했다가 페이드 후 제거합니다. 실제 `char.position`/배우 교체로 캐스팅이 달라질 때만 앙상블을 재배치하며 `layout: push`는 기존 배우를 보간하고 `cut`은 즉시 확정합니다. 빠른 노출 요청은 다음 페인트에 마지막 집합을 반영합니다.
- 구도는 현재 캐스팅에서 한 번 노출된 배우의 배치 집합을 유지합니다. 따라서 `with`로 한 명을 숨겨도 생존 배우가 중앙으로 미끄러지거나 서로 바라보던 방향이 뒤집히지 않습니다. 새 캐릭터 배치/슬롯 이동 시에는 다음 노출 집합을 기준으로 새 구도를 계산합니다. 전체 퇴장 중에도 기존 카메라 배율을 유지해 사라지는 인물이 축소되지 않습니다.
...
기존 작품의 대사마다 반복된 `camera: close/reaction`은 대화 시작의 `medium`으로 정리하고, 중요한 순간에만 명시 대상 컷을 남깁니다. `with`는 이제 노출만 제어하므로 재배치가 필요하면 `char.position`, 나레이션에서 확대를 풀려면 `camera: medium`을 지정합니다. 예전 전신 구도가 필요한 게임은 첫 장면에 `camera: wide`를 명시합니다. 덕만의 2026-09-09 콘텐츠 개편은 기존 scene/action 주소를 보존하고 새 조사 장면을 추가합니다. 인물 기본 배치는 `base.yaml`의 `placement: prompt-top`으로 맞춥니다. 이전 저장도 열리며, 이미 지나간 새 수집 구간은 해당 장을 다시 시작해 볼 수 있습니다. 코난/코난 데모/Live2D는 공통 화자·모바일 UI 개선을 자동 적용받습니다.
