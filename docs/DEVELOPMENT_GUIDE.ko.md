...
- 복구 화면은 선택 복구점을 우선 쓰는 `죽음의 원인 선택으로`, `수동 저장으로`, 자동으로 남긴 `챕터 처음으로`, 백업 파일 불러오기를 제공합니다. 선택 복구점은 선택 키·문구·인덱스도 별도 기록하고 실제 `gameOver`가 발생했을 때만 실패 이력으로 확정합니다. 복귀해 같은 선택 게이트에 도달하면 해당 항목을 저채도 회색 카드와 `원인 선택 · GAME OVER` 배지로 표시하되 비활성화하지 않습니다. 현재 키보드 포커스는 밝은 테두리로 구분하고 가능한 경우 다른 선택지에서 시작합니다. 직접 `gameOver` 옵션과 `goto` 뒤 독립 게임오버 모두 같은 규칙을 사용합니다.
...
장편 역사극의 채널·조건부 선택·누적 상태·다중 실패/엔딩 샘플은 `public/game-list/deokman/`을 참고합니다. 12개 챕터에서 `dialogue/narration/record`, 상태·인벤토리 기반 `when`, 외부 챕터 `goto`, 누적 상태 `branch`, 장면형 `gameOver`, 다중 `ending`을 함께 사용합니다.
