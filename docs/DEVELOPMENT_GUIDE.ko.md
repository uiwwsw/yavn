...
- 화자 강조는 정적 이미지·Live2D 공통의 180ms 밝기 전환만 사용합니다. 화자가 바뀌어도 몸의 크기·위치·시선·슬롯 안의 합성 순서는 유지하며, 반복 상하 이동이나 비균등 확대는 하지 않습니다. Live2D 자체의 표정·시선·모션은 계속 재생합니다. Live2D는 기본 `52cqh` 정사각 캔버스(`prompt-top`은 아래 안전 높이 적용)에 모델을 맞춘 뒤 공통 카메라를 적용하여 세로 투영과 확대가 겹쳐 머리가 잘리지 않도록 합니다.
...
- 화자만 확대하던 모바일/다인 depth 배율과 비화자 불투명도 감쇠는 제거되었습니다. 화자 구분은 밝기 전환이 담당하고, 쇼트 전체 크기 변화는 무대 카메라만 담당합니다.
...
기존 작품의 대사마다 반복된 `camera: close/reaction`은 대화 시작의 `medium`으로 정리하고, 중요한 순간에만 명시 대상 컷을 남깁니다. `with`는 이제 노출만 제어하므로 재배치가 필요하면 `char.position`, 나레이션에서 확대를 풀려면 `camera: medium`을 지정합니다. 예전 전신 구도가 필요한 게임은 첫 장면에 `camera: wide`를 명시합니다. 덕만은 카메라/등장 필드 값만 바꾸어 기존 세이브를 보존하며, 코난/코난 데모/Live2D 테스트는 별도 DSL 수정 없이 공통 엔진 개선을 적용합니다.
