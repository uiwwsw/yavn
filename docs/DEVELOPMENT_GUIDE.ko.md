...
- PC는 모바일과 같은 구성 비율을 출발점으로 사용하되, 좌우 간격을 `min(25cqw, 320px) × calibration.spacing`으로 제한합니다. 따라서 작은 화면에서는 두 번째 레퍼런스처럼 화면을 채우고, 넓은 화면에서는 배우들이 대화권 밖으로 지나치게 벌어지지 않습니다. 2인 중 한 명만 남으면 옆자리를 기억하지 않고 즉시 1인 중앙 구성으로 돌아옵니다.
...
- `assets.characters.<id>.calibration(scale/x/y/spacing)`은 쇼트와 독립적으로 원본 캔버스의 인물 크기·여백을 정규화합니다. `spacing`은 PC 다인 구도에서 해당 인물의 최대 중앙 간격만 조절하며 모바일 분할에는 영향을 주지 않습니다. 표정 에셋은 같은 보정값을 공유하므로 감정 교체 때 눈높이가 뛰지 않습니다.
...
- `assets.characters.<id>.calibration.spacing` (optional): `0.75..1.25`, 기본 `1`. PC 2·3인 구도에서 해당 에셋의 중앙 기준 최대 간격 배율
...
- PC 앙상블은 모바일과 같은 2인 25%/75%, 3인 25%/50%/75% 구성을 출발점으로 삼되 유동 간격에 320px 상한을 사용하며, `calibration.spacing`으로 넓거나 좁은 실루엣을 미세 조정합니다.
