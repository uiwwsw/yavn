...
- `/game-list/:gameId` 직접 진입과 Start Gate 실행 직후에는 게임 설정·본편 로딩이 끝날 때까지 비대화형 부트 화면을 유지합니다. 초기 런처·빈 HUD 렌더를 차단하고 지연 애니메이션에 양방향 fill mode를 적용해 버튼이 첫 프레임에 노출됐다가 다시 사라지는 플래시를 방지합니다.
...
- 2026-08-11: 직접 게임 URL의 첫 렌더와 Start Gate 실행 직후를 비대화형 부트 화면으로 보호하고, 타이틀·프레임·버튼 지연 애니메이션에 첫 키프레임 유지 처리를 추가했습니다. 느린 설정 요청과 iOS 첫 페인트에서도 런처·HUD·CTA가 잠깐 나타났다 사라지지 않으며 DSL 문법 변경은 없습니다.
