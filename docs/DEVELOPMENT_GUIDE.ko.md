...
- 스티커·증거물은 화면 전체가 아니라 HUD·다이얼로그·기기 safe-area를 제외한 `.sticker-safe-frame`을 좌표 기준으로 사용합니다. 스티커·캐릭터 이미지 준비와 초기 캐릭터·카메라 전환의 최대 시간을 기다리고, 보이는 캐릭터의 실제 렌더 경계를 연속 측정해 같은 위치가 확인될 때까지 숨깁니다. 안정된 `.char-layer .char`와 겹치면 작성 위치에서 가장 가까운 좌·우·상단 빈 후보를 최종 위치로 선택합니다. 빈 공간이 없을 때만 단계적으로 균일 축소하며, 확정한 좌표·크기는 픽셀로 잠가 대사창 높이와 후속 카메라 전환을 따라 이동하지 않습니다. 입장 이펙트도 위치 확정 뒤 시작합니다. `clearSticker.leave` 실행 시에는 `renderKey`와 잠긴 안전 배치를 바꾸지 않고 같은 DOM의 퇴장 클래스만 갱신해, 재마운트·숨김 재측정·퇴장 타이머 사이의 경합을 방지합니다. 기기 회전이나 큰 PC 프레임 변경 때만 다시 숨기고 한 번 재배치하며, 모바일 주소창에 의한 높이 변화는 무시합니다.
...
- 모든 캐릭터는 단일 절대 배율을 담당하는 `.char-composition-world`와 대상의 명시적 수평 이동을 담당하는 `.char-camera-world` 안에 유지됩니다. `transform-origin`을 대상마다 순간 변경하거나 구성 배율과 쇼트 배율을 중첩하지 않습니다. 따라서 같은 배율의 좌우 대상 전환도 실제 팬으로 이어지고 `medium -> close -> medium`의 크기 변화가 한 번의 보간으로 처리됩니다.
...
- `say/choice/input.with`가 현재 카메라의 인물 구성을 좁혀도 배치된 캐릭터 DOM과 Live2D 인스턴스는 제거하지 않습니다. 화면 밖 인물에는 이미지·Live2D 공통 `.is-camera-hidden` 퇴장 규칙을 적용해 180ms 동안 `opacity`를 낮추고 완료 시점에 `visibility: hidden`으로 합성에서 제외합니다. 퇴장하는 동안 캐릭터별 마지막 데스크톱/모바일 앵커·2인 역할·시선 반전을 보존하므로, 남은 인원의 레이아웃을 즉시 다시 계산해도 사라지는 인물이 중앙으로 이동하거나 뒤집히지 않습니다. 빠른 진행의 여러 노출 요청은 다음 페인트 프레임에 마지막 집합만 커밋해 건너뛴 대사의 배우가 짧게 나타났다 사라지는 플래시를 제거합니다. `wide/medium` 그룹 숏에서 기존 구성의 일부만 빠지는 2→1·3→2 전환은 이전 레이아웃을 퇴장 시간 동안 유지한 뒤 생존 배우의 위치 이동을 시작해 페이드와 재정렬이 동시에 경쟁하지 않게 합니다. 챕터 로딩 중 최초 노출은 안정화 지연 없이 즉시 커밋합니다. 최초 등장 중 곧바로 숨겨져도 강제 `opacity: 0`으로 한 프레임 노출을 막고, 한 번 보인 배우의 애니메이션 이름은 제거하지 않아 다음 컷에서 `characterEnter`가 다시 시작되지 않습니다. 아직 한 번도 보이지 않은 배치 배우는 숨김 중 진입 애니메이션을 대기했다가 최초 노출 때 한 번만 실행합니다. `prefers-reduced-motion`에서는 레이아웃 대기와 모션을 생략합니다.
...
- 동일 인물의 슬롯 이동과 원본 보정/레거시 프레이밍 오프셋은 레이아웃을 일으키는 `left/width` 대신 합성 가능한 `translate`로 380ms 전환하고 크기는 독립 `scale`로 360ms 전환합니다. 화자 호흡은 시선 반전용 `scaleX`를 보존한 합성 `transform`만 사용하므로 불투명도와 프레이밍 배율 전환을 덮어쓰지 않습니다. DSL의 `push` 기본 520ms, `pan` 기본 380ms, `cut` 즉시 전환은 유지하며 카메라 배율과 수평 팬은 같은 시간축을 사용합니다.
...
- 캐릭터가 처음 노출될 때만 공통 `characterEnter`가 최종 배치 위치에서 240ms opacity 페이드를 실행합니다. 위로 튀는 이동이나 크기 바운스는 섞지 않으며, 같은 ID의 위치·표정·구도 변경은 같은 렌더 키를 사용하므로 등장 애니메이션을 다시 실행하지 않습니다.
...
- `medium`은 1·2·3인 고정 구성에 동일한 기기별 최종 배율을 적용합니다. `close/reaction`은 대상 앵커를 명시적 수평 팬으로 화면 중앙에 옮기고 단일 절대 배율을 함께 전환합니다. `transform-origin`을 대상마다 바꾸지 않으므로 같은 배율의 좌우 대상 교대도 부드럽게 이동합니다.
