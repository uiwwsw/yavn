...
- 스티커·증거물은 화면 전체가 아니라 HUD·다이얼로그·기기 safe-area를 제외한 `.sticker-safe-frame`을 좌표 기준으로 사용합니다. 이미지 로드와 프레임 리사이즈마다 실제 렌더 경계를 측정하고, 보이는 `.char-layer .char`의 경계와 겹치면 작성 위치에서 가장 가까운 좌·우·상단 빈 후보로 옮깁니다. 빈 공간이 없을 때만 단계적으로 균일 축소하므로 큰 원본·고정 px·회전 에셋도 종횡비를 유지합니다. 최초 측정 전에는 스티커를 숨기고 `useLayoutEffect`에서 배치를 확정해, 작성 위치가 한 프레임 보였다가 이동하는 깜빡임을 막습니다.
...
- 스티커 레이어만 중앙 프레임 안의 실제 다이얼로그 상단을 `ResizeObserver`로 측정해 `stickerSafeInset`을 적용합니다. 스티커는 프레임·캐릭터 크기 변화와 카메라 전환 종료 시 충돌 배치를 다시 계산합니다. 따라서 증거물은 대화창과 보이는 인물을 피하지만, 인물은 대사나 아이템만 바뀌었다는 이유로 움직이지 않습니다.
...
- 다이얼로그가 수동 숨김 상태면 스티커 레이어의 하단 inset만 `0`으로 바꿉니다. 캐릭터 레이어는 숨김·복원·타이핑 상태와 무관하게 같은 플레이 프레임을 계속 사용합니다.
