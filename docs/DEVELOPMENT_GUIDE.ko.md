...
- 루트 런처는 manifest의 모든 게임을 전체 폭 데모 캐러셀로 렌더링합니다. 최초 선택 우선순위는 유효한 `?demo=<gameId>` 공유 쿼리, manifest 첫 게임 순서이며 이전 방문의 선택을 `sessionStorage`에서 복원하지 않습니다. 루트 진입은 첫 게임을 정지 상태로 표시하고, 공유 링크의 초기 위치도 스크롤 애니메이션 없이 즉시 맞춥니다. 예전 `#demo=<gameId>` 링크는 유효성을 확인한 뒤 쿼리 형식으로 자동 이전합니다.
...
- 캐러셀은 `embla-carousel-react` 안정판의 transform 기반 스냅 엔진을 사용합니다. 드래그·관성 이동 중 `scroll` 콜백을 사용하지 않고 `select` 이벤트에서 선택 스냅을 한 번만 확정하므로 SEO 메타, 권리 고지, 이미지 로딩 속성이 프레임마다 바뀌지 않습니다. 모든 대표 이미지와 권리 고지 DOM은 이동 전에 안정된 구조로 준비합니다.
- manifest 대기 화면은 쇼케이스와 같은 최소 높이를 가진 정적 상태이며 시머나 런처 진입 애니메이션을 사용하지 않습니다. 첫 데이터 렌더와 캐러셀 이동을 서로 다른 애니메이션처럼 겹쳐 보이게 하지 않습니다.
- 대표 슬라이드의 `GitHub 폴더` URL은 모든 게임에 공통으로 `<repositoryUrl>/tree/main/public/game-list/<encodedGameId>`에서 계산합니다. 데모 ID별 URL 분기를 추가하지 않습니다.
- 엔진 소개 영역은 YAML DSL·게임 흐름·웹 런타임의 역할과 제작 가이드·샘플 YAML 링크를 제공합니다. 검색은 명시적 지우기와 빈 결과 초기화를 지원하며, 태그는 사용 빈도순 상위 8개를 우선 표시합니다. 모바일에서도 제작 가이드와 키보드 접근 가능한 ZIP 입력을 유지하고 런처에서는 링크·버튼·텍스트 선택 같은 브라우저 기본 상호작용을 차단하지 않습니다.
